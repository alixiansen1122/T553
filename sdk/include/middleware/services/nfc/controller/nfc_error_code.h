/**
 * Copyright (c) HiSilicon (Shanghai) Technologies Co., Ltd. 2021. All rights reserved.
...
enum {
    ...
    /** @if Eng Memory set failure.
     *  @else   内存置位失败。 @endif */
    NFC_ERR_CODE_MEMSET_FAIL,
    ...
    /* NFC NDEF process error code. */
    /** @if Eng The function input parameters are invalid for MXX module.
     *  @else   MXX处理模块的函数入参非法。 @endif */
    NFC_ERR_CODE_MXX_PROC_INPUT_PARAM_INVALID = NFC_ERR_CODE_BEGIN + 0x400,
    /** @if Eng The selected MXX card is not exsisted or not inited.
     *  @else   所选mxx卡不存在或未初始化。 @endif */
    NFC_ERR_CODE_MXX_CARD_INVALID,
