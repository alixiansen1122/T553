...
/**
 * @if Eng
 * @brief The Class of Device Audio or Vido.
 * @else
 * @brief 音配设备类型。
 * @endif
 */
#define DEVICE_MAJOR_CLASS_AUDIO_VIDEO 0x400

/**
 * @if Eng
 * @brief The Class of Device Phone.
 * @else
 * @brief 手机设备类型。
 * @endif
 */
#define DEVICE_MAJOR_CLASS_PHONE 0x200
...
typedef enum {
    ...
    HFP_SCO_STATE_DISCONNECTING = 0x03, /*!< @if Eng status of sco is disconnecting. @else sco正在断连。 @endif */
    HFP_SCO_STATE_DISCONNECTED = 0x04,   /*!< @if Eng status of sco is disconnected. @else sco已断连。 @endif */
    HFP_SCO_STATE_CONNECT_FAILURE = 0x05 /*!< @if Eng status of sco connect is failure. @else sco连接失败。 @endif */
...
/**
 * @if Eng
 * @brief Use this funtion to set the inquiry parameters.
 * @par  Use this funtion to set the inquiry parameters.
 * @attention NULL
 * @param  [in] device_class #uint32_t, class of device to report.
 * @param  [in] max_num #uint8_t, max number of response devices.
 * @retval error code
 * @par Dependency:
 * @li  bts_gap.h
 * @else
 ...
 * @par    设置扫描参数。
 * @attention 无
 * @param  [in] device_class 设备类型。
 * @param  [in] max_num 最大上报数量。
 * @retval 执行结果错误码。
 * @par 依赖:
 * @li  bts_gap.h
 * @endif
 */
errcode_t gap_br_set_inquiry_paramters(uint32_t device_class, uint8_t max_num);
...
/**
 * @if Eng
 * @brief Callback function for a new connection is rejected.
 * @par   When registered, this callback notifys application a connect request is rejected.
 * @attention NULL
 * @param  [in] bd_addr #const bd_addr_t *, the address of the remote device.
 * @retval #void no return value.
 * @par Dependency:
 ...
 * @see  gap_call_backs_t
 * @else
 * @brief  通知上层应用有个新的连接请求被拒绝了。
 * @par    注册该回调函数之后，bluetooth调用该回调函数通知上层应用有个新的连接请求被拒绝了。
 * @attention 无
 * @param  [in] 远端设备地址。
 * @retval #void  无返回值。
 * @par 依赖:
 ...
 * @endif
 */
typedef void (*gap_connect_rejection_report_callback)(const bd_addr_t *bd_addr);
...
/**
 * @if Eng
 * @brief Use this funtion to reconnect a media device.
 * @par   Use this funtion to reconnect a media device.
 * @attention NULL
 * @param  [in] addr #const bd_addr_t *, address of the device.
 * @retval #true Execute successfully.
 * @retval #false Execution failed.
 ...
 * @endif
 */
errcode_t gap_reconnect_media_device(const bd_addr_t *addr);

/**
 * @if Eng
 * @brief Use this funtion to set bt flow limit.
 * @par   Use this funtion to set bt flow limit.
 * @attention NULL
 * @param  [in] flow_limit flow limit max bytes,range is [3072-10240] bytes.
 * @retval #true Execute successfully.
 * @retval #false Execution failed.
 * @par Dependency:
 * @li  bts_br_gap.h
 ...
 * @par    配置流控限制字节数。
 * @attention 无
 * @param  [in] 流控限制字节数,范围是[3072-10240]字节数。
 * @retval #true  连接成功。
 * @retval #false  连接失败。
 * @par 依赖:
 * @li  bts_br_gap.h
 * @endif
 */
errcode_t gap_bt_set_flow_limit(uint16_t flow_limit);

/**
 * @if Eng
 * @brief Register callback function for connection rejected event notification.
 * @par   Register callback function for connection rejected event notification.
 * @attention NULL
 * @param  [in] func #gap_connect_rejection_report_callback, a pointer to the callback func.
 * @retval #void no return value.
 * @par Dependency:
 * @li  bts_br_gap.h
 * @else
 * @brief  注册拒绝新手机连接事件上报回调函数。
 * @par    注册拒绝新手机连接事件上报回调函数。
 * @attention 无
 * @param  [in] 指向回调函数的指针。
 * @retval #void 无
 * @par 依赖:
 ...
 * @endif
 */
void gap_register_br_connect_rejection_report_callback(gap_connect_rejection_report_callback func);
/**
 * @}
 */
