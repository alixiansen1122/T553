...
/**
 * @if Eng
 * @brief Enum of pan Network protocol type.
 * @else
 * @brief Pan 网络协议类型
 * @endif
 */
typedef enum {
    PAN_NETWORK_PROTOCOL_IPV4 = 0x0800,    /*!< @if Eng Network protocol is IPV4.
                                                @else 网络协议是IPV4 @endif */
    PAN_NETWORK_PROTOCOL_ARP = 0x0806,     /*!< @if Eng Network protocol is ARP.
                                                @else 网络协议是ARP @endif */
} pan_network_protocol_t;

/**
 * @if Eng
 * @brief Enum of pan packet type.
 * @else
 * @brief Pan 报文类型
 * @endif
 */
typedef enum {
    PAN_PACKET_TYPE_DEST_ONLY_UNICAST = 0,    /*!< @if Eng Packet type is dest only unicast.
                                                @else 报文类型是保留目的地址的单播 @endif */
    PAN_PACKET_TYPE_DEST_ONLY_BORADCAST = 1,     /*!< @if Eng Packet type is dest only broadcast.
                                                @else 报文类型是保留目的地址的广播 @endif */
    PAN_PACKET_TYPE_GENERAL_UNICAST = 2,     /*!< @if Eng Packet type is general unicast.
                                                @else 报文类型是通用类型的单播 @endif */
    PAN_PACKET_TYPE_GENERAL_BORADCAST = 3,     /*!< @if Eng Packet type is general broadcast.
                                                @else 报文类型是通用类型的广播 @endif */
    PAN_PACKET_TYPE_COMPRESSED = 4,     /*!< @if Eng Packet type is compressed.
                                                @else 报文类型是压缩头报文 @endif */
} pan_ethernet_packet_type_t;
 ...
 * @param  [in]  network_protocol network protocol, see @ref pan_network_protocol_t.
 * @param  [in]  packet_type      packet type, see @ref pan_ethernet_packet_type_t
 * @retval #ERRCODE_BT_SUCCESS   Success.
 * @retval Other                Failure. For details, see @ref errcode_bt_t
 * @else
 * @brief 发送网络数据包
 ...
 * @param  [in]  network_protocol 网络协议类型, see @ref pan_network_protocol_t
 * @param  [in]  packet_type      报文类型, see @ref pan_ethernet_packet_type_t
 * @retval #ERRCODE_BT_SUCCESS   成功.
 * @retval Other                失败. 参考 @ref errcode_bt_t
 * @endif
 * @par Dependency:
...
int pan_service_write_data(const unsigned char *ip_packet, const unsigned short ip_length,
    uint16_t network_protocol, uint8_t packet_type);
 ...
 *            @li pan_service.h
 */
typedef void (*pan_rpt_data_callback)(unsigned char *packet, unsigned short packet_length, uint16_t network_protocol);
