...
typedef enum {
    ...
    GAP_BLE_ADV_CONN_NONSCAN_DIR,      /*!< @if Eng Connectable, nonscanble, directed advertising
                                            @else   可连接不可扫描定向广播 @endif */
...
/**
 * @if Eng
 * @brief Struct of ble connect parameter update struct,the reference frame is [Core_v5.3 page 2382].
 * @else
 * @brief ble连接参数更新数据结构，参考蓝牙Core协议【Core_v5.3 page 2382】。
 * @endif
 */
typedef struct {
    uint16_t conn_handle;        /*!< @if Eng connection id
                                      @else   连接 ID @endif */
    uint16_t interval_min;       /*!< @if Eng min interval,the value range is 6-3200,the unit is 1.25ms
                                      @else   最小间隔,值范围6-3200,单位为1.25ms @endif */
    uint16_t interval_max;       /*!< @if Eng max interval,the value range is 6-3200,the unit is 1.25ms
                                      @else   最大间隔,值范围6-3200,单位为1.25ms @endif */
    uint16_t slave_latency;      /*!< @if Eng slave reply min latency,the value range is 0-499
                                      @else   从设备回复最小间隔，值范围0-499 @endif */
    uint16_t timeout_multiplier; /*!< @if Eng interval for disconnection due to timeout,
                                              the value range is 10-3200,the unit is 10ms
                                      @else   超时断连间隔，值范围10-3200，单位为10ms @endif */
...
typedef struct {
    uint32_t min_interval;     /*!< @if Eng Min interval[N * 0.625ms]
                                            The value range: 0x0020 to 0x4000
                                    @else   最小的广播间隔[N * 0.625ms]
                                            可取值范围: 0x0020 to 0x4000 @endif */
...
/**
 * @if Eng
 * @brief Enum of ble authentication result.
 * @else
 * @brief BLE认证结果。
 * @endif
 */
 ...
 * @param  [in] param  a pointer to the advertising parameter.
                       Parameter configuration reference gap_ble_adv_params_t description.
 ...
 * @attention 无
 * @param  [in] adv_id 广播ID
 * @param  [in] param  广播参数。参数设置参考 gap_ble_adv_params_t 描述。
...
/**
 * @if Eng
 * @brief Use this funtion to remove all bond devices info from the paired list.
 * @par   Use this funtion to remove all paired devices info from the paired list.
 * @attention NULL
 * @param  NULL
 * @retval error code.
 * @par Dependency:
 ...
 * @endif
 */
errcode_t gap_ble_remove_all_bond_devices(void);
...
/**
 * @if Eng
 * @brief Use this funtion to set ctkd enable.
 * @par   Use this funtion to set ctkd enable.
 * @attention NULL
 * @param  [in] param  ctkd enable, true：enable，false：disable
 ...
 * @par    设置一键双连开关。
 * @attention 无
 * @param  [in] param  一键双连开关 true：打开 false：关闭
 * @retval 执行结果错误码。 { @ref errcode_bt_t }
 * @par 依赖:
 * @li  bts_def.h
 * @endif
 */
errcode_t gap_ble_set_ctkd_enable(bool ctkd_enable);
...
/**
 * @if Eng
 * @brief Use this funtion to get connection param.
 * @par   Use this funtion to get connection param by connect id.
 * @attention NULL
 * @param  [in] connect id.
 * @param  [out] conn_param.
 * @retval error code  @ref errcode_bt_t.
 * @par Dependency:
 * @li  bts_def.h
 * @else
 * @brief  获取连接参数信息
 ...
 * @endif
 */
errcode_t gap_ble_get_connection_param(uint16_t conn_id, gap_ble_conn_param_update_t *conn_param);
