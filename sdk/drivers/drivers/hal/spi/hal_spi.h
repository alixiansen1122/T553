...
typedef enum hal_spi_trans_type {
    ...
    HAL_SPI_TRANS_TYPE_INST_S_ADDR_Q,       /*!< @if Eng Instruction will be sent in standard mode and address will
                                                         be sent in mode specified by @ref ctrlr0_data_t.spi_frf.
                                                 @else   指令使用单线SPI传输，
                                                         地址按照 @ref ctrlr0_data_t.spi_frf 寄存器的配置传输。 @endif */

    HAL_SPI_TRANS_TYPE_INST_Q_ADDR_Q,       /*!< @if Eng Both instruction and address will be sent in
                                                         the mode specified by @ref ctrlr0_data_t.spi_frf.
                                                 @else   指令和地址都按照 @ref ctrlr0_data_t.spi_frf
                                                         寄存器的配置传输。 @endif */
...
typedef struct hal_spi_attr {
    ...
    uint32_t freq_mhz;                  /*!< @if Eng Indicates the frequency of SPI in MHz.
                                                     Only one of freq_mhz or freq_hz can be non-zero and effective.
                                             @else   SPI的工作频率（MHz）。
                                                     freq_mhz和freq_hz只能有一个非零且有效。 @endif */
#ifdef CONFIG_SPI_SUPPORT_HZ
    uint32_t freq_hz;                   /*!< @if Eng Indicates the frequency of SPI in Hz.
                                                     Only one of freq_mhz or freq_hz can be non-zero and effective.
                                             @else   SPI的工作频率（Hz）。
                                                     freq_mhz和freq_hz只能有一个非零且有效。 @endif */
#endif
 ...
 * @endif
 */
typedef errcode_t (*hal_spi_init_t)(spi_bus_t bus, const hal_spi_attr_t *attr,
 ...
 * @endif
 */
typedef errcode_t (*hal_spi_deinit_t)(spi_bus_t bus);
 ...
 * @endif
 */
typedef errcode_t (*hal_spi_write_t)(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout);

/**
 * @if Eng
 * @brief  Read interface for hal SPI.
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t
 ...
 * @endif
 */
typedef errcode_t (*hal_spi_read_t)(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout);

/**
 * @if Eng
 * @brief  Write and read interface for hal SPI.
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t
 * @param  [in]  data Transfer data. For details, see @ref hal_spi_xfer_data_t
 * @param  [in]  timeout Timeout duration.
 * @retval ERRCODE_SUCC Success.
 ...
 * @endif
 */
typedef errcode_t (*hal_spi_writeread_t)(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout);
...
/**
 * @if Eng
 * @brief  Interface between SPI driver and SPI hal.
 * @else
 * @brief  Driver层SPI和HAL层SPI的接口。
 * @endif
 */
typedef struct {
    hal_spi_init_t          init;               /*!< @if Eng Init device interface.
                                                     @else   HAL层SPI的初始化接口。 @endif */
    hal_spi_deinit_t        deinit;             /*!< @if Eng Deinit device interface.
                                                     @else   HAL层SPI去初始化接口。 @endif */
    hal_spi_write_t         write;              /*!< @if Eng Write data interface.
                                                     @else   HAL层SPI发送数据接口。 @endif */
    hal_spi_read_t          read;               /*!< @if Eng Read data interface.
                                                     @else   HAL层SPI读取数据接口。 @endif */
    hal_spi_writeread_t     writeread;          /*!< @if Eng Write and read data interface.
                                                     @else   HAL层SPI发送和读取数据接口。 @endif */
    hal_spi_ctrl_t          ctrl;               /*!< @if Eng Control device interface.
                                                     @else   HAL层SPI控制接口。 @endif */
} hal_spi_funcs_t;
...
/**
 * @if Eng
 * @brief  Deinit the registers of SPI IPs.
 * @else
 * @brief  去初始化寄存器基地址列表。
 * @endif
 */
...
/**
 * @if Eng
 * @brief  Register @ref hal_spi_funcs_t into the g_hal_spis_funcs.
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t
 * @param  [in]  funcs Interface between SPI driver and SPI hal.
 * @retval ERRCODE_SUCC Success.
 * @retval Other        Failure. For details, see @ref errcode_t
 * @else
 * @brief  注册 @ref hal_spi_funcs_t 到 g_hal_spis_funcs。
 * @param  [in]  bus 串口号。参考 @ref spi_bus_t
 * @param  [in]  funcs Driver层SPI和HAL层SPI的接口实例。
 * @retval ERRCODE_SUCC 成功。
 * @retval Other        失败。参考 @ref errcode_t
 * @endif
 */
...
/**
 * @if Eng
 * @brief  Unregister @ref hal_spi_funcs_t from g_hal_spis_funcs.
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t
 * @retval ERRCODE_SUCC Success.
 * @retval Other        Failure. For details, see @ref errcode_t
 * @else
 * @brief  从g_hal_spis_funcs注销 @ref hal_spi_funcs_t 。
 * @param  [in]  bus 串口号。参考 @ref spi_bus_t
 * @retval ERRCODE_SUCC 成功。
 * @retval Other        失败。参考 @ref errcode_t
 * @endif
...
/**
 * @if Eng
 * @brief  Get interface between SPI driver and SPI hal, For details, see @ref hal_spi_funcs_t
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t
 * @return Interface between SPI driver and SPI hal, For details, see @ref hal_spi_funcs_t
 * @else
 * @brief  获取Driver层SPI和HAL层SPI的接口实例。参考 @ref hal_spi_funcs_t
 * @param  [in]  bus 串口号。参考 @ref spi_bus_t
 * @return Driver层SPI和HAL层SPI的接口实例。参考 @ref hal_spi_funcs_t
 * @endif
 */
