...
#define SPI_MINUMUM_CLOCK_IN_HZ     10000
#define SPI_MAXIMUM_CLOCK_IN_HZ     48000000
...
static bool hal_spi_is_attr_freq_valid(const hal_spi_attr_t *attr, const hal_spi_frame_format_t spi_frame_format)
{
#ifdef CONFIG_SPI_SUPPORT_HZ
    if ((attr->freq_hz == 0 && attr->freq_mhz == 0) ||
        (attr->freq_hz != 0 && attr->freq_mhz != 0)) {
        return false;
    }
    if (attr->freq_hz != 0) {
        if (spi_frame_format == HAL_SPI_FRAME_FORMAT_STANDARD) {
            if ((attr->freq_hz < SPI_MINUMUM_CLOCK_IN_HZ) || (attr->freq_hz > SPI_MAXIMUM_CLOCK_IN_HZ)) {
                return false;
            }
        } else {
            return false;
    ...
    if (attr->freq_mhz != 0) {
        if (spi_frame_format == HAL_SPI_FRAME_FORMAT_STANDARD) {
            if ((attr->freq_mhz < SPI_MINUMUM_CLOCK_IN_MHZ) || (attr->freq_mhz > SPI_MAXIMUM_CLOCK_IN_MHZ)) {
                return false;
            }
        } else {
            if ((attr->freq_mhz < QSPI_MINUMUM_CLOCK_IN_MHZ) || (attr->freq_mhz > QSPI_MAXIMUM_CLOCK_IN_MHZ)) {
                return false;
            }
        }
        return true;
...
static bool hal_spi_is_attr_valid(spi_bus_t bus, const hal_spi_attr_t *attr, const hal_spi_extra_attr_t *extra_attr)
{
    ...
    if (attr->is_slave == 0) {
        ...
        if (hal_spi_is_attr_freq_valid(attr, spi_frame_format) != true) {
            return false;
        }
    }
    return true;
...
{
    hal_spi_frame_format_t spi_frame_format = (hal_spi_frame_format_t)hal_spi_v100_ctrlr0_get_spi_frf(bus);
...
static bool hal_spi_is_busy(spi_bus_t bus, uint32_t timeout)
{
    ...
    while (hal_spi_v100_sr_get_tfe(bus) != 1 || hal_spi_v100_sr_get_rfne(bus) != 0 ||
           hal_spi_v100_sr_get_busy(bus) != 0) {
...
{
    hal_spi_v100_ssienr_set_ssi_en(bus, 0);
    hal_spi_v100_spi_ctrlr0_set_trans_type(bus, qspi_param->trans_type);
    hal_spi_v100_spi_ctrlr0_set_inst_l(bus, qspi_param->inst_len);
    hal_spi_v100_spi_ctrlr0_set_addr_l(bus, qspi_param->addr_len);

    uint32_t wait_cyc = qspi_param->wait_cycles;
    if (wait_cyc > QSPI_WAIT_CYCLE_MAX) {
        wait_cyc = QSPI_WAIT_CYCLE_MAX;
    }
    hal_spi_v100_spi_ctrlr0_set_wait_cycles(bus, wait_cyc);
    hal_spi_v100_ssienr_set_ssi_en(bus, 1);
}
...
static void hal_spi_config_sspi(spi_bus_t bus, const hal_spi_xfer_sspi_param_t *sspi_param)
{
    ...
    hal_spi_v100_rsvd_set(bus, sspi_param->wait_cycles);
}
#endif
...
static void hal_spi_init_config_spi_frame_format(spi_bus_t bus, hal_spi_frame_format_t spi_frame_format)
{
    if (spi_frame_format == HAL_SPI_FRAME_FORMAT_STANDARD ||
        ...
        hal_spi_v100_ctrlr0_set_spi_frf(bus, (uint32_t)spi_frame_format);
    } else {
        if (spi_frame_format == HAL_SPI_FRAME_FORMAT_OCTAL ||
            spi_frame_format == HAL_SPI_FRAME_FORMAT_DOUBLE_OCTAL ||
            spi_frame_format == HAL_SPI_FRAME_FORMAT_SIXT) {
            hal_spi_v100_ctrlr0_set_spi_frf(bus, (uint32_t)HAL_SPI_FRAME_FORMAT_OCTAL);
        }
    }
}

static void hal_spi_init_config_freq(spi_bus_t bus, const hal_spi_attr_t *attr)
{
    uint32_t clk_div = SPI_MINUMUM_CLK_DIV;
#ifdef CONFIG_SPI_SUPPORT_HZ
    if (attr->freq_hz != 0) {
        clk_div = (uint32_t)(attr->bus_clk / attr->freq_hz);
...
#endif
    if (attr->freq_mhz != 0) {
        clk_div = (uint32_t)(attr->bus_clk / spi_mhz_to_hz(attr->freq_mhz));
        if (clk_div < SPI_MINUMUM_CLK_DIV) {
            clk_div = SPI_MINUMUM_CLK_DIV;
        }
    }

    hal_spi_v100_baudr_set_sckdv(bus, clk_div);
}
...
static bool hal_spi_init_config(spi_bus_t bus, const hal_spi_attr_t *attr, const hal_spi_extra_attr_t *extra_attr)
{
    ...
    hal_spi_v100_ssienr_set_ssi_en(bus, 0);
    hal_spi_v100_ctrlr0_set(bus, 0);
    hal_spi_v100_spi_ctrlr0_set(bus, 0);

    hal_spi_init_config_spi_frame_format(bus, (hal_spi_frame_format_t)attr->spi_frame_format);
    hal_spi_v100_ctrlr0_set_frf(bus, (hal_spi_cfg_frame_format_t)attr->frame_format);

    /* Config the clock phase and polarity. */
    hal_spi_v100_ctrlr0_set_scph(bus, attr->clk_phase);
    hal_spi_v100_ctrlr0_set_scpol(bus, attr->clk_polarity);

    /* Config the frame size. */
    hal_spi_v100_ctrlr0_set_dfs_32(bus, attr->frame_size);
    hal_spi_v100_ctrlr0_set_cfs(bus, attr->frame_size);

    /* Config the transfer mode and FIFO threshold. */
    hal_spi_v100_ctrlr0_set_tmod(bus, attr->tmod);
    if (attr->frame_format == SPI_CFG_FRAME_FORMAT_NS_MICROWIRE && attr->tmod == HAL_SPI_TRANS_MODE_TX) {
        hal_spi_v100_mwcr_set_mdd(bus, 1);
    }
    hal_spi_v100_txftlr_set_tft(bus, CONFIG_SPI_TXFTLR);
    hal_spi_v100_rxftlr_set_rft(bus, CONFIG_SPI_RXFTLR);

    /* Config slave info and freq in master mode. */
    if (attr->is_slave == 0) {
        hal_spi_v100_ser_set_ser(bus, (attr->slave_num == 0) ? 0 : bit(attr->slave_num - 1));
        hal_spi_init_config_freq(bus, attr);
    ...
    hal_spi_v100_ctrlr0_set_sste(bus, attr->sste);

    /* Enable the SPI and all of the interrupts. */
    hal_spi_v100_ssienr_set_ssi_en(bus, 1);
    hal_spi_v100_int_set(bus, INT_MASK_REG, 0);

    (void)memcpy_s(&g_hal_spi_attrs[bus], sizeof(hal_spi_attr_t), attr, sizeof(hal_spi_attr_t));
    if (extra_attr) {
        (void)memcpy_s(&g_hal_spi_extra_attrs[bus], sizeof(hal_spi_extra_attr_t),
                       extra_attr, sizeof(hal_spi_extra_attr_t));
    }
                                               ...
                                               hal_spi_frame_format_t spi_frame_format)
{
    if (frame_bytes == SPI_FRAME_BYTES_1) {
        hal_spi_v100_dr_set_dr(bus, (uint8_t)(*data));
    } else {    /* SPI_FRAME_BYTES_4 */
        if (spi_frame_format == HAL_SPI_FRAME_FORMAT_STANDARD) {
            hal_spi_v100_dr_set_dr(bus, hal_spi_trans_bytes_to_word(data));
        } else {    /* SPI_FRAME_FORMAT_QUAD */
            hal_spi_v100_dr_set_dr(bus, *(uint32_t *)data);
        }
    }
}
...
static errcode_t hal_spi_write(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout)
{
    ...
    uint32_t frame_bytes = hal_spi_frame_size_trans_to_frame_bytes(hal_spi_v100_ctrlr0_get_dfs_32(bus));
    ...
    if (unlikely(spi_porting_get_device_mode(bus) == SPI_MODE_MASTER &&
        ...
        if (hal_spi_v100_spi_ctrlr0_get_inst_l(bus) != 0) {
            hal_spi_v100_dr_set_dr(bus, data->cmd);
        }
        if (hal_spi_v100_spi_ctrlr0_get_addr_l(bus) != 0) {
            hal_spi_v100_dr_set_dr(bus, data->addr);
        }
    }
    ...
    while (trans_frames > 0) {
        if (hal_spi_v100_sr_get_tfnf(bus) != 0) {
...
{
    if (frame_bytes == SPI_FRAME_BYTES_1) {
        *data = (uint8_t)hal_spi_v100_dr_get_dr(bus);
    } else {    /* SPI_FRAME_BYTES_4 */
        if (spi_frame_format == HAL_SPI_FRAME_FORMAT_STANDARD) {
            uint32_t data_read = hal_spi_v100_dr_get_dr(bus);
            ...
            *(uint32_t *)data = hal_spi_v100_dr_get_dr(bus);
        }
    }
}
...
static bool hal_spi_read_rx_mode_prepare(spi_bus_t bus)
{
    ...
    if (spi_porting_get_device_mode(bus) == SPI_MODE_MASTER &&
        hal_spi_v100_ctrlr0_get_tmod(bus) == HAL_SPI_TRANS_MODE_RX) {
        hal_spi_v100_dr_set_dr(bus, 0); /* 解决主机读场景时，先任意写一个数据拉低CS发出时钟信号。 */
        while (hal_spi_v100_rxflr_get_rxtfl(bus) == 0) {
...
static errcode_t hal_spi_read(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout)
{
    ...
    while (trans_frames > 0) {
        if (hal_spi_v100_sr_get_rff(bus) == 1) {
            return ERRCODE_SPI_RX_FIFO_FULL;
        }

        if (!hal_spi_read_rx_mode_prepare(bus)) {
            return ERRCODE_SPI_TIMEOUT;
        }

        uint32_t rx_level = hal_spi_v100_rxflr_get_rxtfl(bus);
        while (rx_level > 0) {
...
static void hal_spi_write_unit(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t frame_bytes,
                               uint32_t *trans_frames, hal_spi_frame_format_t spi_frame_format)
{
    if (*trans_frames == 0) {
        return;
    }

    uint8_t *tx_buff = data->tx_buff + (data->tx_bytes - (*trans_frames) * frame_bytes);
    uint32_t wait_time = 0;
...
static errcode_t hal_spi_read_unit(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t frame_bytes,
                                   uint32_t *trans_frames, hal_spi_frame_format_t spi_frame_format)
{
    if (*trans_frames == 0) {
        return ERRCODE_SUCC;
    ...
    uint8_t *rx_buff = data->rx_buff + (data->rx_bytes - (*trans_frames) * frame_bytes);
...
static errcode_t hal_spi_writeread(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout)
{
    uint8_t *tx_buff = data->tx_buff;
    uint32_t frame_bytes = hal_spi_frame_size_trans_to_frame_bytes(hal_spi_v100_ctrlr0_get_dfs_32(bus));
    hal_spi_frame_format_t spi_frame_format = hal_spi_get_frame_format(bus);
    uint32_t tx_trans_frames = data->tx_bytes / frame_bytes;
    uint32_t rx_trans_frames = data->rx_bytes / frame_bytes;
    uint32_t trans_time = 0;

    if ((data->tx_bytes % frame_bytes != 0) || (data->rx_bytes % frame_bytes != 0)) {
        return ERRCODE_SPI_INVALID_BYTES;
    }

    if (spi_frame_format == HAL_SPI_FRAME_FORMAT_QUAD) {
        /* qspi mode: send cmd and register addr */
        if (hal_spi_v100_spi_ctrlr0_get_inst_l(bus) != 0) {
            hal_spi_v100_dr_set_dr(bus, data->cmd);
        }
    ...
    while (tx_trans_frames > 0 || rx_trans_frames > 0) {
        hal_spi_write_unit(bus, data, frame_bytes, &tx_trans_frames, spi_frame_format);
        if (hal_spi_read_unit(bus, data, frame_bytes, &rx_trans_frames, spi_frame_format) != ERRCODE_SUCC) {
            return ERRCODE_SPI_READ_ERROR;
        }
        if (hal_spi_check_timeout_by_count(&trans_time, timeout)) {
            return ERRCODE_SPI_TIMEOUT;
        }
...
static errcode_t hal_spi_ctrl_set_attr(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    if (attr->frame_format == SPI_CFG_FRAME_FORMAT_NS_MICROWIRE) {
        if (attr->tmod == HAL_SPI_TRANS_MODE_TX) {
            ...
            hal_spi_v100_mwcr_set_mdd(bus, 0);
        }
    }
    hal_spi_v100_ctrlr0_set_tmod(bus, attr->tmod);
    ...
    if (attr->ndf > 0) {
        hal_spi_v100_ctrlr1_set_ndf(bus, attr->ndf - 1);
    }
...
static errcode_t hal_spi_ctrl_select_slave(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    if (slave > spi_porting_max_slave_select_get(bus)) {
        return ERRCODE_SPI_SLAVE_SELECT_INVALID;
    }

    hal_spi_v100_ser_set_ser(bus, bit(slave));
...
static errcode_t hal_spi_ctrl_set_dma_cfg(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    hal_spi_v100_dmacr_set_tdmae(bus, (uint32_t)data->is_enable);
    hal_spi_v100_dmacr_set_rdmae(bus, (uint32_t)data->is_enable);
    hal_spi_v100_dmatdlr_set_dmatdl(bus, data->dma_tx_level);
    hal_spi_v100_dmardlr_set_dmardl(bus, data->dma_rx_level);
...
static errcode_t hal_spi_ctrl_get_dma_data_addr(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    *addr = (uint32_t)(uintptr_t)(&(spis_regs(bus)->dr[0]));
...
static errcode_t hal_spi_ctrl_en_rxfi_int(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    hal_spi_v100_imr_set_rxfi(bus, en);
    return ERRCODE_SUCC;
}
...
static errcode_t hal_spi_ctrl_check_rx_fifo_empty(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    if (hal_spi_v100_sr_get_rfne(bus) == 0) {
        *rx_fifo_empty = true;
    } else {
        *rx_fifo_empty = false;
    }
...
static errcode_t hal_spi_ctrl_en_txei_int(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    uint32_t en = (uint32_t)param;
    hal_spi_v100_imr_set_txei(bus, en);
    return ERRCODE_SUCC;
}
...
static errcode_t hal_spi_ctrl_check_fifo_busy(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    while (hal_spi_v100_sr_get_tfe(bus) != 1 || hal_spi_v100_sr_get_busy(bus) != 0) {
        if (hal_spi_check_timeout_by_count(&trans_time, timeout)) {
            return ERRCODE_SPI_TIMEOUT;
        }
    }
...
#if defined(CONFIG_SPI_SUPPORT_INTERRUPT) && (CONFIG_SPI_SUPPORT_INTERRUPT == 1)
void hal_spi_v100_irq_handler(spi_bus_t bus)
{
    if (!g_hal_spi_callback) {
        return;
    }

    if (hal_spi_v100_isr_get_txei(bus) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_TX_EMPTY_ISR, 0);
    }
    if (hal_spi_v100_isr_get_txoi(bus) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_TX_OVERFLOW_ISR, 0);
    }
    if (hal_spi_v100_isr_get_rxfi(bus) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_RX_FULL_ISR, 0);
    }
    if (hal_spi_v100_isr_get_rxui(bus) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_RX_UNDERFLOW_ISR, 0);
    }
    if (hal_spi_v100_isr_get_rxoi(bus) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_RX_OVERFLOW_ISR, 0);
    }
    if (hal_spi_v100_isr_get_msti(bus) == 1) {
        g_hal_spi_callback(bus, SPI_EVT_MULTI_MASTER_ISR, 0);
    }
    hal_spi_v100_icr_set_any(bus);
}
#endif  /* CONFIG_SPI_SUPPORT_SLAVE */

static hal_spi_funcs_t g_hal_spi_v100_funcs = {
    .init = hal_spi_init,
    .deinit = hal_spi_deinit,
    .write = hal_spi_write,
    .read = hal_spi_read,
    .writeread = hal_spi_writeread,
    .ctrl = hal_spi_ctrl
};

hal_spi_funcs_t *hal_spi_v100_funcs_get(void)
{
    return &g_hal_spi_v100_funcs;
}
