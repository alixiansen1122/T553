...
#if defined(CONFIG_UART_SUPPORT_DMA)
#ifdef TEST_SUITE
#define UART_DMA_TRANSFER_TIMEOUT_MS 0xFFFFFFFFULL
#else
#define UART_DMA_TRANSFER_TIMEOUT_MS 1000
#endif
                         ...
                         uart_buffer_config_t *uart_buffer_config)
{
    ...
    if (g_uart_suspend_flag[bus] == false) {
        memcpy_s(&g_uart_pins[bus], sizeof(uart_pin_config_t), pins, sizeof(uart_pin_config_t));
        memcpy_s(&g_uart_attr[bus], sizeof(uart_attr_t), attr, sizeof(uart_attr_t));
        memcpy_s(&g_uart_extra_attr[bus], sizeof(uart_extra_attr_t), extra_attr, sizeof(uart_extra_attr_t));
        memcpy_s(&g_uart_buffer_config[bus], sizeof(uart_buffer_config_t), uart_buffer_config,
    ...
    errcode_t ret = hal_funcs->init(bus, uart_evt_callback, (hal_uart_pin_config_t *)pins, (hal_uart_attr_t *)attr,
                                    (hal_uart_flow_ctrl_t)UART_FLOW_CTRL_SOFT);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
...
int32_t uapi_uart_write_by_dma(uart_bus_t bus, const void *buffer, uint32_t length, uart_write_dma_config_t *dma_cfg)
{
    ...
    uapi_pm_add_sleep_veto(PM_ID_UART_DMA);
    if (uapi_dma_start_transfer(dma_ch) != ERRCODE_SUCC) {
        uapi_pm_remove_sleep_veto(PM_ID_UART_DMA);
        g_dma_trans[bus].channel = 0;
        return UART_DMA_START_TRANSFER_FAIL;
    }

    if (osal_sem_down_timeout(&(g_dma_trans[bus].dma_sem), UART_DMA_TRANSFER_TIMEOUT_MS) != OSAL_SUCCESS) {
