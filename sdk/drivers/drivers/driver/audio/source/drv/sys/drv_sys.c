...
static td_s32 drv_sys_set_dsp_file_path(const td_char *file_path, td_u32 len)
{
    return overlay_set_dsp_file_path(file_path, len);
}
...
td_s32 sap_ioctl_sys_set_dsp_file_path(td_u32 cmd, td_void *arg, td_void *file)
{
    sap_drv_str *drv_str = (sap_drv_str *)arg;
    audio_unused(cmd);
    audio_unused(file);

    return drv_sys_set_dsp_file_path(drv_str->str, drv_str->len);
}
