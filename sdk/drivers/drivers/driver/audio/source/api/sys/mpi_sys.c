...
td_s32 mpi_audio_set_dsp_file_path(const td_char *file_path, td_u32 len)
{
    td_s32 ret;
    sap_drv_str drv_str;
    ...
    drv_str.str = file_path;
    drv_str.len = len;
    ret = audio_ioctl(g_mpi_sys_fd, CMD_IOCTL_SYS_SET_DSP_FILE_PATH, (td_void *)&drv_str);
    if (ret != AUDIO_SUCCESS) {
        mpi_sys_unlock();
        return ret;
    }
