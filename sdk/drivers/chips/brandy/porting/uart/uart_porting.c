...
static uart_config_t g_uart_config[UART_BUS_MAX_NUMBER] = {
...
#if defined(SUPPORT_VOLTE)
    {
        ...
        },
        {
            .tx_pin_mode = (pin_mode_t)HAL_PIO_FUNC_UART1,
            .rx_pin_mode = (pin_mode_t)HAL_PIO_FUNC_UART1,
            .cts_pin_mode = (pin_mode_t)HAL_PIO_FUNC_UART1,
            .rts_pin_mode = (pin_mode_t)HAL_PIO_FUNC_UART1
        },
        UART_0_IRQN,
        (osal_irq_handler)irq_uart0_handler,
        uapi_imsa_pip_receive_data_uart_cb,
        {
            .baud_rate = 2000000,
            .data_bits = UART_DATA_BIT_8,
            .parity = UART_PARITY_NONE,
...
void sw_debug_hsuart_reinit(void)
{
    while (uapi_uart_has_pending_transmissions(UART_BUS_1)) { }
    uapi_uart_deinit(UART_BUS_1);
    uapi_uart_init(UART_BUS_1, &(g_uart_config[UART_BUS_1].pin),
        &(g_uart_config[UART_BUS_1].line_config), NULL,
        &(g_uart_config[UART_BUS_1].buffer_config));
    uapi_uart_register_rx_callback(UART_BUS_1, UART_RX_CONDITION_FULL_OR_SUFFICIENT_DATA_OR_IDLE,
        (uint32_t)(g_uart_config[UART_BUS_1].buffer_config.rx_buffer_size),
        g_uart_config[UART_BUS_1].rx_callback);
}
#endif
