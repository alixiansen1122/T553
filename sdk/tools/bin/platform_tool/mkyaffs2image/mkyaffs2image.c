...
static struct yaffs_summary_tags *sum_tags;
...
static unsigned yaffs_summary_sum(void)
{
    unsigned sum = 0;
    u8 *sum_tags_tmp = (u8 *)sum_tags;
    int length = CHUNKS_PER_SUMMARY * sizeof(struct yaffs_summary_tags);
...
static int yaffs_do_summary(void)
{
    ...
    hdr.block = 0;
    hdr.seq = YAFFS_LOWEST_SEQUENCE_NUMBER;
    hdr.sum = yaffs_summary_sum();

    char data[chunkSize];
    memset(data, 0xff, chunkSize);
    memcpy(data, &hdr, sizeof(hdr));
    memcpy(data + sizeof(hdr), sum_tags, CHUNKS_PER_SUMMARY * sizeof(struct yaffs_summary_tags));
    ...
    t.chunk_id = 1;
    t.serial_number = 1;
    t.n_bytes = CHUNKS_PER_SUMMARY * sizeof(struct yaffs_summary_tags) + sizeof(hdr);
    t.obj_id = YAFFS_OBJECTID_SUMMARY;
    t.seq_number = YAFFS_LOWEST_SEQUENCE_NUMBER;
    t.chunk_used = 1;
    nPages++;

    struct yaffs_packed_tags2 pt;
    memset(&pt, 0, sizeof(struct yaffs_packed_tags2));
    ...
    if (write(outFile, spareData, spareSize) != spareSize)
        fatal("write");
    return 0;
}
...
static int write_chunk(u8 *data, u32 id, u32 chunk_id, u32 n_bytes)
{
    ...
    current_chunk++;
    if ((current_chunk % (CHUNKS_PER_SUMMARY + 1)) == 0) {
        error = yaffs_do_summary();
        if (error < 0) {
            printf("Yaffs2 write summary info failure.\n");
            return error;
        }
        memset(sum_tags, 0x0, CHUNKS_PER_SUMMARY * sizeof(struct yaffs_summary_tags));
        current_chunk = 1;
        summary_index = 0;
    }

    sum_tags[summary_index].chunk_id = chunk_id;
    sum_tags[summary_index].n_bytes = n_bytes;
    sum_tags[summary_index].obj_id = id;
    summary_index++;
...
int main(int argc, char *argv[])
{
    ...
    int sum_tags_bytes = CHUNKS_PER_SUMMARY * sizeof(struct yaffs_summary_tags);
    sum_tags = (struct yaffs_summary_tags *)malloc(sum_tags_bytes);
    if (sum_tags == NULL) {
        printf("Alloc sum_tags failure.\n");
        exit(1);
    }
    memset(sum_tags, 0x0, sum_tags_bytes);
