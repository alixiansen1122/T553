...
static void sample_gap_connect_reject_rpt_callback(const bd_addr_t *bd_addr)
{
    bts_sample_log("conn_request_notify_callback addr: ****%02x%02x%02x%02x\n",
        bd_addr->addr[3], bd_addr->addr[2], /* addr下标 2 3 */
        bd_addr->addr[1], bd_addr->addr[0]); /* addr下标 0 1 */
}
...
void btsrv_at_cmd_gap_register_callbacks(void)
{
    ...
    gap_register_br_connect_rejection_report_callback(sample_gap_connect_reject_rpt_callback);
}
...
static void btsrv_sample_cmd_gap_register_callbacks(const char **params, int32_t params_cnt)
{
    ...
    if ((sample_mode & 0x1) != 0) {
        ...
        gap_register_br_connect_rejection_report_callback(NULL);
