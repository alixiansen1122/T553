...
static void sample_pan_rpt_ethernet(uint8_t *packet, uint16_t packet_length, uint16_t network_protocol)
{
    bts_sample_log("sample pan rpt ethernet network_protocol = 0x%x, length=%lu, first_byte=0x%x\n.",
        network_protocol, packet_length, *packet);
}
...
static void sample_pan_write_data(const char **params, int32_t params_cnt)
{
    ...
    int result = pan_service_write_data(ip_packet, sizeof(ip_packet),
        PAN_NETWORK_PROTOCOL_IPV4, PAN_PACKET_TYPE_DEST_ONLY_UNICAST);
    bts_sample_log("sample pan write ipv4 data result=%d.\n", result);
    result = pan_service_write_data(ip_packet, sizeof(ip_packet),
        PAN_NETWORK_PROTOCOL_ARP, PAN_PACKET_TYPE_DEST_ONLY_BORADCAST);
    bts_sample_log("sample pan write arp data result=%d.\n", result);
    result = pan_service_write_data(ip_packet, sizeof(ip_packet),
        PAN_NETWORK_PROTOCOL_IPV4, PAN_PACKET_TYPE_COMPRESSED);
    bts_sample_log("sample pan write compressed data result=%d.\n", result);
}
