 ...
 * Description: Alipay iot sec port.
 * Author:
 * Create:
 */
...
#define MyPrintf(fmt, arg...) PRINT("[SEC PAY E/] %s: " fmt "\r\n", __FUNCTION__, ##arg)
...
/* ○ description:
    ■ get device id(mac addres format in hex separated ':')，id must ended with ‘\0’, and len_did not include '\0'.All
  letters should be capital，len_did must be 17.For example："AA:BB:CC:00:11:22" ○ param ■ buf_did - buffer to store
  device id ■ len_did - the length of device id ○ return ■ CSI_OK:success，otherwise:fail */
...
/* ○ description:
    ■ get serial number of device(the sn which factory printed on the card)，len of sn <= 32 bytes,sn could only include
  letters or numbers or underline.Only product(type of card) which need to show sn on small routine to realize this
  function，otherwise set buf_sn to "" and len_sn to 0 ○ param ■ buf_sn - buffer to store device serial number ■ len_sn
  - length of device serial number ○ return ■ CSI_OK:success，otherwise:fail */
