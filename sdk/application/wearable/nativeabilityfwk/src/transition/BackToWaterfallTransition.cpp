...
#include "BackToWaterfallTransition.h"
...
namespace OHOS {
REGIST_TRANSITION(TransitionType::TRANSITION_BACK_TO_WATERFALL, BackToWaterfallTransition, 300, true, false);
static const float CURRENT_SCALE_START = 1.0f;
static const float TARGET_SCALE_END = 1.0f;

void BackToWaterfallTransition::OnTransitionStart(UIImageView* current, UIViewGroup* target)
{
    current_ = current;
...
#endif
    uint16_t sliceId = NativeAbility::GetInstance().GetPreSliceId();    UIView* childView = target_->GetChildrenHead();
    while (childView != nullptr) {
        WaterfallItemView* itemView = dynamic_cast<WaterfallItemView*>(childView);
        if (itemView == nullptr) {
    ...
    if (focusView_ == nullptr) {
        WEARABLE_LOGE(WEARABLE_LOG_MODULE_LAUNCHER_FWK, "BackToWaterfallTransition no focus view, use default\n");
    ...
    target_->ScrollBy(0, deltaY);
    Vector2<int16_t> delta2 = target_->GetReboundDistance();
    target_->ScrollBy(delta2.x_, delta2.y_);
    Rect rect = focusView_->GetRect();
    xEnd_ = target_->GetCurrentPosition(focusIndex_).x_;
    yEnd_ = target_->GetCurrentPosition(focusIndex_).y_;
...
void BackToWaterfallTransition::TransitionAlg(uint32_t time)
{
    ...
    current_->Scale(Vector2<float>(curScale, curScale), Vector2<float>(xEnd_, yEnd_));
...
void BackToWaterfallTransition::OnTransitionEnd()
{
