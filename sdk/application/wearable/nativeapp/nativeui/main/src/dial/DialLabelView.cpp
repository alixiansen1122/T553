...
#include "main/dial/DialLabelView.h"
#include "securec.h"

namespace OHOS {
DialLabelView::DialLabelView(void)
{
}
...
bool DialLabelView::SetText(const char* text)
{
    if (text == nullptr) {
        return false;
    }
...
void DialLabelView::SetTextWithWildcardInner(const char* text)
{
    ...
    UILabel::SetText(dstBuf_);
}

bool DialLabelView::SetTextWithWildcard(const char* text)
{
    if (text == nullptr) {
        return false;
    }
...
void DialLabelView::HandleTextData(const std::string* data, uint16_t num)
{
...
void DialLabelView::HandleFloatData(const float* data, uint16_t num)
{
    char tempBuf[MAX_BUF_SIZE] = {0};
    if (num == 1) {
...
void DialLabelView::HandleFloatData(float data)
{
    char tempBuf[MAX_BUF_SIZE] = {0};
...
void DialLabelView::SetLang(uint8_t lang)
{
    language_ = lang;
    OnLangChange(FontGlobalManager::GetInstance()->GetCurrentLangId());
}

void DialLabelView::OnLangChange(uint8_t langId)
{
