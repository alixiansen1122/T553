...
static const char *ANILABEL_TEXT_FIRST = (char *)"正在获取";
static const int16 ANILABEL_FONT_FIRST = 38;

const char *transSearchImagePath[ALIPAY_TRANS_SEARCH_IMAGE_NUM] = {
...
static const int16 ALIPAY_TRANS_IMAGE_X = 162;
static const int16 ALIPAY_TRANS_IMAGE_Y = 112;
static const int16 ALIPAY_TRANS_IMAGE_WIDTH = 128;
static const int16 ALIPAY_TRANS_IMAGE_HEIGHT = 128;

static const int16 ALIPAY_TRANS_IMAGE_DELAY = 200;
...
AlipayTransAni::AlipayTransAni(AlipayPresenter *presenter)
{
    WEARABLE_LOGD(WEARABLE_LOG_MODULE_APP, "AlipayTransAni::AlipayTransAni");
    presenter_ = presenter;
}

AlipayTransAni::~AlipayTransAni()
{
    WEARABLE_LOGD(WEARABLE_LOG_MODULE_APP, "AlipayTransAni::~AlipayTransAni");
    RemoveAll();
...
bool AlipayTransAni::AlipayInitLabel(void)
{
    alipayLabel = new UILabel();
    if (alipayLabel == nullptr) {
        WEARABLE_LOGE(WEARABLE_LOG_MODULE_APP, "AlipayTransAni::InitExit new alipayLabel fail");
        return false;
    }
...
bool AlipayTransAni::AlipayInitImageAni(void)
{
    alipayImageAni = new UIImageAnimatorView();
    if (alipayImageAni == nullptr) {
        WEARABLE_LOGE(WEARABLE_LOG_MODULE_APP, "AlipayTransAni::AlipayInitImageAni new alipayImageAni fail");
        return false;
    }
    for (uint8_t i = 0; i < ALIPAY_TRANS_SEARCH_IMAGE_NUM; i++) {
        searchImageInfo[i].imageInfo = ImageCacheManager::GetInstance().LoadSingleRes(transSearchImagePath[i]);
        searchImageInfo[i].imageType = IMG_SRC_IMAGE_INFO;
        searchImageInfo[i].pos.x = ALIPAY_TRANS_IMAGE_X;
        searchImageInfo[i].pos.y = ALIPAY_TRANS_IMAGE_Y;
        searchImageInfo[i].width = ALIPAY_TRANS_IMAGE_WIDTH;
        searchImageInfo[i].height = ALIPAY_TRANS_IMAGE_HEIGHT;
    }

    alipayImageAni->SetPosition(RES_IMAGE_X, RES_IMAGE_Y, RES_IMAGE_WIDTH, RES_IMAGE_HEIGHT);
    alipayImageAni->SetImageAnimatorSrc(searchImageInfo, ALIPAY_TRANS_SEARCH_IMAGE_NUM, ALIPAY_TRANS_IMAGE_DELAY);
...
bool AlipayTransAni::InitView()
{
    WEARABLE_LOGD(WEARABLE_LOG_MODULE_APP, "AlipayTransAni::Init");
    SetDraggable(true);
    SetTouchable(true);
    ...
    if (!AlipayInitLabel()) {
        WEARABLE_LOGE(WEARABLE_LOG_MODULE_APP, "AlipayTransAni::InitView AlipayInitLabel() false");
        return false;
    }
    if (!AlipayInitImageAni()) {
        WEARABLE_LOGE(WEARABLE_LOG_MODULE_APP, "AlipayTransAni::InitView AlipayInitImageAni() false");
        return false;
    }
