...
#include "ui_test_message/ui_test_message.h"
#include "ui_test_nested/ui_test_nested.h"
...
#include "ui_test_slipflow/ui_test_slipflow.h"
#include "ui_test_foldable/ui_test_foldable.h"
#include "ui_test_sphere_view/ui_test_sphere_view.h"
...
#include "ui_test_transition/ui_test_transition_view.h"
...
namespace OHOS {
UITestGroup::~UITestGroup()
{
    TearDownTestCase();
    if (testCaseList_ != nullptr) {
        delete testCaseList_;
        testCaseList_ = nullptr;
    }
}
...
void UITestGroup::SetUpTestCase()
{
    ...
    TearDownTestCase();
    testCaseList_->PushBack(TestCaseInfo{"自研用例", nullptr});
    testCaseList_->PushBack(TestCaseInfo{"开源用例", nullptr});
}

void UITestGroup::SetUpHSTestCase()
{
    if (testCaseList_ == nullptr) {
        testCaseList_ = new List<TestCaseInfo>();
    }
    ...
    testCaseList_->PushBack(TestCaseInfo{"滑动列表", new UITestListFPS()});
    ...
    testCaseList_->PushBack(TestCaseInfo{"折叠控件", new UITestFoldable()});
    testCaseList_->PushBack(TestCaseInfo{"覆盖流", new UITestCoverflow()});
    testCaseList_->PushBack(TestCaseInfo{"滑动流", new UITestSlipflow()});
    ...
    testCaseList_->PushBack(TestCaseInfo{"球型菜单", new UITestSphereView()});
    ...
    testCaseList_->PushBack(TestCaseInfo{"滑动冲突", new UITestNested()});
    ...
    testCaseList_->PushBack(TestCaseInfo{"页面动效", new UITestTransitionView()});
...
void UITestGroup::SetUpOHTestCase()
{
    if (testCaseList_ == nullptr) {
        testCaseList_ = new List<TestCaseInfo>();
    }
...
void UITestGroup::TearDownTestCase()
{
    ...
    ListNode<TestCaseInfo>* node = testCaseList_->Begin();
    while (node != testCaseList_->End()) {
        if (node->data_.testObj != nullptr) {
            delete node->data_.testObj;
            node->data_.testObj = nullptr;
        }
        node = testCaseList_->Remove(node);
    }
    testCaseList_->Clear();
}
} // namespace OHOS
