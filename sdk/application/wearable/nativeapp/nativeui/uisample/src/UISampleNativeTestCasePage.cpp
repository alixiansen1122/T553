 ...
 * Created: 2025-7
 */
#include "uisample/UISampleNativeTestCasePage.h"
...
REGIST_SLICE_PAGE(VIEW_UISAMPLE, SAMPLE_PAGES::SAMPLE_NATIVE_TESTCASE_PAGE, UISampleNativeTestCasePage, false);
static UISampleNativeTestCasePage* g_instance = nullptr;
static RootView::OnKeyActListener *g_keyListener = nullptr;

UISampleNativeTestCasePage::UISampleNativeTestCasePage()
{
    g_instance = this;
}

UISampleNativeTestCasePage* UISampleNativeTestCasePage::GetInstance()
{
    return g_instance;
}
...
static UIView* CreateViewCb(uint8_t type)
{
    ...
    item->SetOnClickListener(UISampleNativeTestCasePage::GetInstance());
    return item;
}
...
bool UISampleNativeTestCasePage::BackMenuPage(UIView& view)
{
    if (container_ != nullptr) {
        container_->RemoveAll();
...
void UISampleNativeTestCasePage::OnStart(void *data)
{
    if (container_ == nullptr) {
        container_ = new UIScrollViewNested();
        if (container_ == nullptr) {
            WEARABLE_LOGD(WEARABLE_LOG_MODULE_APP, " UISampleNativeTestCasePage::OnStart::container_ is nullptr");
            return;
        }
    }
...
void UISampleNativeTestCasePage::CreateTitle()
{
    if (title_ == nullptr) {
        title_ = new UILabel();
        if (title_ == nullptr) {
            WEARABLE_LOGD(WEARABLE_LOG_MODULE_APP, " UISampleNativeTestCasePage::OnStart::title_ is nullptr");
            return;
        }
    }
...
void UISampleNativeTestCasePage::CreateMainList()
{
    if (mainList_ == nullptr) {
        mainList_ = new UISimpleList();
    }
    if (mainList_ == nullptr) {
        WEARABLE_LOGD(WEARABLE_LOG_MODULE_APP, " UISampleNativeTestCasePage::OnStart::mainList_ is nullptr");
        return;
    }
    ...
    UITestGroup::GetInstance()->SetUpOHTestCase();
    mainList_->ClearAll();
...
void UISampleNativeTestCasePage::CreateBackButton()
{
    if (backBtn_ == nullptr) {
...
bool UISampleNativeTestCasePage::OnClick(UIView& view, const ClickEvent& event)
{
    if (isInTestCase_) {
...
bool UISampleNativeTestCasePage::OnDrag(UIView& view, const DragEvent& event)
{
    UNUSED(view);
