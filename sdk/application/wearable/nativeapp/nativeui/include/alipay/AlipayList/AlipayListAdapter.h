...
#ifndef ALIPAY_LIST_ADAPTER_H
#define ALIPAY_LIST_ADAPTER_H
...
namespace OHOS {
class AlipayListAdapter : public AbstractAdapter {
public:
    explicit AlipayListAdapter();
    ~AlipayListAdapter() override;

    /* *
     * @brief Sets the UIView adapter data, which is a AlipayListItem linked list.
     *
     * @param data Indicates the AlipayListItem linked list data.
     * @since 1.0
     * @version 1.0
     */
    void SetData(std::list<AlipayListItem> &data);
    ...
    void AddListItem(const AlipayListItem &item);
...
private:
    std::list<AlipayListItem> listData;
