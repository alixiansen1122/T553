...
#include "upg.h"
#endif
#ifdef SUPPORT_AT_GNSS
#include "gnss_at_process.h"
#endif
#ifdef SUPPORT_AT_VOLTE
...
#endif
#ifdef SUPPORT_AT_GNSS
    uint32_t gnss_cmd_count;
    at_cmd_table_t *gnss_table = get_gnss_cmd_table(&gnss_cmd_count);
    at_cmd_add_moudle(gnss_table, gnss_cmd_count);
#endif
#ifdef SUPPORT_AT_VOLTE
#ifdef SUPPORT_VOLTE_AUTOTEST
    ims_autotest_init();
#endif
    uint32_t volte_cmd_count;
    at_cmd_table_t *volte_table = get_volte_cmd_table(&volte_cmd_count);
    at_cmd_add_moudle(volte_table, volte_cmd_count);
#endif
}
