...
static void sdio_init(void)
{
    ...
    /* sdio probe失败, NANDFLASH访问异常bugfix */
#ifdef CFG_DRIVERS_NANDFLASH
    clocks_clk_src_t clk_src = (clocks_clk_src_t)hal_clocks_ccrg_module_get_src(HAL_CLOCKS_MODULE_SDIOM);
    if (is_sdio_working() == false) {
        dprintf("sdio probe fail, switch sdiom clk to pll\r\n");
        clocks_set_sdio_freq(CLOCKS_SDIO_FREQ_H);
    } else {
        dprintf("sdio probe succ, sdiom clk_src = %d\r\n", clk_src);
    }
#endif
}
#endif
...
#if DSP_EXIST == YES
#ifdef CONFIG_LOAD_FROM_FS
#define DSP_FILE_PATH "/user/dsp/"
#endif
static void audio_init()
{
    ...
    ret = uapi_audio_init();
    if (ret != 0) {
        PRINT("audio init error:[%x]\r\n", ret);
    } else {
        PRINT("audio init OK\r\n");
    }

#ifdef CONFIG_LOAD_FROM_FS
    ret = uapi_audio_set_dsp_file_path(DSP_FILE_PATH, sizeof(DSP_FILE_PATH));
    if (ret != 0) {
        PRINT("audio set dsp file path error:[%x]\r\n", ret);
    }
#endif
}
#endif
...
static void thread_main(const void *unused)
{
    ...
    if ((cause != REBOOT_CAUSE_APPLICATION_STD_CHIP_WDT_FRST) &&
        ...
        // write_fileinfo("/user/exc/hard_watchdog_error.bin",
        //                0, (const uint8_t *)MCPU_TRACE_MEM_REGION_START, CPU_TRACE_MEM_REGION_LENGTH);
        cpu_trace_enable_mcpu_repeat();
    }
