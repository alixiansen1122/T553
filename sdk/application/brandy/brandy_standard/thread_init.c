...
#ifdef SUPPORT_UART_AGENT_DEMO
#include "app_meidi_demo.h"
#endif
...
#define TASK_COMMON_APP_DELAY_MS    600000
#else
...
#ifdef SUPPORT_VOLTE
#define TASK_PRIORITY_IMSA          (osPriority_t)(19)
#define TASK_PRIORITY_IMSA_PIP_UART (osPriority_t)(19)
#define TASK_PRIORITY_IMSA_PIP_SPI  (osPriority_t)(20)
#endif

#define BTH_SDK_STACK_SIZE          0xC00
#define AT_GRAPHIC_STACK_SIZE       0xC00
#ifdef SUPPORT_ESIM
#define AT_APP_STACK_SIZE           0x6000
#else
...
#define APP_MAIN_STACK_SIZE         0xE00
#define SAVE_FILE_STACK_SIZE        0xC00
#define AUTO_OTA_STACK_SIZE         0xC00
#define LWIP_MAIN_STACK_SIZE        0xC00
#define SIKEY_MAIN_STACK_SIZE       0xF00

#ifdef SUPPORT_VOLTE
#define IMSA_PROCESS_STACK_SIZE         0x2000
#define IMSA_PROCESS_IMS_PIP_UART_SIZE  0x1000
#define IMSA_PROCESS_IMS_PIP_SPI_SIZE   0x1000
#endif
#define TIME_MAX                64
...
static uint32_t g_app_delay_ms = TASK_COMMON_APP_DELAY_MS;
app_task_definition_t g_app_tasks[] = {
...
#if defined(SUPPORT_LWIP) && !defined(CONFIG_FT_DITING)
    ...
    { { "at_cmd", 0, NULL, 0, NULL, 0xC00, TASK_PRIORITY_APP, 0, 0 },
      ...
      (CreateQueueFunction_t)at_cmd_task_queue_init, USE_EXTEND_MEM },
#endif
    ...
    { { "at_audio", 0, NULL, 0, NULL, AT_AUDIO_STACK_SIZE, TASK_PRIORITY_APP, 0, 0 },
      ...
      (CreateQueueFunction_t)audio_at_task_queue_init, USE_EXTEND_MEM },
#endif
#ifdef SUPPORT_AT_BT_MANAGER
    { { "at_bt_manager", 0, NULL, 0, NULL, AT_BT_MANAGER_STACK_SIZE, TASK_PRIORITY_APP, 0, 0 },
      ...
      (CreateQueueFunction_t)bt_manager_at_task_queue_init, USE_EXTEND_MEM },
#endif
#ifdef SUPPORT_AT_MEDIA
    { { "at_media", 0, NULL, 0, NULL, AT_MEDIA_STACK_SIZE, TASK_PRIORITY_APP, 0, 0 },
      (osThreadFunc_t)media_at_task_process_handler, NULL,
      (CreateQueueFunction_t)media_at_task_queue_init, USE_EXTEND_MEM },
#endif
#ifdef HSO_SUPPORT
...
#endif
#ifdef SUPPORT_VOLTE
#ifndef CONFIG_FT_DITING
    /* imsa处理线程 SUPPORT_VOLTE     */
    { { "imsa", 0, NULL, 0, NULL, IMSA_PROCESS_STACK_SIZE, TASK_PRIORITY_IMSA, 0, 0 },
      (osThreadFunc_t)imsa_process_thread, NULL,
      (CreateQueueFunction_t)imsa_process_task_init, USE_EXTEND_MEM },
#endif
    /* imsa ims uart通道处理线程 */
    { { "imsa_pip_task", 0, NULL, 0, NULL, IMSA_PROCESS_IMS_PIP_UART_SIZE, TASK_PRIORITY_IMSA_PIP_UART, 0, 0 },
      (osThreadFunc_t)imsa_pip_task_message_proc, NULL,
      (CreateQueueFunction_t)imsa_pip_task_init, USE_EXTEND_MEM },

    /* imsa ims spi通道处理线程 */
    { { "imsa_pip_spi", 0, NULL, 0, NULL, IMSA_PROCESS_IMS_PIP_SPI_SIZE, TASK_PRIORITY_IMSA_PIP_SPI, 0, 0 },
      (osThreadFunc_t)imsa_pip_spi_message_proc, NULL,
      (CreateQueueFunction_t)imsa_pip_spi_init, USE_EXTEND_MEM },
#endif
#ifdef SUPPORT_UART_AGENT_DEMO
    { { "meidi_app", 0, NULL, 0, NULL, MEIDI_APP_STACK_SIZE, TASK_PRIORITY_TRANSMIT, 0, 0 },
        (osThreadFunc_t)app_meidi_task_thread, NULL, NULL, USE_DTCM_MEM },
#endif
    { { "sikey_main", 0, NULL, 0, NULL, SIKEY_MAIN_STACK_SIZE, TASK_PRIORITY_TRANSMIT, 0, 0 },
        (osThreadFunc_t)sikey_main, NULL, NULL, USE_EXTEND_MEM },
};
...
static void print_system_sleep_info(void)
{
    ...
    PRINT("M core run time %d, deepsleep time:%d, lightsleep time:%d, m sleep flag:%d"NEWLINE,
    ...
    PRINT("M core last veto id:%d, last veto count:%d, total_counts=%d"NEWLINE, sleep_info->sleep_veto.last_veto_id,
        sleep_info->sleep_veto.last_veto_count, pm_veto_ptr->veto_counts.total_counts);
...
static void sikey_main(void)
{
    PRINT("sikey_main run start."NEWLINE);
    http_task_init();
    ws_task_init();
    gnss_task_init();
