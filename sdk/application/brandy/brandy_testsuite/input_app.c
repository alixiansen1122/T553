...
static void button_report_event_to_gui(uint16_t state)
{
    ...
    if (g_button_event_cb != NULL) {
        ...
        data.state = state; // 1:press 0:release
        g_button_event_cb(&data);
    }
}

static void button_report_event(event_button_t event)
{
    input_dev_data_t data;
    ...
    switch (event) {
        case BUTTON_PWR_PRESS:
            ...
            button_report_event_to_pms();
            #endif
            button_report_event_to_gui(1);
            break;
        case BUTTON_PWR_CLICKUP:
            button_report_event_to_gui(0);
            break;
        default:
            break;
    }
...
static void button_report_click_state(button_id_t id)
{
    if (g_curr_button_state == BUTTON_RELEASE_STATUS && button_get_state(id) == BUTTON_RELEASE_STATUS) {
        ...
        } else if (g_button[id].is_clicked == true) {
            /* continue click */
            g_button[id].click_cnt++;
            g_button[id].click_start_time = g_button[id].end_time;
        }
        /* click invilid */
        if (g_button[id].click_cnt + 1 > BUTTON_INVALID_CLICK_TIMES) {
