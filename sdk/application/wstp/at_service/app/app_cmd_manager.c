...
static uint32_t app_test_show_task_heap(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(SHOWTSKHEAP, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_test_show_all_task_heap(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(SHOWALLTSKHEAP, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_test_show_mem_free(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(SHOWMEMFREE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_test_show_heap_statistic(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(SHOWHEAPST, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_test_show_mem_info(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(SHOWALLMEMINFO, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_test_show_task_info(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(SHOWTSKINFO, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_test_show_task_waterline(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(SHOWTSKWTL, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
#endif

#ifdef SUPPORT_FACTORY_TEST
static uint32_t app_at_bsn_read_write(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACBSN, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_at_scrambing_code_read_write(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(FACSCRAMBLING, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_at_psn_read_write(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACPSN, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_imei_read_write(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACIMEI, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_hardcode_read_write(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACHARDCODE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_aeskey_read_write(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACAESKEY, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_ticket_read_write(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACTICKET, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_get_devicecheck(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACDEVICECHECK, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_get_chg_current(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACCHGCURRENT, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_get_battery_level(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACBATTERYLEVEL, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
#endif

#ifdef CFG_DRIVERS_SDIO
static uint32_t app_sdio_test(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(SDIOTEST, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
#endif

static uint32_t app_hsuart_reinit(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(HSUARTREINIT, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_set_gpio_dir_fac(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(SETGPIODIRFAC, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_set_gpio_level_fac(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(SETGPIOLEVELFAC, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_get_gpio_level_fac(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GETGPIOLEVELFAC, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_set_gpio_dir(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(SETGPIODIR, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_set_gpio_level(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(SETGPIOLEVEL, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_get_gpio_level(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GETGPIOLEVEL, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

#ifdef SUPPORT_NFC
static uint32_t app_nfc_reboot(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(NFCREBOOT, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_nfc_reset(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(NFCRESET, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_nfc_business_start(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(NFCBUSINESSSTART, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_nfc_business_stop(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(NFCBUSINESSSTOP, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_nfc_mxxce_switch_card(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(NFCMXXCESWITCHCARD, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_nfc_open_log(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(NFCOPENLOG, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_nfc_close_log(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(NFCCLOSELOG, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_nfc_send_nci_cmd(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(NFCSENDNCICMD, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
#endif

static uint32_t app_gsensor_data_get(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GSENSORDATAGET, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
#ifdef SUPPORT_ESIM
static uint32_t app_esim_get_info(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMGETINFO, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_init(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMINIT, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_get_profile_class_list(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMGETLIST, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_get_profile(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMGETPROFILE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_edit_profile(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMEDITPROFILE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_delete_profile(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMDELETEPROFILE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_enable_profile(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMENABLE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_disable_profile(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMDISABLE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_notification(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMNOTIFICATION, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_download(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMDOWNLOAD, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_reset(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMRESET, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_set_iccid(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMSETICCID, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_set_ac_code(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMSETACCODE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_status(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMBINDSTATUS, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_write_ppk(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMWRITEPPK, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_bind(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMBIND, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_verify(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMVERIFY, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_bind_main(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMBINDMAIN, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_esim_unlock(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    }
    app_pkg_msg_send(ESIMUNLOCK, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
#endif

static uint32_t app_web_test(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(WEBTEST, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_get_gsensor_data(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GET_GSENSOR_DATA, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_get_barometer_data(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GET_BAROMETER_DATA, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_sk_audio_test(uint8_t *para, uint32_t para_len, char *str_cmd)

{
    ...
    app_pkg_msg_send(SK_AUDIO_TEST, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_modem_update(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(MODEM_FIRMWARE_UPDATE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_facflag_handle(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(FACFLAG, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_shut_down_device(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(SHUT_DOWN_DEVICE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_get_hw_version(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GET_HW_VERSION, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}


static uint32_t app_get_sw_version(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GET_SW_VERSION, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_get_iccid(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GET_ICCID, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_set_autostart(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(SET_AUTO_CONFIG, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_get_autostart(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(GET_AUTO_CONFIG, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_set_cfun0(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(SET_MT_CFUN0, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_cali_sensor(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(CALI_SENSOR_DATA, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_rgb_led_test(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL) {
        return ERRCODE_FAIL;
    ...
    app_pkg_msg_send(RGB_LED_TEST, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
static uint32_t app_firmware_update(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    app_pkg_msg_send(MCU_FIRMWARE_UPDATE, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}

static uint32_t app_at_gnss_test(uint8_t *para, uint32_t para_len, char *str_cmd)
{
    unused(str_cmd);
    if (para == NULL)
    {
    ...
    app_pkg_msg_send(GNSS_TEST, DIAG_CONTROL_FALSE, para, para_len);
    return ERRCODE_SUCC;
}
...
#endif
#ifdef LOSCFG_MEM_DFX_SHOW_CALLER_RA
    {"AT^SHOWTSKHEAP", app_test_show_task_heap, "" },
    {"AT^SHOWALLTSKHEAP", app_test_show_all_task_heap, "" },
    {"AT^SHOWMEMFREE", app_test_show_mem_free, "" },
    {"AT^SHOWHEAPST", app_test_show_heap_statistic, "" },
    {"AT^SHOWMEMINFO", app_test_show_mem_info, "" },
    {"AT^SHOWTSKINFO", app_test_show_task_info, "" },
    {"AT^SHOWTSKWTL", app_test_show_task_waterline, "" },
#endif
#ifdef CFG_DRIVERS_SDIO
    { "AT^SDIOTEST",  app_sdio_test,   "" },
#endif
    { "AT^HSUARTREINIT",     app_hsuart_reinit,   "" },
    { "AT^SETIODIRFAC",      app_set_gpio_dir_fac,     "" },
    { "AT^SETIOLEVELFAC",    app_set_gpio_level_fac,    "" },
    { "AT^GETIOLEVELFAC",    app_get_gpio_level_fac,   "" },
    { "AT^SETIODIR",         app_set_gpio_dir,     "" },
    { "AT^SETIOLEVEL",       app_set_gpio_level,    "" },
    { "AT^GETIOLEVEL",       app_get_gpio_level,   "" },
#ifdef SUPPORT_NFC
    { "AT^NFCREBOOT",          app_nfc_reboot, "" },
    { "AT^NFCRESET",           app_nfc_reset, "" },
    { "AT^NFCBUSINESSSTART",   app_nfc_business_start, "" },
    { "AT^NFCBUSINESSSTOP",    app_nfc_business_stop, "" },
    { "AT^NFCMXXCESWITCHCARD", app_nfc_mxxce_switch_card, "" },
    { "AT^NFCOPENLOG",         app_nfc_open_log, "" },
    { "AT^NFCCLOSELOG",        app_nfc_close_log, "" },
    { "AT^NFCSENDNCICMD",      app_nfc_send_nci_cmd, "" },
#endif
    { "AT^GSENSORDATAGET",   app_gsensor_data_get, "" },
#ifdef SUPPORT_ESIM
    { "AT^ESIMGETINFO",        app_esim_get_info, "" },
    { "AT^ESIMINIT",           app_esim_init, "" },
    { "AT^ESIMGETLIST",        app_esim_get_profile_class_list, "" },
    { "AT^ESIMGETPROFILE",     app_esim_get_profile, "" },
    { "AT^ESIMEDITPROFILE",    app_esim_edit_profile, "" },
    { "AT^ESIMDELETEPROFILE",  app_esim_delete_profile, "" },
    { "AT^ESIMENABLE",         app_esim_enable_profile, "" },
    { "AT^ESIMDISABLE",        app_esim_disable_profile, "" },
    { "AT^ESIMNOTIFICATION",   app_esim_notification, "" },
    { "AT^ESIMDOWNLOAD",       app_esim_download, "" },
    { "AT^ESIMRESET",          app_esim_reset, "" },
    { "AT^ESIMBINDSTATUS",     app_esim_status, "" },
    { "AT^ESIMWRITEPPK",       app_esim_write_ppk, "" },
    { "AT^ESIMBIND",           app_esim_bind, "" },
    { "AT^ESIMVERIFY",         app_esim_verify, "" },
    { "AT^ESIMUNLOCK",         app_esim_unlock, "" },
    { "AT^ESIMSETICCID",       app_esim_set_iccid, "" },
    { "AT^ESIMSETACCODE",      app_esim_set_ac_code, "" },
    { "AT^ESIMBINDMAIN",       app_esim_bind_main, "" },
#endif

#ifdef SUPPORT_FACTORY_TEST
    { "AT^BSN", app_at_bsn_read_write,   "" },
    { "AT^PSN", app_at_psn_read_write,   "" },
    { "AT^IMEI", app_at_imei_read_write,   "" },
    { "AT^BIDINGCODE", app_at_hardcode_read_write,   "" },
    { "AT^AESKEY", app_at_aeskey_read_write,   "" },
    { "AT^DEVICECHECK", app_at_get_devicecheck,   "" },
    { "AT^GETCHARGERCURRENT", app_at_get_chg_current,   "" },
    { "AT^GETBATTERYLEVEL", app_at_get_battery_level,   "" },
    { "AT^FACFLAG",  app_facflag_handle, "" },
#endif
    { "AT^WEBTEST",   app_web_test, "" },
    { "AT^GET_GSENSOR_DATA", app_get_gsensor_data, "" },
    { "AT^GET_BAROMETER_DATA", app_get_barometer_data, "" },
    { "AT^SKAUDIO_TEST", app_sk_audio_test, "" },
    { "AT^UPDATEMODEM", app_modem_update, "" },
    { "AT^POWEROFF", app_shut_down_device, "" },
    { "AT^GET_HW_VER", app_get_hw_version, "" },
    { "AT^GETWATCHVERSION", app_get_sw_version, "" },
    { "AT^GETICCID", app_get_iccid, "" },
    { "AT^SETAUTOSTART", app_set_autostart, "" },
    { "AT^SET_CFUN0", app_set_cfun0, "" },
    { "AT^CALI_SENSOR", app_cali_sensor, "" },
    { "AT^RGB_LED_TEST", app_at_rgb_led_test, "" },
    { "AT^UPDATEMCU",       app_firmware_update,               "" },
    { "AT^GPS",          app_at_gnss_test, "" },
    { "AT^GETAUTOSTART", app_get_autostart, "" },
};
