...
int aw883xx_pid_2066_dev_init(void *aw883xx_val)
{
	int ret = -1;
	struct aw_device *aw_pa = NULL;
	struct aw883xx *aw883xx = (struct aw883xx *)aw883xx_val;

	aw_pr_dbg("enter");

	if (aw883xx == NULL) {
		aw_pr_err("aw883xx is null");
		return -EINVAL;
	}

	if (aw883xx->chip_id != PID_2066_ID) {
		aw_dev_dbg(aw883xx->dev, "The id:0x%x does not match:0x%x",
					aw883xx->chip_id, PID_2066_ID);
		return -EINVAL;
	}

	aw_pa = calloc(1, sizeof(struct aw_device));
	if (aw_pa == NULL) {
		aw_dev_err(aw883xx->dev, "calloc failed");
		return -ENOMEM;
	}
	...
	aw_pa->txen_desc.reg = AW_PID_2066_I2SCTRL3_REG;
	aw_pa->txen_desc.mask = AW_PID_2066_I2STXEN_MASK;
	aw_pa->txen_desc.enable = AW_PID_2066_I2STXEN_ENABLE_VALUE;
	aw_pa->txen_desc.disable = AW_PID_2066_I2STXEN_DISABLE_VALUE;
	...
	aw883xx->aw_pa = aw_pa;
	ret = aw883xx_device_probe(aw_pa, aw883xx->init_info);
