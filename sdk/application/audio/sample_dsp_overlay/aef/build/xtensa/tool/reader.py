...
def find_key(d, val):
    # find a dict key corresponding to the requested value (if there are more keys with the same value,
    # the first matching one is returned)
    for k, v in d.items():
        if v == val:
            return k
    return None
...
class p3elfError(Exception):
    # all exception classes defined by p3elf inherit this exception class to make it easy for
    # library consumers to catch all exceptions raised by p3elf
    pass
...
class ELFBase:
    """ Base class representing a single ELF file. Initialzing an instance of it sets up the path,
    opens the file for reading, extracts basic byteclass-independant information (endianness, byteclass).
    """
    ...
    def __init__(self, file_path):
        ...
        # 3. check if the file is an actual ELF file (via the magic number in the header), and if not,
        # raise an exception and close the _reader
...
class ELFReader(ELFBase):
    # header fields that are returned as tuples because
    # they also have a string representation (i.e. EI_MACHINE, where 0x3e corresponds to 'amd64')
    header_tuple_fields = ['EI_OSABI', 'EI_MACHINE', 'EI_OBJTYPE']
    ...
    def get_header_field(self, field):
        ...
        self._reader.seek(
            consts.HEADER_FIELDS_DESC[field][0][0] if self.byteclass == 32 else consts.HEADER_FIELDS_DESC[field][0][1])
        num_val = int.from_bytes(self._reader.read(
            consts.HEADER_FIELDS_DESC[field][1][0] if self.byteclass == 32 else consts.HEADER_FIELDS_DESC[field][1][1]),
            self.endianness)

        # modules are not subscriptable so I cannot automatically query
        # the appropriate dictionary from "consts" (i.e. consts['EI_OSABI']),
        # unless I import them into this namespace, which I don't want to do
    ...
    def get_header(self):
        """ Parse the entirety of the ELF header and return a dictionary with all the appropriate fields.
            The byteclass and the endianness are saved as attributes of the class instance object
            once it is initialized, but are also provided in the dict returned by this method
        """
        return {f: self.get_header_field(f) for f in consts.HEADER_FIELDS_DESC}

    def get_progheader_field(self, field, index=0):
        # fetch a specific field from the program header - if no program headers exist
        # at the specified index, raise an appropriate exception
        ...
        self._reader.seek(
            self.get_header_field('EI_PHOFF') + (index * phsize))  # seek reader to start of program header
        self._reader.seek(consts.PROGHEADER_FIELDS_DESC[field][0][0] if self.byteclass == 32 else
                          consts.PROGHEADER_FIELDS_DESC[field][0][1], 1)  # seek to offset of requested field
        num_val = int.from_bytes(self._reader.read(
            consts.PROGHEADER_FIELDS_DESC[field][1][0] if self.byteclass == 32 else
            consts.PROGHEADER_FIELDS_DESC[field][1][1]), self.endianness)
    ...
    def get_sectionheader_field(self, field, index=0):
        ...
        self._reader.seek(consts.SECTHEADER_FIELDS_DESC[field][0][0] if self.byteclass == 32 else
                          consts.SECTHEADER_FIELDS_DESC[field][0][1], 1)  # seek to offset of requested field
        num_val = int.from_bytes(self._reader.read(
            consts.SECTHEADER_FIELDS_DESC[field][1][0] if self.byteclass == 32 else
            consts.SECTHEADER_FIELDS_DESC[field][1][1]), self.endianness)
        ...
        if field in ELFReader.sectheader_tuple_fields:
            ...
            # SH_NAME is actually a 4byte offset to a string in the .shstrtab section
            # that represents the name of the current section
            self._reader.seek(self.get_sectionheader_field('SH_OFFSET', self.get_header_field('EI_SHSTRNDX')) + num_val,
                              0)
            name = bytes()
            t = self._reader.read(1)
            # the strings aren't of a fixed size, rather they're null terminated
            # (indicated by the fact that the SH_ENTSIZE field of the SHT_STRTAB section header is 0x0)
            while int.from_bytes(t, self.endianness) != 0x0:
                name += t
                t = self._reader.read(1)
            ret_val = num_val, name.decode('utf8')
        elif field == "SH_FLAGS":
            ret_val = num_val, [flag_name for flag_name, flag_bits in consts.SH_FLAGS.items() if num_val & flag_bits]
        else:
            ret_val = num_val
    ...
    def get_raw_segment(self, progheader_index):
        """ Return a bytes object containing the entire segment associated
            with the program header at the specified index
        """
        _tell = self._reader.tell()
    ...
    def get_section(self, name):
        """ Return a bytes object representing the content of a section
        """
        _tell = self._reader.tell()
