...
static inline uint32_t saturate_sub(uint32_t x, uint32_t y, uint32_t saturation)
{
    return (x < y) ? (saturation + x - y) : (x - y);
}
...
/* 不要在外部使用该接口!!! */
static inline uint32_t circ_buf_rollback_write_pos(tiot_circ_buf *cb, uint32_t len)
{
    *cb->write = saturate_sub(*cb->write, len, cb->size);
    return len;
}
