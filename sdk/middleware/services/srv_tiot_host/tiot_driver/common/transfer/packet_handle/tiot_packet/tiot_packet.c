...
static inline void free_packet(tiot_packet_context *ctx, tiot_packet *packet)
{
#ifdef CONFIG_BOARD_DYNAMIC_ALLOC
    tiot_unused(ctx);
    tiot_packet_free(packet);
#else
    circ_buf_update_read_pos(&(ctx->rx_buff.buf), tiot_packet_len(packet));
#endif
}

static inline void discard_packet(tiot_packet_context *ctx, tiot_packet *packet)
{
#ifdef CONFIG_BOARD_DYNAMIC_ALLOC
    tiot_unused(ctx);
    tiot_packet_free(packet);
#else
    circ_buf_rollback_write_pos(&(ctx->rx_buff.buf), tiot_packet_len(packet));
#endif
}

static inline void copy_from_packet(tiot_packet_context *ctx, tiot_packet *packet, uint8_t *buff)
{
#ifdef CONFIG_BOARD_DYNAMIC_ALLOC
    tiot_unused(ctx);
    tiot_packet_copy_to(packet, buff);
    tiot_packet_free(packet);
#else
    circ_buf_read(&(ctx->rx_buff.buf), buff, tiot_packet_len(packet));
#endif
}

static inline int32_t expand_cur_packet(tiot_packet_context *ctx, uint16_t len)
{
    if (ctx->packet == NULL) {
        return -1;
    }
#ifdef CONFIG_BOARD_DYNAMIC_ALLOC
    ctx->pos = tiot_packet_expand(ctx->packet, len);
    return ctx->pos == NULL ? -1 : ERRCODE_TIOT_SUCC;
#else
    if (circ_buf_query_free(&(ctx->rx_buff.buf)) <= len) {
        return -1;
    }
    tiot_packet_expand(ctx->packet, len);
    return ERRCODE_TIOT_SUCC;
#endif
}

static inline void enqueue_cur_packet(tiot_packet_context *ctx)
{
    if (tiot_packet_queue_enqueue(ctx->rx_queue) != ERRCODE_TIOT_SUCC) {
        tiot_print_err("[TIoT:rx]circ queue [%u] no space.\r\n", ctx->rx_queue_id);
        discard_packet(ctx, ctx->packet);
    }
    /* the packet has been enqueued or freed. */
    tiot_packet_reset(ctx->packet);
}

static inline void discard_cur_packet(tiot_packet_context *ctx)
{
    if (ctx->packet != NULL) {
        discard_packet(ctx, ctx->packet);
        tiot_packet_reset(ctx->packet);
...
static inline void tiot_cur_packet_update(tiot_packet_context *ctx, uint8_t subsys_code)
{
    ...
    if ((ctx->packet == NULL) || (queue_id != ctx->rx_queue_id)) {
        ...
        ctx->rx_queue = tiot_packet_manager_find_queue(&ctx->rx_manager, queue_id);
        ctx->packet = tiot_packet_queue_get_packet(ctx->rx_queue);
    }
}
...
static inline void rx_buff_init(tiot_packet_circ_buffer *rx_buff, tiot_packet_buffer *buff)
{
    (void)memset_s(buff->start, buff->size, 0, buff->size);
    circ_buf_init(&(rx_buff->buf), &(rx_buff->buff_write), &(rx_buff->buff_read), buff->start, buff->size);
}
#endif
...
#ifndef CONFIG_BOARD_DYNAMIC_ALLOC
    rx_buff_init(&ctx->rx_buff, param->buff);
#endif
...
{
    int32_t copy_len;
    tiot_packet_queue *queue = ctx->rx_queue;
...
static void packet_handle_data(tiot_packet_context *ctx, uint8_t *data, uint16_t len)
{
#ifndef CONFIG_BOARD_DYNAMIC_ALLOC
    /* Copy non-sys payload. */
    if ((ctx->current_frame == TIOT_PKT_FRAME_PAYLOAD) &&
        (ctx->head.subsys_code != SYS_MSG)) {
        if (circ_buf_query_free(&(ctx->rx_buff.buf)) > len) {
            circ_buf_write(&(ctx->rx_buff.buf), data, len);
        }
        return;
    }
#endif
    /* Copy head, tail, and payload if dynamic alloc, else sys payload. */
    if (ctx->pos != NULL) {
        (void)memcpy_s(ctx->pos, len, data, len);
        ctx->pos += len;
    } else {
        tiot_print_dbg("[TIoT][rx]pkt ctx pos is NULL.\n");
    }
}
...
static void tiot_packet_head_done(tiot_packet_context *ctx)
{
    ...
    if (expand_cur_packet(ctx, payload_len) != ERRCODE_TIOT_SUCC) {
        tiot_print_err("[TIoT:rx]no mem or rx buff no space.\n");
        packet_frame_error(ctx);
    }
...
uint16_t tiot_packet_handle(tiot_packet_context *ctx, uint8_t *data, uint32_t data_len)
{
    uint16_t packet_len = (uint16_t)(data_len & 0xFFFF);  /* 取低16位，包格式只需要16位大小. */
    uint16_t handle_len = packet_len < ctx->frame_need_read ? packet_len : ctx->frame_need_read;
    ctx->frame_need_read -= handle_len;

    packet_handle_data(ctx, data, handle_len);
    ...
    switch (ctx->current_frame) {
        ...
        case TIOT_PKT_FRAME_TAIL:
            ...
            break;
        default:
            tiot_print_dbg("[TIoT][rx]err frame: %u\n", ctx->current_frame);
            break;
    }
    return handle_len;
}
...
{
    uint8_t *pos = tiot_packet_expand(ctx->packet, line_len);
    if (pos != NULL) {
        (void)memcpy_s(pos, line_len, line_start, line_len);
    } else {
        tiot_print_err("[TIoT:rx]no mem for ascii line.\n");
    }
}
#else
static inline void tiot_packet_ascii_line_store(tiot_packet_context *ctx, uint8_t *line_start, uint32_t line_len)
{
    if (circ_buf_query_free(&(ctx->rx_buff.buf)) <= line_len) {
        tiot_print_err("[TIoT:rx]rx buff no space for ascii line.\n");
        return;
    }
    tiot_packet_expand(ctx->packet, line_len);
...
static int tiot_packet_read_out_check(const void *param)
{
    ...
    check = (xfer->rx_wait_func(xfer) != 0) ||
            (tiot_packet_queue_has_data(query->queue));
    return check;
}
                                           ...
                                           uint8_t *buff, uint32_t buff_len)
{
    ...
    do {
        ...
        copy_len = tiot_packet_len(packet);
        if (copy_len == 0) {
            tiot_print_err("[TIoT][rx]invalid packet.\n");
            free_packet(ctx, packet);
            continue;
        }
        ...
        if (copy_len > buff_len) {
            tiot_print_info("[TIoT][rx]not enough buff len:%u for this packet: %u.\n", buff_len, copy_len);
            free_packet(ctx, packet);
            continue;
        }
        ...
        copy_from_packet(ctx, packet, tmp_buff);
        break;
    } while (packet != NULL);
                                ...
                                const tiot_xfer_rx_param *param)
{
    ...
    if (queue->flags & TIOT_PACKET_QUEUE_RX_WAITING) {
        tiot_print_err("[TIoT:pkt][%u]there's already a thread blocking read().\r\n", param->subsys);
    ...
    queue->flags &= ~TIOT_PACKET_QUEUE_RX_WAITING;
    if (read_ret == 0) {
        tiot_print_warning("[TIoT:pkt][%u]osal_wait timeout.\r\n", param->subsys);
    } else if (read_ret < 0) {
...
int32_t tiot_packet_read_out(tiot_xfer_manager *xfer, uint8_t *buff, uint32_t len, const tiot_xfer_rx_param *param)
{
    ...
    if (param->timeout == 0) { /* 非阻塞. */
        /* 没有数据直接返回. */
        if (tiot_packet_read_out_check(&query) == 0) {
            tiot_print_dbg("[TIoT:pkt][%u]no data to read.\r\n", param->subsys);
            return 0;
        }
    } else { /* 阻塞. */
