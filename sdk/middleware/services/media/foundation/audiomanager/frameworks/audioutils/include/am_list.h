 ...
 *              可不注册；数据类型是指针，若不注册duplicate函数，由用户自己保障内存的分配、复制和释放。
 *        2、关于free函数
 *           a）若用户注册free函数，则调用该函数释放数据。
 *           b）如果数据是int型、且长度<=sizeof(uintptr_t)，则用户不需要注册资源释放函数；
 *           c）若数据为指针，不注册free函数，由用户自己保障私有数据privatedata的内存释放。
 ...
 */
ListIterator ListIterBegin(const List *list);
 ...
 */
ListIterator ListIterEnd(const List *list);
