...
public:
    ...
    int32_t Init(void);
    int32_t Deinit(void);
    int32_t ProcVidFrame(int64_t ptsMs, SyncRet &result);
    int32_t ProcAudFrame(int64_t ptsMs, SyncRet &result);
...
private:
    void WaitForFirstVideoFrame(SyncRet &result, bool &timeOut);
    int32_t ProcessFristAudioFrame(int64_t timeUs);
    void ProcessAudioDiff(int64_t timeUs);
    SyncRet CheckAVDiff(int64_t latenessMs, uint32_t &continueLost);
    void UpdateCurTimeWithAudio(int64_t& nowUs);
    SyncRet ProcessFristVideoFrame(int64_t ptsUs);
    int32_t CheckVidSyncEnable(SyncRet &result, int32_t &ret);
    int32_t CheckAudSyncEnable(SyncRet &result, int32_t &ret);
    int32_t TPlayProcess(int64_t timestampUs, SyncRet &syncRet);
private:
    PlayerSyncAttr syncAttr_;
    MediaMutexHandle audSyncLock_;
    MediaMutexHandle vidSyncLock_;
    bool isVidEnable_;
    bool isAudEnable_;
