                                        ...
                                        int maxLen, int &actualLen)
{
    ...
    param[index].val = static_cast<void *>(&(attr.adecAttr.mime));
    param[index].size = sizeof(attr.adecAttr.mime);
    index++;
    param[index].key = KEY_BUFFERSIZE;
    param[index].val = static_cast<void *>(&(attr.adecAttr.bufSize));
    ...
    if (attr.adecAttr.mime == MEDIA_MIMETYPE_AUDIO_PCM) {
        ...
        param[index].val = static_cast<void *>(&attr.adecAttr.channelCnt);
...
int32_t Decoder::CreateHandle(const std::string &name, AvAttribute &attr)
{
    ...
    if (memset_s(param, PARAM_MAX_NUM * sizeof(Param), 0x00, PARAM_MAX_NUM * sizeof(Param)) != EOK) {
        MEDIA_ERR_LOG("memset_s failed");
        return CODEC_FAILURE;
    }
    if (!ConvertAttributeToParams(attr, param, PARAM_MAX_NUM, actualLen)) {
        MEDIA_ERR_LOG("convert attr to params failed");
        return CODEC_FAILURE;
    }
...
int32_t Decoder::SetCallback(CodecCallback &cb, uintptr_t instance)
{
