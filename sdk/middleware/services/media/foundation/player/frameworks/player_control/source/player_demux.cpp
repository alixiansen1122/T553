...
namespace OHOS {
namespace Media {
const uint32_t READ_FRAME_TIMEOUT = 20;
      ...
      fd_(-1)
{
    ...
    streamSource_.param.codecType = CODEC_BUT;
...
int32_t PlayerDemuxer::SetSource(int32_t fd, uint64_t offset)
{
...
int32_t PlayerDemuxer::SetSource(const StreamSourceParam &streamSource)
{
    CHECK_FAILED_RETURN(inited_, true, -1, "not inited");
...
int32_t PlayerDemuxer::SetCallBack(const PlayEventCallback &callBack)
{
    CHECK_FAILED_RETURN(inited_, true, -1, "not inited");
    callBack_ = callBack;
    return 0;
...
int32_t PlayerDemuxer::GetStreamSourceFileInfo(FormatFileInfo &fileInfo) const
{
    if (sourceType_ != SOURCE_TYPE_STREAM) {
        return -1;
    }
    fileInfo.s64FileSize = -1;
    fileInfo.s64StartTime = 0;
    fileInfo.u32Bitrate = 0;
    fileInfo.s64Duration = -1;
    if (streamSource_.param.channelCount > 0) {
        fileInfo.u32AudioChannelCnt = streamSource_.param.channelCount;
    }
    if (streamSource_.param.sampleRate > 0) {
        fileInfo.u32SampleRate = streamSource_.param.sampleRate;
    }
    fileInfo.enAudioType = streamSource_.param.codecType;
    fileInfo.s32UsedAudioStreamIndex = 0;
    fileInfo.trackCnt = 1;
    return 0;
}

void PlayerDemuxer::GetAlbumInfoToFileInfo(FormatFileInfo &fileInfo) const
{
    ParameterItem param = {};
    param.key = KEY_TYPE_ALBUM_ART_INFO;
    param.size = 0;
    param.value.pValue = nullptr;
    if (FormatDemuxerGetParameter(demuxer_, 0, &param) != 0) {
        return;
    }

    if (param.size <= 0 || param.value.pValue == nullptr) {
        return;
    }

    fileInfo.albumInfoCnt = param.size;
    fileInfo.albumInfo = static_cast<const FormatAlbumArtInfoItem *>(param.value.pValue);
}
void PlayerDemuxer::GetFormatFileInfoFromProgramInfo(int32_t trackId[], int32_t trackNum,
    FormatFileInfo &fileInfo, ProgramInfo *programInfo) const
{
    (void)trackNum; /* Unused Variables */
    ...
    for (uint32_t i = 0; i < programInfo->trackNum; i++) {
        if (programInfo->track[i].trackId == trackId[0] || programInfo->track[i].trackId == trackId[1]) {
            if (programInfo->track[i].trackType == TRACK_TYPE_VIDEO) {
                ...
                fileInfo.fFrameRate = programInfo->track[i].vidTrack.fpsNum;
                ...
                fileInfo.sampleFmt = programInfo->track[i].audTrack.sampleFmt;
            }
        }
    }
}

void PlayerDemuxer::GetStreamResolutionFromFileInfo(FormatFileInfo &fileInfo, FileInfo info) const
{
    int32_t index = 0;
    for (uint32_t i = 0; i < info.programNum; i++) {
        ProgramInfo programInfo = info.programInfo[i];
        for (uint32_t j = 0; j < programInfo.trackNum; j++) {
    ...
    fileInfo.trackCnt = info.programInfo[0].trackNum;
}

int32_t PlayerDemuxer::GetFileInfo(FormatFileInfo &fileInfo) const
{
    uint32_t i;
    FileInfo info;
    int32_t programId = -1;
    int32_t trackId[2] = {-1, -1};
    int32_t trackNum = 2;
    ProgramInfo *programInfo = nullptr;
    CHECK_FAILED_RETURN(prepared_, true, -1, "not prepared");
    if (sourceType_ == SOURCE_TYPE_STREAM) {
        return GetStreamSourceFileInfo(fileInfo);
    }
    ...
    GetFormatFileInfoFromProgramInfo(trackId, trackNum, fileInfo, programInfo);
    GetStreamResolutionFromFileInfo(fileInfo, info);
    GetAlbumInfoToFileInfo(fileInfo);
    return 0;
}

int32_t PlayerDemuxer::SelectTrack(int32_t programId, int32_t trackId) const
{
    CHECK_FAILED_RETURN(prepared_, true, -1, "not prepared");
    return FormatDemuxerSelectTrack(demuxer_, programId, trackId);
}

int32_t PlayerDemuxer::UnselectTrack(int32_t programId, int32_t trackId) const
{
    CHECK_FAILED_RETURN(prepared_, true, -1, "not prepared");
    return FormatDemuxerUnselectTrack(demuxer_, programId, trackId);
}

int32_t PlayerDemuxer::GetSelectedTrack(int32_t &programId, int32_t trackId[], int32_t &nums) const
{
    CHECK_FAILED_RETURN(prepared_, true, -1, "not prepared");
...
int32_t PlayerDemuxer::ReadFrame(FormatFrame &frame) const
{
    CHECK_FAILED_RETURN(started_, true, -1, "not started");
    return FormatDemuxerReadFrame(demuxer_, &frame, READ_FRAME_TIMEOUT);
}

int32_t PlayerDemuxer::FreeFrame(FormatFrame &frame) const
{
    CHECK_FAILED_RETURN(started_, true, -1, "not started");
    return FormatDemuxerFreeFrame(demuxer_, &frame);
}

int32_t PlayerDemuxer::Seek(int32_t streamIndex, int64_t timeStampUs, FormatSeekMode mode) const
{
    CHECK_FAILED_RETURN(started_, true, -1, "not started");
    return FormatDemuxerSeek(demuxer_, streamIndex, timeStampUs, mode);
}

int32_t PlayerDemuxer::Stop() const
{
    if (demuxer_ == nullptr) {
        return 0;
    }
...
int32_t PlayerDemuxer::SetParam(int32_t trackId, const ParameterItem *metaData, int32_t metaDataCnt) const
{
    CHECK_FAILED_RETURN(inited_, true, -1, "not inited");
    return FormatDemuxerSetParameter(demuxer_, trackId, metaData, metaDataCnt);
}

int32_t PlayerDemuxer::GetParam(int32_t trackId, ParameterItem &metaData) const
{
    CHECK_FAILED_RETURN(inited_, true, -1, "not inited");
