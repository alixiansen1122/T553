...
do { \
    if ((value) != (target)) { \
        MEDIA_ERR_LOG("sink_manager %s ", (printfString) ? (printfString) : " "); \
        return ret; \
    } \
} while (0)
#else
...
int32_t SinkManager::AddNewSink(const SinkAttr &attr)
{
    int32_t ret;
    if (sync_ == nullptr) {
...
int32_t SinkManager::Flush(void) const
{
    int32_t i;
...
int32_t SinkManager::Pause()
{
    ...
    for (int32_t i = 0; i < MAX_PIPELINE_SINK_NUM; i++) {
        if (videoSinkInfo_[i].sink != nullptr) {
            videoSinkInfo_[i].sink->Pause();
        }
    }
    if ((audioSinkInfo_[0].sink == nullptr || recieveAudioEos_ || speed_ != 1.0) && (sync_ != nullptr)) {
        sync_->Reset(SYNC_CHN_VID);
    }
...
int32_t SinkManager::Resume()
{
    ...
    for (int32_t i = 0; i < MAX_PIPELINE_SINK_NUM; i++) {
        if (audioSinkInfo_[i].sink != nullptr) {
            ret = audioSinkInfo_[i].sink->Resume();
            if (ret != HI_SUCCESS) {
                MEDIA_ERR_LOG("audio sink %d resume failed", i);
                return ret;
            }
        }
    }
    for (int32_t i = 0; i < MAX_PIPELINE_SINK_NUM; i++) {
        if (videoSinkInfo_[i].sink != nullptr) {
            ret = videoSinkInfo_[i].sink->Resume();
            if (ret != HI_SUCCESS) {
                MEDIA_ERR_LOG("video sink %d resume failed", i);
                return ret;
            }
        }
    }
...
int32_t SinkManager::SetSpeed(const float speed)
{
    int32_t ret;
    if (speed_ == speed) {
        return 0;
    ...
    if (audioSinkInfo_[0].sink != nullptr) {
        MEDIA_ERR_LOG("support only video stream");
        return -1;
    }

    ret = sync_->Reset(SYNC_CHN_VID);
    if (ret != HI_SUCCESS) {
        MEDIA_ERR_LOG("sync reset vid failed, ret: %d", ret);
        return ret;
    }

    ret = sync_->SetSpeed(speed);
    if (ret != HI_SUCCESS) {
        MEDIA_ERR_LOG("sync set speed failed, ret: %d", ret);
        return ret;
    }
...
int32_t SinkManager::GetSpeed(float &speed) const
{
    speed = speed_;
    return 0;
}
...
int32_t SinkManager::RenderEsFrame(const EsFrameInfo &frame) const
{
    int ret;
    if (frame.type == ESFRAME_TYPE_AUDIO) {
        ret = audioSinkInfo_[0].sink->RenderEsFrame(frame);
    } else {
        MEDIA_ERR_LOG("RenderEsFrame not support frame type: %d", frame.type);
        ret = -1;
    }
    return ret;
}

int32_t SinkManager::SetVolume(const float left, const float right)
{
    int i;
...
int32_t SinkManager::GetVolume(float &left, float &right) const
{
    left = leftVolume_;
    right = rightVolume_;
    return 0;
...
int32_t SinkManager::RegisterCallBack(const PlayEventCallback &callback)
{
    int32_t i;
    callBack_ = callback;
    for (i = 0; i < MAX_PIPELINE_SINK_NUM; i++) {
...
int32_t SinkManager::GetStatus(PlayerStreamInfo &streamInfo) const
{
    if (audioSinkInfo_[0].sink != nullptr) {
...
int32_t SinkManager::SetParam(const std::string &key, const DataType type, const void *value, int32_t len) const
{
    MEDIA_UNUSED(type);
    return audioSinkInfo_[0].sink->SetParam(key, type, value, len);
}

int32_t SinkManager::GetParam(const std::string &key, const DataType type, void *value, int32_t size)
{
    MEDIA_UNUSED(type);
    return audioSinkInfo_[0].sink->GetParam(key, type, value, size);
}

void SinkManager::RenderEos(const bool isAudio)
{
    if (audioSinkInfo_[0].sink != nullptr && isAudio) {
        audioSinkInfo_[0].sink->RenderEos();
        recieveAudioEos_ = true;
    }
    if (videoSinkInfo_[0].sink != nullptr && !isAudio) {
        videoSinkInfo_[0].sink->RenderEos();
        recieveVideoEos_ = true;
    }
}

bool SinkManager::HaveCacheFrame(bool isAudio)
{
    if (videoSinkInfo_[0].sink != nullptr && !isAudio) {
        return videoSinkInfo_[0].sink->HaveCacheFrame();
    }
    return -1;
}
...
{
#ifndef UNSUPPORT_AUDIO_DECODER
    if (audioSinkInfo_[0].sink != nullptr && audioSink) {
        return audioSinkInfo_[0].sink->DequeReleaseFrame(frame);
    }
#endif
    if (videoSinkInfo_[0].sink != nullptr && !audioSink) {
        return videoSinkInfo_[0].sink->DequeReleaseFrame(frame);
    }
    return -1;
}

int32_t SinkManager::DequeReleaseEsFrame(bool audioSink, EsFrameInfo &frame)
{
    if (audioSinkInfo_[0].sink != nullptr && audioSink) {
        return audioSinkInfo_[0].sink->DequeReleaseEsFrame(frame);
    }

    return -1;
}

void SinkManager::GetRenderPosition(int64_t &position) const
{
    ...
    /* use the max position of audio and video sink, if have recieved audio eos */
    int64_t audioPos;
    int64_t videoPos;
...
bool SinkManager::IsOffloadSupported(const AudioOffloadParam &offloadInfo) const
{
    if (audioSinkInfo_[0].sink != nullptr) {
        return audioSinkInfo_[0].sink->IsOffloadSupported(offloadInfo);
    }
    return false;
}

int32_t SinkManager::SetOutputDevice(const int32_t deviceId) const
{
    if (audioSinkInfo_[0].sink != nullptr) {
        return audioSinkInfo_[0].sink->SetOutputDevice(deviceId);
    }
    return -1;
}

void SinkManager::RecordLastPosition()
{
    if (audioSinkInfo_[0].sink != nullptr) {
        audioSinkInfo_[0].sink->RecordLastPosition();
    }
}
}
}
