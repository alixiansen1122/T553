...
enum PlayerControlError {
    ...
    PLAYERCONTROL_ERROR_UNKNOWN,              /* Unknown error. */
...
enum PlayerControlEvent {
    ...
    PLAYERCONTROL_BUFFER_EMPTY,              /* audio buffer empty */
    PLAYERCONTROL_FIRST_VIDEO_FRAME,
    PLAYERCONTROL_FIRST_AUDIO_FRAME,
    PLAYERCONTROL_EVENT_PAUSED,              /* play pause */
...
struct SourceFdParam {
    int32_t fd;
    uint64_t offset;
};

struct StreamParam {
    CodecFormat codecType;
    uint32_t sampleRate;
    uint32_t sampleFormat;
    uint32_t channelCount;
    uint32_t channelLayout;
};
struct StreamSourceParam {
    StreamParam  param;
    BufferStream stream;
};
#ifdef __cplusplus
