...
typedef struct {
    ...
    uint32_t maxHeight;
    uint32_t bufSize;
    uint32_t frameBufCnt; // The number of buffer is not determined by the preceding parameters.
    void *priv;
} VdecAttr;
...
public:
    ...
    int32_t GetMetadata(Param *params, int paramCnt);
    ...
    int32_t SetCallback(CodecCallback &cb, uintptr_t instance);
