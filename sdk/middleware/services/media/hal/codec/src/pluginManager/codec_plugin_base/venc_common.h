...
typedef struct {
    ...
    int32_t (*pfnQueueInput)(void *vencHdl, const InputInfo *inputInfo, uint32_t frameIndex, bool block);
    int32_t (*pfnDequeueOutput)(void *vencHdl, OutputInfo *outInfo, bool block);
    int32_t (*pfnQueueOutput)(void *vencHdl, OutputInfo *outInfo);
} AvVideoEncoder;

void *VEncoderGetImpl(void);

int32_t VEncoderImplUnRegister(void);
