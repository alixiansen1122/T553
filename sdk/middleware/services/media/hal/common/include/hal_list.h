 ...
 * Create: 2020-06-01
 * Notes: 1、本头文件是在 rawlist.h 基础上，借鉴C++面向对象抽象方法，从方便使用者使用的角度出发，抽象提炼的接口。
 *           使用者不需要看到链表节点，只关注自己的数据。
 *        2、关于 duplicate 函数
 *           a）duplicate 函数在初始化时注册；
 *           b）如果使用者注册 duplicate 函数，则调用该函数进行数据拷贝，否则直接复制userData(uintptr_t)数据。
 *              使用者的数据类型是int型，且长度 <= sizeof(uintptr_t)，如32位环境下uint32_t、64位环境下uint32_t、uint64_t等，
 *              可不注册；数据类型是指针，若不注册duplicate函数，由使用者自己保障内存的分配、复制和释放。
 *        2、关于free函数
 *           a）若使用者注册free函数，则调用该函数释放数据。
 *           b）如果数据是int型、且长度<=sizeof(uintptr_t)，则使用者不需要注册资源释放函数；
 *           c）若数据为指针，不注册free函数，由使用者自己保障私有数据privatedata的内存释放。
 *        4、链表处理内部无锁，如果使用者需要支持多线程并发，则需要在外层加锁。
 *   +------------------------------------------------------------------------+
 *   | +-------------------------------------------------------------------+  |
 *   | |       head                                                        |  |
 *   | |   +----------+    +-------------------------+     +-----------+   |  |
 ...
 *         | freefunc |    | 长度<=sizeof(uintptr_t) |     | (pointer) |--------+  使用者私有资源
 *         | dupfunc  |    +-------------------------+     +-----------+       \|/
 *         +----------+                                                  +-------------+
 *                                                                       | privatedata |
 *                                                                       +-------------+
 ...
 * @ingroup list
 * @brief 使用者内存释放函数原型
 * @par 描述：资源释放函数原型，一般用于机制批量free内存时，内存中可能含有使用者私有资源，这是需要使用者自行释放
 * @param ptr    [IN] 指向数据的指针
 * @retval 无
 */
...
/**
 * @ingroup list
 * 该结构被用来保存双向链表中节点的前向指针和后向指针，以及少量数据或指针。
 */
typedef struct {
    RawListNode rawNode;
    uintptr_t      userdata;
 ...
 * @ingroup list
 * @brief 初始化链表
 * @par 描述：初始化链表，按需注册数据dup函数和数据资源free函数。\n
 * dup 函数为NULL，表示链表中的数据类型是 int 型的，且长度 <= sizeof(uintptr_t)；其它情况，dup 函数都不能为空。\n
 * free 函数为NULL，表示链表中的数据类型是 int 型的，且长度 <= sizeof(uintptr_t)；其它情况，free 函数都不能为空。
 * @param list      [IN] 链表
 * @param dupFunc   [IN] 数据dup函数
 * @param freeFunc  [IN] 数据free函数
 */
 ...
 * @ingroup list
 * @brief 链表去初始化
 * @par 描述：链表去初始化：删除所有节点，调使用者注册的free函数释放使用者资源，去注册钩子函数。
 * @param list        [IN]  链表
 */
 ...
 * @brief 在链表头插入数据。
 * @param list      [IN] 链表
 * @param userData  [IN] 待插入的数据或指向使用者私有数据的指针
 * @retval OK       插入数据成功
 ...
 * @ingroup list
 * @brief 从链表头部POP一个节点
 * @par 描述：从链表中移除头节点，同时释放节点内存。如果在初始化时注册了free函数，还会调该钩子函数释放使用者私有资源。\n
 * 如果链表为空，则不做任何事情。
 * @param list [IN]  链表
 */
 ...
 * @ingroup list
 * @brief 从链表尾部POP一个节点。
 * @par 描述：从链表中移除尾节点，同时释放节点内存。如果在初始化时注册了free函数，还会调该钩子函数释放使用者私有资源。\n
 * 如果链表为空，则不做任何事情。
 * @param list [IN]  链表
 */
...
/**
 * @ingroup list
 * @brief 访问链表头节点，返回头节点的数据。
 * @par 描述：访问链表头节点，返回头节点的数据
 * @attention 注意：如果链表为空，则不能区分是因为链表为空返回0，还是返回的真实数据就是0。\n
 * 因此使用者在调用本函数前必须先判链表是否为空。
 * @param list [IN] 链表
 * @retval 头节点的数据/指针。如果链表为空，则返回0。
 */
uintptr_t HalListFront(List *list);

/**
 * @ingroup list
 * @brief 访问链表尾节点，返回尾节点的数据。
 * @attention 注意：如果链表为空，则不能区分是因为链表为空返回0，还是返回的真实数据就是0。\n
 * 因此使用者在调用本函数前必须先判链表是否为空。
 * @param list  [IN] 链表
 * @retval 尾节点的数据/指针。如果链表为空，则返回0。
 */
 ...
 * 如果在初始化时注册了free函数，还会调该钩子函数释放数据中的句柄、指针等私有资源。
 * @param list [IN] 链表
