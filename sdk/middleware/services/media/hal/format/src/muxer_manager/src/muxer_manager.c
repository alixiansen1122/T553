...
void MuxerManagerInit(void)
{
    ...
    MediaHalInitStaticMutexLock(&muxerMgr->mutex);
    MediaHalMutexLock(muxerMgr->mutex);
    if (muxerMgr->inited) {
        MediaHalMutexUnLock(muxerMgr->mutex);
        return;
    }
...
int32_t MuxerCreate(FormatHandle * const handle, const FormatOutputConfig *outputConfig)
{
    ...
    ctx->cfgParam.type = outputConfig->type;
    ctx->cfgParam.format = (outputConfig->format != OUTPUT_FORMAT_INVALID) ? outputConfig->format :
    ...
    if (outputConfig->type == OUTPUT_TYPE_FD) {
        ...
        if (memcpy_s(ctx->cfgParam.url, URL_LEN, outputConfig->url, URL_LEN) != EOK) {
...
int32_t MuxerAddTrack(const FormatHandle handle, const TrackSource *trackSource)
{
    ...
    MuxerTrackSource mediaTrackSource;
    mediaTrackSource.type = trackSource->trackSourceType;
    if (mediaTrackSource.type == STREAM_TYPE_VIDEO) {
        ...
        audioInfo->sampleBitWidth = trackSource->trackSourceInfo.audioInfo.sampleBitWidth;
        ...
        if (memset_s(&mediaTrackSource, sizeof(mediaTrackSource), 0, sizeof(MuxerTrackSource)) != EOK) {
            MEDIA_HAL_LOGI(MODULE_NAME, "memset_s failed");
        }
    }
...
int32_t MuxerSetParameter(const FormatHandle handle, int32_t trackId, const ParameterItem *item, int32_t itemNum)
{
    ...
    for (int32_t index = 0; index < itemNum; index++) {
        ret = ctx->plugin->fmt_invoke(ctx->muxer, item[index].key,
            (void *)&item[index].value.u32Value);
        if (ret != MEDIA_HAL_OK) {
            MEDIA_HAL_LOGE(MODULE_NAME, "set param by key[%d] failed !", item[index].key);
            break;
        }
    }
    MediaHalMutexUnLock(ctx->mutex);
...
int32_t MuxerGetParameter(const FormatHandle handle, int32_t trackId, ParameterItem *item, int32_t itemNum)
{
    ...
    for (int32_t index = 0; index < itemNum; index++) {
        ret = ctx->plugin->fmt_invoke(ctx->muxer, item[index].key,
            item[index].key == INVOKE_CURRENT_FILE_INFO ? (void *)&item[index].value.pValue :
            (void *)&item[index].value.u32Value);
        if (ret != MEDIA_HAL_OK) {
            MEDIA_HAL_LOGE(MODULE_NAME, "get param by key[%d] failed !", item[index].key);
            break;
        }
    }
    MediaHalMutexUnLock(ctx->mutex);
