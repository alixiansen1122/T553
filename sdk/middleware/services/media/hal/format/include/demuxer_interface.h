...
* Description: demuxer interfaces
...
typedef enum {
    ...
    DEMUXER_INVOKE_GET_ALBUM_ART_INFO,
    DEMUXER_INVOKE_BUT
} DemuxerInvokeType;
...
typedef struct {
    ...
    uint64_t offset; /* Local file descriptor */
    union {
        int fd; /* Local file descriptor */
        char url[URL_LEN]; /* Network URI or local file path. The maximum length is
