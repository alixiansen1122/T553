...
public:
     ...
     * The current module supports preview streams, video streams, photographing streams, and the like. For details,
     * see {@link StreamIntent}.
     *
     ...
     * application unaware of the stopping of the existing streams, <b>type</b> is set to <b>DYNAMIC_SUPPORTED</b>
     * so that the upper-layer service or application can directly add the new stream.
     * If the stream can be created only after the upper-layer service or application stops capturing all streams,
     * <b>type</b> is set to <b>RE_CONFIGURED_REQUIRED</b>. If the stream is not supported, <b>type</b> is set to
     * <b>NOT_SUPPORTED</b>.
     ...
     * @param modeSetting Indicates the stream configuration parameters, including the frame rate and 3A. 3A stands
     * for automatic focus (AF), automatic exposure (AE), and automatic white-balance (AWB).
     * @param info Indicates the stream configuration information. For details, see {@link StreamInfo}.
     * @param type Indicates the support type of the dynamically created stream. The supported types are defined in
     * {@link StreamSupportType}.
     *
     * @return Returns <b>NO_ERROR</b> if the operation is successful; returns an error code defined in
     * {@link CamRetCode} otherwise.
     *
     * @since 1.0
     * @version 1.0
     */
     ...
     *
     * @param streamInfos Indicates the list of stream information, which is defined by {@link StreamInfo}. The
     * passed stream information may be changed. Therefore, you can run {@link GetStreamAttributes} to obtain the
     * latest stream attributes after the stream is created.
     *
     * @return Returns <b>NO_ERROR</b> if the operation is successful; returns an error code defined in
     * {@link CamRetCode} otherwise.
     *
     ...
     *
     * @param mode Indicates the operation mode of the stream. For details, see {@link OperationMode}.
     * @param modeSetting Indicates the stream configuration parameters, including the frame rate
     ...
     *
     * @param attributes Indicates the obtained stream attributes. Stream information passed by the <b>streamInfos</b>
     * parameter in {@link CreateStreams}
     ...
     *
     * If a producer handle has been specified during stream creation (by {@link CreateStreams}), you do not need to
     * call this function. If you want to attach a new procedure handle, call {@link DetachBufferQueue} to detach the
     * existing procedure handle and then call this function to attach the new one. You do not need to attach a
     * procedure handle for IoT devices that do not support or require image data caching and transferring of
     * preview streams. In this case, set <b>bufferQueue_</b> in the {@link StreamInfo} parameter of
     ...
     * @version 1.0
     */
    virtual CamRetCode AttachBufferQueue(int streamId, const OHOS::sptr<OHOS::Surface> &producer) = 0;
     ...
     * There are two image capture modes: continuous capture and single capture. A continuous capture is performed
     * inside the module after being triggered, and the consumer can continuously receive captured image data after
     * calling this function only once. If this function is called again, the current capture is stopped, the capture
     * request configuration is updated, and a new capture is performed. This mode is mainly used in preview, video
     * recording, or continuous shooting scenarios. After a single capture is triggered, only one frame of image data
     * is captured. This mode applies to the single shooting scenario. When the capture is started,
     * {@link OnCaptureStarted} is called to notify of the start. To stop a continuous capture,
     * call {@link CancelCapture}. When the capture ends, {@link OnCaptureEnded} is called to notify the caller of
     * the information such as the number of captured frames. <b>enableShutterCallback_</b> in {@link CaptureInfo}
     * is used to enable the callback for each capture. After the callback is enabled, {@link OnFrameShutter} is
     * called upon each capture. In the scenario where multiple streams are captured at the same time, this module
     * ensures that the captured data of multiple streams is reported simultaneously.
     *
     * @param captureId Indicates the ID of the capture request. You must ensure that the ID of the capture request
     * is unique when the camera is started.
     * @param info Indicates the capture request configuration information. For details, see {@link CaptureInfo}.
     * @param isStreaming Specifies whether to continuously capture images. The value <b>true</b> means to
     * continuously capture images, and <b>false</b> means the opposite.
     *
     * @return Returns <b>NO_ERROR</b> if the operation is successful; returns an error code defined
     * in {@link CamRetCode} otherwise.
     *
     ...
     * Converting to an offline stream enables the bottom-layer device to close and the offline stream to take over
