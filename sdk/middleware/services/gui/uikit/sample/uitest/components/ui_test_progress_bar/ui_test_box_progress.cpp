...
void UITestBoxProgress::SetUp()
{
    if (container_ == nullptr) {
        ...
        uiViewGroup->SetPosition(positionX_, positionY_ + 20, 240, 360); // 20: offset; 240: width; 360: height
        container_->Add(uiViewGroup);
        UILabel* label = new UILabel();
        uiViewGroup->Add(label);
        container_->Add(uiViewGroup);
        // 50: offset; 200: width; 48: height
        label->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE, TEXT_DISTANCE_TO_TOP_SIDE + 50, 200, 48);
        label->SetText("BoxProgress");
        label->SetFont(DEFAULT_VECTOR_FONT_FILENAME, FONT_DEFAULT_SIZE);
        uiViewGroupFrame_ = new UIViewGroup();
        uiViewGroup->Add(uiViewGroupFrame_);
        // 50: offset; 200: width; 210: height
        uiViewGroupFrame_->SetPosition(VIEW_DISTANCE_TO_LEFT_SIDE2, TEXT_DISTANCE_TO_LEFT_SIDE + 50, 200, 210);
        ...
        // 10: offset; 100: width; 10: height
        boxProgress_->SetPosition(10, 10, 100, 10);
    ...
    if (scroll_ == nullptr) {
        ...
        // 250: x-pos; 11: y-pos
        scroll_->SetPosition(250, 11);
        // 300: width; 100: Adjust the visible height of scroll
        scroll_->Resize(300, Screen::GetInstance().GetHeight() - 100);
...
void UITestBoxProgress::UIKitBoxProgressTestUIBoxProgress001()
{
    ...
    positionX_ = TEXT_DISTANCE_TO_TOP_SIDE;
    positionY_ = 0;
...
void UITestBoxProgress::SetMoveBtnEvent(UIView& view)
{
    ...
    if (&view == resetBtn_) {
        ...
        boxProgress_->SetImage(RES_PATH"red.png", RES_PATH"blue.png");
    } else if (&view == noImageBtn_) {
        boxProgress_->SetImage(static_cast<ImageInfo*>(nullptr));
    }
}

void UITestBoxProgress::SetStyleBtnEvent(UIView& view)
{
    static uint16_t progress = 20; // 20: progress
    static uint16_t step = 0;
    static uint16_t width = 100; // 100: width
...
{
    SetMoveBtnEvent(view);
    SetStyleBtnEvent(view);
