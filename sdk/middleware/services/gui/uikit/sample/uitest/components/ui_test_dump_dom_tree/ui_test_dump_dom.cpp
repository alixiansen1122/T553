...
const uint16_t offsetx_ = 10;
const uint16_t offsety_ = 160;
const uint16_t offsetX = 20;
const uint16_t offsetY = 100;
const uint16_t x_coordinate1 = 230;
const uint16_t y_coordinate1 = 68;
const uint16_t y_coordinate2 = 108;
const uint16_t y_coordinate3 = 310;
const uint16_t x_coordinate = 105;
const uint16_t y_coordinate = 73;
const uint16_t g_height_ = 200;
const uint16_t g_height2_ = 200;
const uint16_t g_height3_ = 200;
const uint16_t g_height4_ = 500;
const uint16_t x_position = 90;
const uint16_t y_position = 300;
const uint16_t radius = 90;
const uint16_t angle_end = 270;
const uint16_t letter_space = 30;
const uint16_t dumph_ = 29;
namespace OHOS {
...
void UITestDumpDomTree::UIKit_TestDumpDom_001()
{
    if (container_ != nullptr) {
        ...
        label->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE + offsetX, TEXT_DISTANCE_TO_TOP_SIDE + offsetY);
        positionY_ += label->GetHeight();

        GridLayout* layout = new GridLayout();
        layout->SetPosition(VIEW_DISTANCE_TO_LEFT_SIDE + offsetX, VIEW_DISTANCE_TO_TOP_SIDE + offsetY,
...
void UITestDumpDomTree::UIKit_TestDumpDom_002()
{
    if (container_ != nullptr) {
        ...
        group2->SetHeight(g_height_); // 200: height
        ...
        group2->Add(label);
        label->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE, TEXT_DISTANCE_TO_TOP_SIDE + offsetY);

        UIImageView* imageView = new UIImageView();
        imageView->SetPosition(VIEW_DISTANCE_TO_TOP_SIDE, TEXT_DISTANCE_TO_LEFT_SIDE + offsetY);
        ...
        UILabelButton* dumpButton1 = GetLabelButton("dump");
        dumpButton1->SetPosition(VIEW_DISTANCE_TO_TOP_SIDE, VIEW_DISTANCE_TO_TOP_SIDE + offsetY);
...
void UITestDumpDomTree::UIKit_TestDumpDom_003()
{
    if (container_ != nullptr) {
        ...
        group5->SetHeight(g_height_); // 200: height
        ...
        slider->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE, VIEW_DISTANCE_TO_TOP_SIDE, BACK_BUTTON_HEIGHT,
            BACK_BUTTON_HEIGHT + BACK_BUTTON_HEIGHT);
        ...
        slider->SetValue(BACK_BUTTON_HEIGHT);     // 50: value of progress bar
        slider->SetStep(0);
        ...
        slider->SetKnobWidth(BACK_BUTTON_HEIGHT); // 50: knob width
        ...
        dumpButton->SetPosition(x_coordinate, y_coordinate); // 120: x-coordinate, 73: y-coordinate
        /* dump node here */
        if (clickDumpDomListener7_ == nullptr) {
...
void UITestDumpDomTree::UIKit_TestDumpDom_004()
{
    if (container_ != nullptr) {
        ...
        group4->SetHeight(g_height_); // 100: height
        ...
        // y_coordinate2: x-coordinate, 68: y-coordinate, 90: new width, 10: new height
        boxProgress->SetPosition(VIEW_DISTANCE_TO_LEFT_SIDE, y_coordinate1, 90, 10);
        ...
        UILabelButton* dumpButton = GetLabelButton("dump");
        dumpButton->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE, TEXT_DISTANCE_TO_TOP_SIDE + offsetY);
        /* dump node here */
        if (clickDumpDomListener8_ == nullptr) {
...
void UITestDumpDomTree::UIKit_TestDumpDom_005()
{
    if (container_ != nullptr) {
        ...
        group3->SetHeight(g_height4_); // 500: height
        ...
        label2->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE + offsetX, VIEW_DISTANCE_TO_TOP_SIDE + offsetx_,
            BUTTON_WIDHT1 + offsetx_, dumph_);
        label2->SetViewId("dump_label");
        group3->Add(label2);

        UILabelButton* labelButton = GetLabelButton("LabelButton");
        labelButton->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE, y_coordinate2); // 48: x position, 108: y position
        labelButton->SetViewId("dump_label_button");
        group3->Add(labelButton);

        UIArcLabel* label3 = new UIArcLabel();
        label3->SetArcTextCenter(x_position, y_position); // 70: x position, 330: y position
        label3->SetArcTextRadius(radius);     // 150: radius
        label3->SetArcTextAngle(0, angle_end);   // 0: start angle, 270: end angle
        label3->SetFont(DEFAULT_VECTOR_FONT_FILENAME, FONT_DEFAULT_SIZE);
        label3->SetText("012345678ABCDEF");
        label3->SetStyle(STYLE_LETTER_SPACE, letter_space); // 30: letter space
        label3->SetViewId("dump_arc_label");
        group3->Add(label3);

        UILabelButton* dumpButton1 = GetLabelButton("dump");
        dumpButton1->SetPosition(x_coordinate1, 48); // 230: x-coordinate, 48: y-coordinate
        /* dump node here */
        if (clickDumpDomListener9_ == nullptr) {
        ...
        UILabelButton* dumpButton2 = GetLabelButton("dump");
        dumpButton2->SetPosition(x_coordinate1, y_coordinate2); // 230: x-coordinate, 108: y-coordinate
        /* dump node here */
        if (clickDumpDomListener10_ == nullptr) {
        ...
        UILabelButton* dumpButton3 = GetLabelButton("dump");
        dumpButton3->SetPosition(x_coordinate1, y_coordinate3); // 230: x position, 310: y position
        /* dump node here */
        if (clickDumpDomListener11_ == nullptr) {
...
void UITestDumpDomTree::UIKit_TestDumpDom_006()
{
    if (container_ != nullptr) {
        ...
        circleProgress->SetPosition(BUTTON_HEIGHT1, VIEW_DISTANCE_TO_LEFT_SIDE, 150, 150);
        circleProgress->SetCenterPosition(BACK_BUTTON_HEIGHT, BACK_BUTTON_HEIGHT); // 50: x-coordinate, 50: y-coordinate
        circleProgress->SetRadius(BACK_BUTTON_HEIGHT); // 50: radius
        ...
        circleProgress->SetValue(BACK_BUTTON_HEIGHT); // 50: value of progress bar
        ...
        UILabelButton* dumpButton = GetLabelButton("dump");
        dumpButton->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE - offsetx_, TEXT_DISTANCE_TO_TOP_SIDE + offsety_);
        /* dump node here */
        if (clickDumpDomListener12_ == nullptr) {
...
{
    const int16_t labelButtonWidth = BUTTON_WIDHT2;
    const int16_t labelButtonHeight = BUTTON_HEIGHT1;
