...
const UIView* UITestChartPillar::GetTestView()
{
    ...
    UiKitChartPillarTestSwitchCap005();
    UiKitChartPillarTestSwitchPillarType006();
    return container_;
}

void UITestChartPillar::UiKitChartPillarTestAddDataSerial001()
{
    UILabel* label = GetTitleLabel("UIChartPillarExt效果 ");
    if (label == nullptr) {
        return;
    }
...
void UITestChartPillar::UiKitChartPillarTestSwitchCap005()
{
    switchCapBtn_ = new UILabelButton();
    positionX_ = 48; // 48: x-coordinate
    SetUpButton(switchCapBtn_, "Cap切换");
}

void UITestChartPillar::UiKitChartPillarTestSwitchPillarType006()
{
    switchPillarTypeBtn_ = new UILabelButton();
    positionX_ = switchCapBtn_->GetX() + switchCapBtn_->GetWidth() + g_blank;
    positionY_ = switchCapBtn_->GetY();
    SetUpButton(switchPillarTypeBtn_, "模式切换");
}
...
bool UITestChartPillar::OnClick(UIView& view, const ClickEvent& event)
{
    if (&view == addDataSerialBtn_) {
        ...
        reverseState_ = !reverseState_;
        chart_->EnableReverse(reverseState_);
        chart_->Invalidate();
    } else if (&view == setAxisColorBtn_) {
        chart_->GetXAxis().SetLineColor(Color::Orange());
        chart_->GetYAxis().SetLineColor(Color::Green());
        chart_->Invalidate();
    } else if (&view == setAxisVisibleBtn_) {
        chart_->GetXAxis().SetVisible(false);
        chart_->GetYAxis().SetVisible(false);
        chart_->Invalidate();
    } else if (&view == switchCapBtn_) {
        if (chart_->GetStyle(STYLE_LINE_CAP) == CapType::CAP_ROUND) {
            chart_->SetStyle(STYLE_LINE_CAP, CapType::CAP_NONE);
        } else {
            chart_->SetStyle(STYLE_LINE_CAP, CapType::CAP_ROUND);
        }
        chart_->Invalidate();
    } else if (&view == switchPillarTypeBtn_) {
        if (chart_->GetPillarType() == UIChartPillarExt::PillarType::PILLAR_NORMAL) {
            chart_->SetPillarType(UIChartPillarExt::PillarType::PILLAR_STACKED);
        } else {
            chart_->SetPillarType(UIChartPillarExt::PillarType::PILLAR_NORMAL);
        }
        chart_->Invalidate();
    }
    return true;
...
void UITestChartPillar::SetUpButton(UILabelButton* btn, const char* title)
{
    ...
    container_->Add(btn);
    btn->SetPosition(positionX_, positionY_, BUTTON_WIDHT1, BUTTON_HEIGHT2);
