...
const uint16_t offsetX = 102;
const uint16_t offsetY = 10;
const uint16_t offHeight = 30;
const uint16_t clock_positionx_ = 150;
const uint16_t clock_positiony_ = 70;
const uint16_t offset_ = 50;
const uint16_t offset2_ = 100;
const uint16_t offset3_ = 150;
const uint16_t offset4_ = 200;
const uint16_t diff_ = 100;
const uint16_t width_ = 58;
const uint16_t move_ = 100;
const uint16_t width1_ = 48;
} // namespace
...
void UITestAnalogClock::UIKitTestLineHandAnalogClock001()
{
    if (container_ != nullptr) {
        ...
        group1->SetHeight(Screen::GetInstance().GetHeight() + diff_); // 100: height bigger value
        ...
        label->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE + offsetX, TEXT_DISTANCE_TO_TOP_SIDE + offsetY,
            Screen::GetInstance().GetWidth(), offHeight);
...
void UITestAnalogClock::CreateButtons001(UIViewGroup* group, UIImageView* curFace, UIAnalogClock* clock)
{
    ...
    button1->SetPosition(curFace->GetWidth() + width_, clock_positiony_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (changeModeListener_ == nullptr) {
    ...
    button2->SetPosition(curFace->GetWidth() + width_, clock_positiony_ + offset_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (clickMoveLeftListener_ == nullptr) {
    ...
    button3->SetPosition(curFace->GetWidth() + width_, clock_positiony_ + offset2_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (clickMoveRightListener_ == nullptr) {
    ...
    button4->SetPosition(curFace->GetWidth() + width_, clock_positiony_ + offset3_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (clickMoveTopListener_ == nullptr) {
    ...
    button5->SetPosition(curFace->GetWidth() + width_, clock_positiony_ + offset4_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (clickMoveBottomListener_ == nullptr) {
...
void UITestAnalogClock::UIKitTestImageHandAnalogClock002()
{
    if (container_ != nullptr) {
        ...
        group2->SetPosition(0, Screen::GetInstance().GetHeight() - diff_); // 50: increase y-coordinate
        group2->SetWidth(Screen::GetInstance().GetWidth());
        group2->SetHeight(Screen::GetInstance().GetHeight() + diff_); // 100: increase y-coordinate
        container_->Add(group2);

        UILabel* label = new UILabel();
        group2->Add(label);
        label->SetPosition(VIEW_DISTANCE_TO_LEFT_SIDE + offsetX, 0, Screen::GetInstance().GetWidth(), offHeight);
...
void UITestAnalogClock::CreateButtons002(UIViewGroup* group, UIImageView* curFace, UIAnalogClock* clock)
{
    ...
    button1->SetPosition(curFace->GetWidth() + width_, width1_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (changeModeListener1_ == nullptr) {
    ...
    button2->SetPosition(curFace->GetWidth() + width_, width1_ + offset_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (clickMoveLeftListener1_ == nullptr) {
        clickMoveLeftListener1_ = static_cast<UIView::OnClickListener*>(
            new TestBtnOnClickMovePositionListener(
            static_cast<UIView*>(button2), clock,
            TestBtnOnClickMovePositionListener::MoveType::MOVE_LEFT, move_)); // 10: grid
    }
    ...
    button3->SetPosition(curFace->GetWidth() + width_, width1_ + offset2_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (clickMoveRightListener1_ == nullptr) {
        clickMoveRightListener1_ = static_cast<UIView::OnClickListener*>(
            new TestBtnOnClickMovePositionListener(
            static_cast<UIView*>(button3), clock,
            TestBtnOnClickMovePositionListener::MoveType::MOVE_RIGHT, move_)); // 10: move value
    }
    ...
    button4->SetPosition(curFace->GetWidth() + width_, width1_ + offset3_, BUTTON_WIDTH, BUTTON_HEIGHT);
    if (clickMoveTopListener1_ == nullptr) {
        clickMoveTopListener1_ = static_cast<UIView::OnClickListener*>(
            new TestBtnOnClickMovePositionListener(
            static_cast<UIView*>(button4), clock,
            TestBtnOnClickMovePositionListener::MoveType::MOVE_TOP, move_)); // 10: move value
    }
    button4->SetOnClickListener(clickMoveTopListener1_);
    UILabelButton* button5 = SetUpButton("下移");
    group->Add(button5);
    // 58: increase x-coordinate; 248: y-coordinate
    button5->SetPosition(curFace->GetWidth() + width_, width1_ + offset4_, BUTTON_WIDTH, BUTTON_HEIGHT);
    /* Move bottom */
    if (clickMoveBottomListener1_ == nullptr) {
        clickMoveBottomListener1_ = static_cast<UIView::OnClickListener*>(
            new TestBtnOnClickMovePositionListener(
            static_cast<UIView*>(button5), clock,
            TestBtnOnClickMovePositionListener::MoveType::MOVE_BOTTOM, move_)); // 10: move value
    }
