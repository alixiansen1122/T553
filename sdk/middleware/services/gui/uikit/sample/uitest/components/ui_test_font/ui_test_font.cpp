...
const uint16_t TEXT_BUF_SIZE = 200;
const uint16_t CONTAINER_Y = 100;
} // namespace
...
void UITestFont::SetUp()
{
    if (container_ == nullptr) {
        ...
        container_->Resize(Screen::GetInstance().GetWidth(),
                           Screen::GetInstance().GetHeight() - LABEL_HEIGHT - CONTAINER_Y);
...
void UITestFont::Font_FontEngine_FontConvert_Test_FontTestSetFontId_001()
{
    if (container_ != nullptr) {
        ...
        label->SetFont(DEFAULT_VECTOR_FONT_FILENAME, 18); // 18: means font size
        label->SetText("Hello!\nOHOS UIKIT!");
