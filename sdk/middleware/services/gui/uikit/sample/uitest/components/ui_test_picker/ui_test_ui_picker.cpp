...
void UITestUIPicker::UIKitPickerTestBase001()
{
    ...
    label->SetPosition(positionX_ * 3, 30); // 3 : 3 times of label , 30 : init height
    if (picker1_ == nullptr) {
        picker1_ = new UIPicker();
    }
    picker1_->SetPosition(positionX_ + 20, label->GetY() + g_blank, g_listW, g_listH); // 20 : adjust postion
    ...
    positionX_ = picker1_->GetWidth() + picker1_->GetX();
    ...
    positionY_ += label->GetY() + 10;   // 10 Gap of item
