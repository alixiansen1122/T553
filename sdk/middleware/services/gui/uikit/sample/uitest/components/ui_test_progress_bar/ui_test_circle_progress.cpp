...
const int16_t GROUP_WIDTH = 230;
const int16_t GROUP_HEIGHT = 300;
const int16_t MARGIN_TOP = 50;
} // namespace
...
void UITestCircleProgress::SetUp()
{
    if (container_ == nullptr) {
        ...
        uiViewGroup->SetPosition(positionX_, positionY_ + 20, GROUP_WIDTH, GROUP_HEIGHT); // 20: gap
        container_->Add(uiViewGroup);
        UILabel* label = new UILabel();
        uiViewGroup->Add(label);
        label->SetPosition(TEXT_DISTANCE_TO_LEFT_SIDE,
            TEXT_DISTANCE_TO_TOP_SIDE + MARGIN_TOP, 200, MARGIN_TOP); // 200:width
        label->SetText("CircleProgress");
        label->SetFont(DEFAULT_VECTOR_FONT_FILENAME, FONT_DEFAULT_SIZE);
        uiViewGroupFrame_ = new UIViewGroup();
        uiViewGroup->Add(uiViewGroupFrame_);
        uiViewGroupFrame_->SetPosition(VIEW_DISTANCE_TO_LEFT_SIDE2,
            VIEW_DISTANCE_TO_TOP_SIDE + MARGIN_TOP, 180, 190); // 180:width 190: height
        ...
        circleProgress_->SetPosition(0, 0, 150, 150);    // 150:width 150: height
    ...
    if (scroll_ == nullptr) {
        ...
        scroll_->SetPosition(210, 11);                      // 210: x-coordinate; 11: y-coordinate
        scroll_->Resize(305, container_->GetHeight() - 11); // 305: width; // 11: decrease height
...
void UITestCircleProgress::UIKitCircleProgressTestSetAngle015()
{
    ...
    positionX_ += BUTTON_WIDTH + VIEW_DISTANCE_TO_LEFT_SIDE2;
...
void UITestCircleProgress::SetBtnEvent1(const UIView& view,
    uint16_t& progress,
    uint16_t& step,
    int16_t& min,
    int16_t& max)
{
    if (&view == incProgressBtn_) {
        progress++;
...
void UITestCircleProgress::SetBtnEvent2(const UIView& view,
    Point& center,
    Point& imgPos,
    int16_t& startAngle,
    int16_t& endAngle)
{
    if (&view == incStartAngleBtn_) {
        startAngle++;
...
void UITestCircleProgress::SetBtnEvent3(const UIView& view, int16_t& radius)
{
    if (&view == imageBtn_) {
        circleProgress_->SetImage(RES_PATH"red.png", RES_PATH"blue.png");
...
bool UITestCircleProgress::OnClick(UIView& view, const ClickEvent& event)
{
    ...
    SetBtnEvent1(view, progress, step, min, max);
    SetBtnEvent2(view, center, imgPos, startAngle, endAngle);
    SetBtnEvent3(view, radius);
