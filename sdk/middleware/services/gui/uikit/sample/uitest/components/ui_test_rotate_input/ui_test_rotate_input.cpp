...
static int16_t g_smallButtonH = 60;
static int16_t g_smallButtonW = 150;
...
static int16_t g_testx = 150;
static int16_t g_testy = 40;
static int16_t g_offset = 200;
static int16_t g_height = 50;
static int16_t g_blank1 = 260;
static int16_t g_size = 15;
...
void UITestRotateInput::SetUp()
{
    if (container_ == nullptr) {
        ...
        container_->SetReboundSize(g_height);
    }
...
void UITestRotateInput::UIKit_Rotate_Event_List_001()
{
    ...
    label->SetPosition(g_testx, lastY_ + g_blank);
    if (adapter_ == nullptr) {
    ...
    list_->SetPosition(g_testx, label->GetY() + g_blank, g_listW, g_listH - g_offset); // 200: mean list reduce height
...
void UITestRotateInput::UIKit_Rotate_Event_Scroll_View_002()
{
    ...
    scroll_->SetPosition(BUTTON_WIDHT1, label->GetY() + g_blank, g_scrollW, g_scrollH);
    ...
    scroll_->SetReboundSize(g_height);
...
void UITestRotateInput::UIKit_Rotate_Event_Swipe_View_003()
{
    ...
    container_->Add(label);
    label->SetPosition(g_testx, lastY_ + g_blank + g_offset);
    if (swipe_ == nullptr) {
...
void UITestRotateInput::UIKit_Rotate_Event_Slider_004()
{
    ...
    slider_->SetPosition(g_testx, label->GetY() + g_blank, g_sliderW, g_sliderH);
    slider_->SetValidHeight(g_offset);
    slider_->SetValue(g_blank);
...
void UITestRotateInput::UIKit_Rotate_Event_Time_Picker_005()
{
    ...
    timePicker_->SetPosition(BUTTON_WIDHT1 + BACK_BUTTON_HEIGHT, label->GetY() + g_blank, g_listW, g_listH);
    timePicker_->SetItemHeight(g_height);
...
void UITestRotateInput::UIKit_Rotate_Event_Picker_006()
{
    ...
    picker_->SetPosition(BUTTON_WIDHT2, label->GetY() + g_blank, g_listW, g_listH);
    picker_->SetItemHeight(g_height);
...
void UITestRotateInput::CreateLabelButton(int16_t x, int16_t y, UIView::OnClickListener* listener, const char* text)
{
    ...
    labelButton->SetFont(DEFAULT_VECTOR_FONT_FILENAME, g_size);
...
void UITestRotateInput::SetTestLabelButton(UILabel* label, UIView* view)
{
    ...
    CreateLabelButton(g_testx, label->GetY() + g_blank1, listener1_, "factor: 0");
    if (listener2_ == nullptr) {
        listener2_ = static_cast<UIView::OnClickListener*>(
            new TestSetRotateFactorListener(view, 15)); // 15: rotate factor
    }
    CreateLabelButton(g_testx, lastY_ + g_blank, listener2_, "factor: 15");
    if (listener3_ == nullptr) {
        listener3_ = static_cast<UIView::OnClickListener*>(
            new TestSetRotateFactorListener(view, -15)); // -15: rotate factor
    }
    CreateLabelButton(g_testx, lastY_ + g_blank, listener3_, "factor: -15");
    if (listener4_ == nullptr) {
    ...
    CreateLabelButton(x, label->GetY() + g_blank1, listener4_, "factor: 30");
    if (listener5_ == nullptr) {
