...
#ifdef VERSION_IOT
static bool GetBytePerPixel(uint32_t colorMode, uint8_t& byte)
{
    switch (colorMode) {
        case ARGB8888:
            byte =  4; // 4 bytes per pixel
            break;
        case RGB888:
            byte = 3; // 3 bytes per pixel
            break;
        case RGB565:
            byte = 2; // 2 bytes per pixel
            break;
        default:
            return false;
    }
...
RetCode FileImgDecoder::ReadToCache(ImgResDsc& dsc)
{
    ...
    if (!dsc.inCache_) {
        ...
        if (dsc.imgInfo.header.compressMode != COMPRESS_MODE_NONE &&
            dsc.imgInfo.header.compressMode != COMPRESS_MODE_HFBC &&
            dsc.imgInfo.header.compressMode != COMPRESS_MODE_HFBC_ABYPASS) {
            ...
            readSuccess = (static_cast<int32_t>(pxCount) == read(dsc.fd,
                reinterpret_cast<void*>(const_cast<uint8_t*>(dsc.imgInfo.data)), pxCount));
        }
        if (!readSuccess) {
            ImageCacheFree(dsc.imgInfo);
...
#ifdef _WIN32
    ImgDecompress::GetInstance().Decompress(dsc.imgInfo, true);
#endif
    return RetCode::OK;
}
