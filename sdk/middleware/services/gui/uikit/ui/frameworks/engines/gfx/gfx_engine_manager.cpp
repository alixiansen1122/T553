...
void BaseGfxEngine::ClipCircle(const ImageInfo* info, float x, float y, float radius)
{
    ...
    if (info != nullptr) {
        ImageCacheManager::GetInstance().UpdateImageInfoIfNecessary(*const_cast<ImageInfo*>(info));
        if (info->data == nullptr) {
            GRAPHIC_LOGE("ClipCircle failed. UpdateImageInfoIfNecessary failed.");
            return;
        }
    }
#endif
...
void BaseGfxEngine::Blur(BufferInfo& dst, float radius)
{
    GRAPHIC_LOGE("Blur not support.");
}
