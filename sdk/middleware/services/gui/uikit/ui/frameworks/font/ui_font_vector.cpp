...
#include "common/ui_emoji_table.h"
#endif
...
UIFontVector::~UIFontVector()
{
    ...
    delete bitmapCache_;
#if CAPABILITY_HW_SUPPORT_EMOJI
    ImageCacheManager::GetInstance().UnloadAllInMultiRes(std::string{EMOJI_PATH});
#endif
}
...
bool UIFontVector::GetEmoji(uint32_t unicode, ImageInfo &unicodeInfo)
{
#if CAPABILITY_HW_SUPPORT_EMOJI
    constexpr int len = sizeof(EMOJI_TABLE) / sizeof(EMOJI_TABLE[0]);
    if (len == 0) {
        return false;
    }
    int index = -1;
    ...
    ImageInfo *info = ImageCacheManager::GetInstance().LoadOneInMultiRes(EMOJI_TABLE[index].resId, EMOJI_PATH);
    if (info == nullptr) {
        return false;
    }

    memcpy_s(&unicodeInfo, sizeof(unicodeInfo), info, sizeof(ImageInfo));
    return true;
#else
    return false;
#endif
...
int16_t UIFontVector::GetEmojiHeight()
{
    return GetHeight();
}

int16_t UIFontVector::GetEmojiWidth(uint32_t unicode, uint8_t fontId)
{
    ImageInfo info;
    if (!GetEmoji(unicode, info)) {
        return INVALID_RET_VALUE;
    }
    uint8_t old = GetBaseFontId();
    SetBaseFontId(fontId);
    float h = GetEmojiHeight();
    SetBaseFontId(old);
    float scale = h / info.header.height;
    float w = scale * info.header.width;
    return w;
}
...
int16_t UIFontVector::GetWidth(uint32_t unicode, uint8_t fontId)
{
    ...
    if (error != RET_VALUE_OK) {
        return GetEmojiWidth(unicode, fontId);
    }
...
uint8_t* UIFontVector::GetBitmap(uint32_t unicode, GlyphNode& glyphNode, uint8_t fontId)
{
    ...
    bitmap =  bitmapCache_->GetBitmap(key_, unicode);
    if (bitmap != nullptr) {
        return bitmap + sizeof(Metric);
    }
    return nullptr;
...
void UIFontVector::SetFace(FT_Face ftFace, uint32_t unicode) const
{
...
#if CAPABILITY_HWDRAW_ALPHA_BITMAP
    int alignWidth = ALIGN_BYTE(f.cols , BYTE_ALIGNMENT);
    uint32_t bitmapSize = alignWidth * ftFace->glyph->bitmap.rows;
#else
    ...
    if (bitmap != nullptr) {
        ...
        for (int i = 0; i < f.rows; i++) {
            uint8_t* rowStart = bitmap + i * alignWidth;
            memcpy_s(rowStart, f.cols, ftFace->glyph->bitmap.buffer + i * f.cols, f.cols);
            memset_s(rowStart + f.cols, alignWidth - f.cols, 0, alignWidth - f.cols);
        }
#else
