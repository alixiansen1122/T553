...
UIFontCache::UIFontCache(uint8_t* ram, uint32_t size)
{
    ...
    AllocInfo allocInfo;
    allocInfo.expectedSize = hashTableSize;
    allocInfo.usage = HBM_USE_ASSIGN_SIZE | HBM_USE_MEM_FONT;
    GrallocBuffer buffer;
    if (!GrallocEngines::GetInstance()->AllocBuffer(allocInfo, buffer)) {
        GRAPHIC_LOGE("UIFontCache AllocBuffer failed.");
        return;
    }
    hashTable_ = reinterpret_cast<ListHead*>(buffer.virAddr);
#else
    if (ram == nullptr) {
        return;
    }
...
#if CAPABILITY_HWDRAW_ALPHA_BITMAP
        AllocInfo allocInfo;
        allocInfo.expectedSize = allocSize;
        allocInfo.usage = HBM_USE_ASSIGN_SIZE | HBM_USE_MEM_FONT;
        GrallocBuffer buffer;
        if (!GrallocEngines::GetInstance()->AllocBuffer(allocInfo, buffer)) {
            GRAPHIC_LOGE("GetSpace AllocBuffer failed.");
            return nullptr;
        }
        bitmap = reinterpret_cast<Bitmap*>(buffer.virAddr);
#else
...
#if CAPABILITY_HWDRAW_ALPHA_BITMAP
            GrallocEngines::GetInstance()->FreeBuffer(reinterpret_cast<uint8_t*>(toFree));
#else
...
#if CAPABILITY_HWDRAW_ALPHA_BITMAP
    GrallocEngines::GetInstance()->FreeBuffer(reinterpret_cast<uint8_t*>(bitmap));
#else
