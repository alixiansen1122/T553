                          ...
                          OpacityType opa) const
{
    ...
    screenBuffer += y * gfxDstBuffer.stride + x * bufferPxSize;
    COLOR_FILL_BLEND(screenBuffer, bufferMode, &fillColor, ARGB8888, opa);
}

#if CAPABILITY_HWDRAW_LETTER_VECTOR
bool DrawUtils::DrawLetterVectorPath(BufferInfo& gfxDstBuffer, const LabelLetterInfo& letterInfo) const
{
    UIFont* fontEngine = UIFont::GetInstance();
    GlyphPathData path;
    if (!fontEngine->GetGlyphPathData(letterInfo.letter, letterInfo.shapingId, path)) {
        GRAPHIC_LOGD("GetGlyphPathData failed\n");
        return false;
    }

    if ((path.width == 0) || (path.cmdsNum == 0)) {
        return true;
    }
    ...
    uint16_t letterW = path.width;
    uint16_t letterH = path.height;

    int16_t posX = letterInfo.pos.x + path.left + letterInfo.offsetX;
    int16_t originX = letterInfo.pos.x + letterInfo.offsetX;
    int16_t posY = letterInfo.pos.y + head.ascender - path.top - letterInfo.offsetY;
    int16_t originY = letterInfo.pos.y + head.ascender - letterInfo.offsetY;

    if (letterInfo.direct == TEXT_DIRECT_RTL) {
        originX = originX - path.advance;
        posX = posX - path.advance;
    }

    if (letterInfo.transMap != nullptr) {
        if (!letterInfo.transMap->GetBoxRect().IsIntersect(letterInfo.mask)) {
            return true;
        }
    } else if ((posX + letterW < letterInfo.mask.GetLeft()) || (posX > letterInfo.mask.GetRight()) ||
        (posY + letterH < letterInfo.mask.GetTop()) || (posY > letterInfo.mask.GetBottom())) {
        return true;
    }

    VGUMatrix3 matrix;
    if (memset_s(&matrix, sizeof(VGUMatrix3), 0, sizeof(VGUMatrix3)) != EOK) {
        GRAPHIC_LOGE("DrawLetter memset_s error\n");
    }
    if (letterInfo.transMap != nullptr) {
        Matrix4<float> m1 = letterInfo.transMap->GetOrigTransformMatrix();
        Matrix4<float> m2;
        m2[0][0] = path.scale;
        m2[1][1] = path.scale;
        m2[3][0] = originX;  // 3: index
        m2[3][1] = originY;  // 3: index
        m2 = m1 * m2;
        const float* srcMat = m2.GetData();

        matrix.m[0][0] = srcMat[0];
        matrix.m[1][0] = srcMat[1];
        matrix.m[2][0] = srcMat[3]; // 2, 3: index
        matrix.m[0][1] = srcMat[4]; // 4: index
        matrix.m[1][1] = srcMat[5]; // 5: index
        matrix.m[2][1] = srcMat[7]; // 2, 7: index
        matrix.m[0][2] = srcMat[12]; // 2, 12: index
        matrix.m[1][2] = srcMat[13]; // 2, 13: index
        matrix.m[2][2] = 1; // 2: index
    } else {
        matrix.m[0][0] = path.scale;
        matrix.m[1][1] = path.scale;
        matrix.m[0][2] = originX;  // 2: index
        matrix.m[1][2] = originY;  // 2: index
    }

    uintptr_t vguPath = LiteMGfxEngine::GetInstance()->InitPath(true);
    if (LiteMGfxEngine::GetInstance()->SetPathData(vguPath, path.cmdsNum, path.cmds, path.pointsNum, path.points)) {
        LiteMGfxEngine::GetInstance()->FillPath(gfxDstBuffer, vguPath, letterInfo.mask, letterInfo.color,
            letterInfo.opa, &matrix);
        LiteMGfxEngine::GetInstance()->DeinitPath(vguPath);
        return true;
    }
    return true;
}

void Convert2BufferInfo(const ImageInfo &info, const Rect &inputRect, BufferInfo &bufferInfo)
{
    bufferInfo.rect = inputRect;
    bufferInfo.virAddr = static_cast<void *>(const_cast<uint8_t *>(info.data));
#if ENABLE_GFX_ENGINES || defined(VERSION_IOT)
    bufferInfo.phyAddr = bufferInfo.virAddr;
#endif
    bufferInfo.stride = static_cast<int32_t>(
        ImageLoad::GetAlignedStride(info.header.width, info.header.compressMode, info.header.colorMode));
    bufferInfo.mode = static_cast<ColorMode>(info.header.colorMode);
    bufferInfo.width = info.header.width;
    bufferInfo.height = info.header.height;
#ifdef VERSION_IOT
    bufferInfo.compressMode = info.header.compressMode;
#endif
    bufferInfo.color = 0;
}

static void DrawEmoji(BufferInfo &gfxDstBuffer, const ImageInfo &info, const LabelLetterInfo &letterInfo)
{
    int16_t posX = letterInfo.pos.x + letterInfo.offsetX;
    int16_t posY = letterInfo.pos.y + letterInfo.offsetY;
    UIFont* fontEngine = UIFont::GetInstance();
    if (fontEngine == nullptr) {
        return;
    }
    int16_t letterW = fontEngine->GetWidth(letterInfo.letter, letterInfo.shapingId);
    int16_t letterH = fontEngine->GetHeight();
    if (letterInfo.direct == TEXT_DIRECT_RTL) {
        posX = posX - letterW;
    }
    BufferInfo src;
    memset_s(&src, sizeof(src), 0, sizeof(src));
    Convert2BufferInfo(info, Rect(0, 0, info.header.width - 1, info.header.height - 1), src);
    BlendOption blendOption;
    blendOption.opacity = letterInfo.opa;
    blendOption.transMap.SetTransMapRect(src.rect);
    float scaleX = static_cast<float>(letterW) / info.header.width;
    float scaleY = static_cast<float>(letterH) / info.header.height;
    blendOption.transMap.Scale(Vector2<float>{scaleX, scaleY}, Vector2<float>{0, 0});
    blendOption.transMap.Translate(Vector2<int16_t>(posX, posY));
    Rect mask = letterInfo.mask;
    if (letterInfo.transMap != nullptr) {
        Matrix4<float> m1 = letterInfo.transMap->GetOrigTransformMatrix();
        Matrix4<float> m0 = blendOption.transMap.GetTransformMatrix();
        blendOption.transMap.SetMatrix(m1 * m0);
    }
    if (!mask.Intersect(mask, blendOption.transMap.GetBoxRect())) {
        return;
    }
    Matrix4<float> matrix = blendOption.transMap.GetTransformMatrix();
    if (!(FloatEqual(matrix[0][0], 1) && FloatEqual(matrix[0][1], 0) && FloatEqual(matrix[0][3], 0) &&
        FloatEqual(matrix[1][0], 0) && FloatEqual(matrix[1][1], 1) && FloatEqual(matrix[1][3], 0) && // 3: pos
        FloatEqual(matrix[3][3], 1))) { // 3: pos, to avoid twinkle
        mask.SetLeft(mask.GetLeft() + 1);
        mask.SetRight(mask.GetRight() - 1);
        mask.SetTop(mask.GetTop() + 1);
        mask.SetBottom(mask.GetBottom() - 1);
    } else { // to avoid vau error (srcRect is not euqal to dstRect) when no scale
        int16_t srcLeft = mask.GetLeft() - matrix[3][0];
        int16_t srcTop = mask.GetTop() - matrix[3][1];
        src.rect = {srcLeft, srcTop, srcLeft + mask.GetWidth() - 1, srcTop + mask.GetHeight() - 1};
    }
    ...
    BaseGfxEngine::GetInstance()->Blit(gfxDstBuffer, {0, 0}, src, mask, blendOption);
}

static void DrawEmojiRotate(BufferInfo &gfxDstBuffer, const LabelLetterInfo &letterInfo, float rotateAngle)
{
    UIFont* fontEngine = UIFont::GetInstance();
    ImageInfo info;
    if (!fontEngine->GetEmoji(letterInfo.letter, info)) {
        return;
    }
    BufferInfo src;
    Convert2BufferInfo(info, Rect(0, 0, info.header.width - 1, info.header.height - 1), src);
    ...
    float x = letterInfo.x;
    float y = letterInfo.y;
    Matrix4<float> mat;
    Matrix4<float> rotate = Matrix4<float>::Rotate(rotateAngle,
            Vector3<float>(0, 0, 0), Vector3<float>(0, 0, 1));
    float scaleX = static_cast<float>(letterW) / info.header.width;
    float scaleY = static_cast<float>(letterH) / info.header.height;
    Matrix4<float> scale = Matrix4<float>::Scale(Vector3<float>(scaleX, scaleY, 0), Vector3<float>(0, 0, 0));
    Matrix4<float> translateToCamera = Matrix4<float>::Translate(Vector3<float>(x, y, 0));
    mat = translateToCamera * rotate * scale;

    BlendOption blendOption;
    blendOption.opacity = letterInfo.opa;
    blendOption.transMap.SetMatrix(mat);
    blendOption.mode = BLEND_SRC_OVER;
    BaseGfxEngine::GetInstance()->Blit(gfxDstBuffer, {0, 0}, src, letterInfo.mask, blendOption);
}

void DrawUtils::DrawArcLablePath(BufferInfo& gfxDstBuffer, const LabelLetterInfo& letterInfo,
    const float& rotateAngle) const
{
    UIFont* fontEngine = UIFont::GetInstance();
    GlyphPathData path;
    if (!fontEngine->GetGlyphPathData(letterInfo.letter, letterInfo.shapingId, path)) {
        DrawEmojiRotate(gfxDstBuffer, letterInfo, rotateAngle);
        return;
    }
    ...
    float x = letterInfo.x;
    float y = letterInfo.y + head.ascender;

    Matrix4<float> mat;
    Matrix4<float> rotate = Matrix4<float>::Rotate(rotateAngle,
            Vector3<float>(x, y - head.ascender, 0), Vector3<float>(x, y - head.ascender, 1));
    Matrix4<float> scale = Matrix4<float>::Scale(Vector3<float>(path.scale, path.scale, 1), Vector3<float>(x, y, 0));
    mat = rotate * scale;
    Matrix4<float> translateToCamera = Matrix4<float>::Translate(Vector3<float>(x, y, 0));
    mat = mat * translateToCamera;

    const float* srcMat = mat.GetData();
    VGUMatrix3 matrix;
    if (memset_s(&matrix, sizeof(VGUMatrix3), 0, sizeof(VGUMatrix3)) != EOK) {
        GRAPHIC_LOGE("DrawLetter memset_s error\n");
    }
...
{
#if CAPABILITY_HWDRAW_LETTER_VECTOR
    if (DrawLetterVectorPath(gfxDstBuffer, letterInfo)) {
        return;
    }
#elif defined(CAPABILITY_SWDRAW_LETTER_BITMAP) && CAPABILITY_SWDRAW_LETTER_BITMAP
    return DrawLetterBitmap(gfxDstBuffer, letterInfo);
#endif
    UIFont* fontEngine = UIFont::GetInstance();
    ...
    if (fontMap == nullptr) {
#if CAPABILITY_HW_SUPPORT_EMOJI
        ImageInfo info;
        if (fontEngine->GetEmoji(letterInfo.letter, info)) {
            DrawEmoji(gfxDstBuffer, info, letterInfo);
        }
#endif
        return;
    }
                           ...
                           const OpacityType opa) const
{
    ...
    DRAW_UTILS_PREPROCESS(gfxDstBuffer, opa);
    screenBuffer += subRect.GetY() * gfxDstBuffer.stride + subRect.GetX() * bufferPxSize;
    ...
    for (int16_t i = rowStart; i < rowEnd; i++) {
        ...
        screenBuffer += gfxDstBuffer.stride - (colEnd - colStart) * bufferPxSize;
    }
}
                          ...
                          const uint8_t* imgPhyAddr) const
{
    ...
    LiteMGfxEngine::GetInstance()->Convert2VGUFormat(colorMode, colorFmt);
    ...
    bool isAlpha = ((colorMode == A1) ||  (colorMode == A2) || (colorMode == A4)  || (colorMode == A8));
    if (isAlpha) {
#if CAPABILITY_HWDRAW_ALPHA_BITMAP
        imageWidthInByte = DisplayDev::GetInstance()->CalcStride(imageWidthInByte, colorFmt, cmpMode);
#endif
    } else {
#if defined(VERSION_IOT) && ENABLE_VGU_ENGINE
        imageWidthInByte = DisplayDev::GetInstance()->CalcStride(mapWidth, colorFmt, cmpMode);
#endif
    }
    ...
    src.mode = colorMode;
    if (isAlpha) {
        src.color = gfxDstBuffer.color;
        gfxDstBuffer.color = 0;
    } else {
        src.color = 0;
    }
#ifdef VERSION_IOT
...
#ifdef VERSION_IOT
    blendOption.transMap.Translate(Vector2<int16_t>(dstPos.x, dstPos.y));
    blendOption.mode = BLEND_SRC_OVER;
#endif
                                     ...
                                     const OpacityType& opa) const
{
    ...
    int32_t halBufferDeltaByteLen = gfxDstBuffer.stride;
    int16_t width = fillArea.GetWidth();
    int16_t height = fillArea.GetHeight();
    uint8_t* dest = static_cast<uint8_t*>(gfxDstBuffer.virAddr);
    dest += fillArea.GetTop() * gfxDstBuffer.stride + fillArea.GetLeft() * destByteSize;
#ifdef VERSION_IOT
    int32_t dstMaxSize = gfxDstBuffer.stride * gfxDstBuffer.height -
        (fillArea.GetTop() * gfxDstBuffer.stride + fillArea.GetLeft() * destByteSize);
#else
    ...
    if ((opa == OPA_OPAQUE) && (fillColor.alpha == OPA_OPAQUE)) {
        uint8_t midRow = 0;
        if ((fillArea.GetTop() <= (gfxDstBuffer.height / 2)) && (fillArea.GetBottom() >= (gfxDstBuffer.height / 2))) {
            midRow = gfxDstBuffer.height / 2 - fillArea.GetTop();
        } else if (fillArea.GetTop() <= (gfxDstBuffer.height / 2)) {
            midRow = height - 1;
        }
        uint8_t* midRowDst = dest + midRow * halBufferDeltaByteLen;
        for (int16_t col = 0; col < width; ++col) {
            dstTmp = midRowDst + (col * destByteSize);
            COLOR_FILL_COVER(dstTmp, mode, fillColor.red, fillColor.green, fillColor.blue, ARGB8888);
        }
        uint8_t* memStart = midRowDst;
...
void DrawUtils::BlendAlphaWithSoftWare(const uint8_t* src,
                                       const Rect& srcRect,
                                       uint32_t srcStride,
                                       ColorMode srcMode,
                                       uint32_t color,
                                       ...
                                       uint8_t* dest,
                                       uint32_t destStride,
                                       ColorMode destMode) const
{
    uint8_t srcPxBitSize = GetPxSizeByColorMode(srcMode);
    uint32_t firstPxBitOffset = (srcStride << SHIFT_3) * srcRect.GetY() +  srcPxBitSize * srcRect.GetX();
    uint32_t firstPxByteOffset = firstPxBitOffset >> SHIFT_3;
    uint8_t bitOffset = firstPxBitOffset & 0x7;
    src += firstPxByteOffset;

    uint32_t width = srcRect.GetWidth();
    uint32_t height = srcRect.GetHeight();
    uint8_t destByteSize = GetByteSizeByColorMode(destMode);
    uint8_t bitMask = (1 << srcPxBitSize) - 1;
    uint16_t opaScale = OPA_OPAQUE / bitMask;

    for (uint32_t row = 0; row < height; ++row) {
        uint8_t* destTmp = dest;
        uint32_t bitOffsetTemp = bitOffset;
        uint8_t* srcLine = const_cast<uint8_t*>(src);
        for (uint32_t col = 0; col < width; ++col) {
            uint8_t* srcTmp = srcLine + (bitOffsetTemp >> SHIFT_3);
            uint16_t alp = ((*srcTmp >> (bitOffsetTemp & 0x7)) & bitMask) * opaScale; // & 0x7 means mod 8
            alp = alp * opa / OPA_OPAQUE;
            COLOR_FILL_BLEND(destTmp, destMode, &color, ARGB8888, alp);
            destTmp += destByteSize;
            bitOffsetTemp += srcPxBitSize;
        }
        dest += destStride;
        src += srcStride;
    }
                                  ...
                                  uint32_t y) const
{
    ...
    if ((srcMode == A1) || (srcMode == A2) || (srcMode == A4) || (srcMode == A8)) {
        BlendAlphaWithSoftWare(src1, srcRect, srcStride, srcMode, color, opa, dest, destStride, destMode);
        return;
    }
...
static void DrawPixelIn3DTriangleBilinear888(uint8_t* screenBuffer, const TriangleScanInfo& in,
    uint32_t px, int64_t u, int64_t v, const ColorMode bufferMode)
{
    uint8_t* imgHead = const_cast<uint8_t*>(in.info.data);
    const Color24 p1 = *(reinterpret_cast<Color24*>(&imgHead[px]));
    const Color24 p2 = *(reinterpret_cast<Color24*>(&imgHead[px + in.pixelSize]));
    const Color24 p3 = *(reinterpret_cast<Color24*>(&imgHead[px + in.srcLineWidth]));
    const Color24 p4 = *(reinterpret_cast<Color24*>(&imgHead[px + in.srcLineWidth + in.pixelSize]));
#if ENABLE_FIXED_POINT
    int64_t decU = FO_DECIMAL(u);
    int64_t decV = FO_DECIMAL(v);
    int64_t decUMinus1 = FIXED_NUM_1 - decU;
...
static void DrawPixelIn3DTriangleBilinear565(uint8_t* screenBuffer, const TriangleScanInfo& in,
    uint32_t px, int64_t u, int64_t v, const ColorMode bufferMode)
{
    uint8_t* imgHead = const_cast<uint8_t*>(in.info.data);
    const Color16 p1 = *(reinterpret_cast<Color16*>(&imgHead[px]));
    const Color16 p2 = *(reinterpret_cast<Color16*>(&imgHead[px + in.pixelSize]));
    const Color16 p3 = *(reinterpret_cast<Color16*>(&imgHead[px + in.srcLineWidth]));
    const Color16 p4 = *(reinterpret_cast<Color16*>(&imgHead[px + in.srcLineWidth + in.pixelSize]));
#if ENABLE_FIXED_POINT
    int64_t decU = FO_DECIMAL(u);
    int64_t decV = FO_DECIMAL(v);
    int64_t decUMinus1 = FIXED_NUM_1 - decU;
...
static void DrawPixelIn3DTriangleBilinear8888(uint8_t* screenBuffer, const TriangleScanInfo& in,
    uint32_t px, int64_t u, int64_t v, const ColorMode bufferMode)
{
#if COLOR_DEPTH == 32
    uint8_t* imgHead = const_cast<uint8_t*>(in.info.data);
    const Color32 p1 = *(reinterpret_cast<Color32*>(&imgHead[px]));
    const Color32 p2 = *(reinterpret_cast<Color32*>(&imgHead[px + in.pixelSize]));
    const Color32 p3 = *(reinterpret_cast<Color32*>(&imgHead[px + in.srcLineWidth]));
    const Color32 p4 = *(reinterpret_cast<Color32*>(&imgHead[px + in.srcLineWidth + in.pixelSize]));
#if ENABLE_FIXED_POINT
    int64_t decU = FO_DECIMAL(u);
    int64_t decV = FO_DECIMAL(v);
    int64_t decUMinus1 = FIXED_NUM_1 - decU;
...
void DrawUtils::Draw3DTriangleTrueColorBilinear(const TriangleScanInfo& in, const ColorMode bufferMode)
{
    int16_t maskLeft = in.mask.GetLeft();
    int16_t maskRight = in.mask.GetRight();
    int16_t xMinErr = 0;
        ...
        for (int16_t x = xMin; x <= xMax; x++) {
            ...
            if ((u >= 0) && (intU < in.info.header.width - 1) && (v >= 0) && (intV < in.info.header.height - 1)) {
                ...
                switch (in.info.header.colorMode) {
                    case RGB888: {
                        DrawPixelIn3DTriangleBilinear888(screenBuffer, in, px1, u, v, bufferMode);
                        break;
                    }
                    case RGB565: {
                        DrawPixelIn3DTriangleBilinear565(screenBuffer, in, px1, u, v, bufferMode);
                        break;
                    }
                    case ARGB8888: {
                        DrawPixelIn3DTriangleBilinear8888(screenBuffer, in, px1, u, v, bufferMode);
                        break;
                    }
                    default:
                        return;
...
// parameters below are Q15 fixed-point number
    ...
    if (isTrueColor) {
        ...
        } else if (part.transMap.Is3DTransform()) {
            fuc = Draw3DTriangleTrueColorBilinear;
...
#ifdef VERSION_IOT
    const int32_t srcLineWidth = (int32_t)ImageLoad::GetAlignedStride(
        part.info.header.width, COMPRESS_MODE_NONE, part.info.header.colorMode);
    uint32_t screenBufferWidth = (uint32_t)gfxDstBuffer.stride;
#else
    const int32_t srcLineWidth = part.info.header.width * pixelSize;
    uint32_t screenBufferWidth = gfxDstBuffer.width;
#endif
...
void DrawUtils::AddBorderToImageData(TransformDataInfo& newDataInfo)
{
    ...
    uint16_t stride = 0;
    uint16_t oldWidthInByte = width * newDataInfo.pxSize / FONT_WEIGHT_8;
    uint16_t oldStride = oldWidthInByte;
    int16_t diff = 0;
...
#ifdef VERSION_IOT
        stride = ALIGN_BYTE(width, BYTE_ALIGNMENT) * newDataInfo.pxSize / FONT_WEIGHT_8;
        oldStride = ALIGN_BYTE(newDataInfo.header.width, BYTE_ALIGNMENT) * newDataInfo.pxSize / FONT_WEIGHT_8;
#endif
    } else {
        width += offset * FONT_WEIGHT_8 / newDataInfo.pxSize;
        height += offset;
        diff = border;
        stride = width * newDataInfo.pxSize / FONT_WEIGHT_8;
        if ((width * newDataInfo.pxSize) % FONT_WEIGHT_8 != 0) {
            stride++;
        }
        if ((newDataInfo.header.width * newDataInfo.pxSize) % FONT_WEIGHT_8 != 0) {
            oldWidthInByte++;
            oldStride++;
        }
    }

    uint8_t* newData = static_cast<uint8_t*>(UIMalloc(stride * height));
    if (newData == nullptr) {
        GRAPHIC_LOGE("newData == nullptr");
        return;
    }
    if (memset_s(newData, stride * height, 0, stride * height) != EOK) {
        GRAPHIC_LOGE("memset failed.");
    ...
    tmp += stride * border + diff;
    for (int i = 0; i < newDataInfo.header.height; ++i) {
        if (memcpy_s(tmp, oldWidthInByte, data, oldWidthInByte) != EOK) {
            goto ERROR;
        }
        tmp += stride;
        data += oldStride;
...
#if COLOR_DEPTH == 32
                COLOR_FILL_COVER(mem[col * bufferPxSize], bufferMode, colorBuf[row * gfxDstBuffer.width + col].red,
                                 colorBuf[row * gfxDstBuffer.width + col].green,
                                 colorBuf[row * gfxDstBuffer.width + col].blue, ARGB8888);
#else
                COLOR_FILL_COVER(mem[col * bufferPxSize], bufferMode, colorBuf[row * gfxDstBuffer.width + col].red,
                                 colorBuf[row * gfxDstBuffer.width + col].green,
                                 colorBuf[row * gfxDstBuffer.width + col].blue, RGB565);
#endif
            }
            mem += gfxDstBuffer.stride;
        }
                                   ...
                                   uint16_t weight) const
{
    ...
    if ((x1 >= mask.GetLeft()) && (x1 <= mask.GetRight()) && (y1 >= mask.GetTop()) && (y1 <= mask.GetBottom())) {
        screenBuffer += y1 * gfxDstBuffer.stride + x1 * bufferPxSize;
    ...
    if ((x2 >= mask.GetLeft()) && (x2 <= mask.GetRight()) && (y2 >= mask.GetTop()) && (y2 <= mask.GetBottom())) {
        ...
        screenBuffer += y2 * gfxDstBuffer.stride + x2 * bufferPxSize;
                                ...
                                uint16_t weight) const
{
    ...
    result.full = Color::ColorTo32(color);
    if ((x >= mask.GetLeft()) && (x <= mask.GetRight()) && (y >= mask.GetTop()) && (y <= mask.GetBottom())) {
        screenBuffer += y * gfxDstBuffer.stride + (uint16_t)x * bufferPxSize;
        OpacityType fillOpa = weight * opa / OPA_OPAQUE;
        COLOR_FILL_BLEND(screenBuffer, bufferMode, &result, ARGB8888, fillOpa);
    }
}
                                   ...
                                   uint16_t weight) const
{
    ...
    if ((x0 >= mask.GetLeft()) && (x0 <= mask.GetRight())) {
        screenBuffer += y * gfxDstBuffer.stride + x0 * bufferPxSize;
        OpacityType fillOpa = weight * opa / OPA_OPAQUE;
        COLOR_FILL_BLEND(screenBuffer, bufferMode, &result, ARGB8888, fillOpa);
    }
    if ((x >= mask.GetLeft()) && (x <= mask.GetRight())) {
        screenBuffer = static_cast<uint8_t*>(gfxDstBuffer.virAddr);
        screenBuffer += y * (int32_t)gfxDstBuffer.stride + x * bufferPxSize;
        if (opa == OPA_OPAQUE) {
            COLOR_FILL_COVER(screenBuffer, bufferMode, result.red, result.green, result.blue, ARGB8888);
        } else {
    ...
    if ((x1 >= mask.GetLeft()) && (x1 <= mask.GetRight())) {
        screenBuffer = static_cast<uint8_t*>(gfxDstBuffer.virAddr);
        screenBuffer += y * gfxDstBuffer.stride + x1 * bufferPxSize;
        OpacityType fillOpa = (weight ^ OPA_OPAQUE) * opa / OPA_OPAQUE;
        COLOR_FILL_BLEND(screenBuffer, bufferMode, &result, ARGB8888, fillOpa);
    }
}
                                   ...
                                   uint16_t weight) const
{
    ...
    if ((y0 >= mask.GetTop()) && (y0 <= mask.GetBottom())) {
        screenBuffer += y0 * gfxDstBuffer.stride + x * bufferPxSize;
        OpacityType fillOpa = weight * opa / OPA_OPAQUE;
        COLOR_FILL_BLEND(screenBuffer, bufferMode, &result, ARGB8888, fillOpa);
    }
    if ((y >= mask.GetTop()) && (y <= mask.GetBottom())) {
    ...
    if ((y1 >= mask.GetTop()) && (y1 <= mask.GetBottom())) {
        screenBuffer = static_cast<uint8_t*>(gfxDstBuffer.virAddr);
        screenBuffer += y1 * gfxDstBuffer.stride + x * bufferPxSize;
        OpacityType fillOpa = (weight ^ OPA_OPAQUE) * opa / OPA_OPAQUE;
        COLOR_FILL_BLEND(screenBuffer, bufferMode, &result, ARGB8888, fillOpa);
    }
}
...
void DrawUtils::DrawLetterBitmap(BufferInfo& gfxDstBuffer, const LabelLetterInfo& letterInfo) const
{
    GlyphNode node;
    uint8_t* fontMap = UIFont::GetInstance()->GetBitmap(letterInfo.letter, node, letterInfo.shapingId);
    if (fontMap == nullptr) {
        return;
    }

    int16_t posX;
    int16_t posY = letterInfo.pos.y + letterInfo.offsetY + node.top;
    if (letterInfo.direct == TEXT_DIRECT_RTL) {
        /* RTL */
        posX = letterInfo.pos.x - node.advance + node.left + letterInfo.offsetX;
    } else {
    ...
    if ((posX + node.cols < letterInfo.mask.GetLeft()) || (posX > letterInfo.mask.GetRight()) ||
        (posY + node.rows < letterInfo.mask.GetTop()) || (posY > letterInfo.mask.GetBottom())) {
        return;
    }

    uint16_t rowStart = (posY >= letterInfo.mask.GetTop()) ? 0 : (letterInfo.mask.GetTop() - posY);
    uint16_t rowEnd =
        (posY + node.rows <= letterInfo.mask.GetBottom()) ? node.rows : (letterInfo.mask.GetBottom() - posY + 1);
    uint16_t colStart = (posX >= letterInfo.mask.GetLeft()) ? 0 : (letterInfo.mask.GetLeft() - posX);
    uint16_t colEnd =
        (posX + node.cols <= letterInfo.mask.GetRight()) ? node.cols : (letterInfo.mask.GetRight() - posX + 1);

    Color32 fillColor;
    fillColor.full = Color::ColorTo32(letterInfo.color);

    DRAW_UTILS_PREPROCESS(gfxDstBuffer, letterInfo.opa);
    screenBuffer += (posY + rowStart) * gfxDstBuffer.stride + (posX + colStart) * bufferPxSize;
    uint8_t* tempFontByte = fontMap + node.dataOff;
    for (int16_t i = rowStart; i < rowEnd; i++) {
        int16_t col = colStart;
        while (col < colEnd) {
            uint8_t validOpacity = *tempFontByte;
            if (validOpacity != 0) {
                if (letterInfo.opa != OPA_OPAQUE) {
                    validOpacity = static_cast<OpacityType>(static_cast<uint16_t>(validOpacity) * letterInfo.opa);
                }
                COLOR_FILL_BLEND(screenBuffer, bufferMode, &fillColor, ARGB8888, validOpacity);
            }
            screenBuffer += bufferPxSize;
        ...
        tempFontByte = fontMap + node.dataOff + (node.reserve * (i + 1));
        screenBuffer += gfxDstBuffer.stride - (colEnd - colStart) * bufferPxSize;
    }
    return;
}
