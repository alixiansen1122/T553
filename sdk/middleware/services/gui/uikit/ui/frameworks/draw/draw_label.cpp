                            ...
                            OpacityType opaScale)
{
    ...
    float posY;
    float rotateAngle;
    float spaceAngle = TypedText::GetAngleForArcLen(static_cast<float>(style.letterSpace_), letterHeight,
    ...
    while (i < arcTextInfo.lineEnd) {
        ...
        float letterAngle = TypedText::GetAngleForArcLen(static_cast<float>(letterWidth), letterHeight,
                                                            arcTextInfo.radius, arcTextInfo.direct, orientation);
        if (xorFlag) {
            angle += letterAngle;
        }
        ...
        // 2: half
        float fineTuningAngle = letterAngle / 2;
        ...
        if (xorFlag) {
            ...
            angle += letterAngle + spaceAngle;
        }

#if CAPABILITY_HWDRAW_LETTER_VECTOR
        LabelLetterInfo letterInfo{Point { MATH_ROUND(posX), MATH_ROUND(posY) }, mask, style.textColor_, opaScale,
            0, 0, letter, arcTextInfo.direct, fontId, 0, 0, nullptr, posX, posY };
        DrawUtils::GetInstance()->DrawArcLablePath(gfxDstBuffer, letterInfo, rotateAngle);
#else
