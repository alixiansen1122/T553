...
{
    if (name == nullptr) {
        if (UIFont::GetInstance()->IsVectorFont() && (fontSize_ != size)) {
            fontSize_ = size;
            needRefresh_ = true;
        } else {
#if defined(CAPABILITY_SWDRAW_LETTER_BITMAP) && CAPABILITY_SWDRAW_LETTER_BITMAP
            fontSize_ = size;
            fontId_ = UIFont::GetInstance()->GetFontId(nullptr, size);
            needRefresh_ = true;
#endif
        }
        return;
...
{
#if defined(CAPABILITY_SWDRAW_LETTER_BITMAP) && CAPABILITY_SWDRAW_LETTER_BITMAP
    uint8_t size = UIFont::GetInstance()->SetCurrentFontId(fontId);
    if (size >= 0) {
        fontSize_ = size;
        fontId_ = fontId;
        needRefresh_ = true;
                  ...
                  OpacityType opaScale)
{
    ...
    if (transMap_ == nullptr) {
        mask.Intersect(mask, textRect);
    }

    if (mask.IsValid()) {
        Draw(gfxDstBuffer, mask, textRect, style, offsetX, tailIndex, opaScale);
    }
}
                ...
                OpacityType opaScale)
{
    ...
    uint32_t maxLineBytes = 0;
    uint16_t lineCount = GetLine(lineMaxWidth, style.letterSpace_, tailIndex, maxLineBytes);
    Point pos;
    ...
    bool isTransValid = false;
    if (transMap_ != nullptr && !transMap_->IsInvalid())  {
        isTransValid = true;
    }
    for (int i = 0; i < lineCount; i++) {
        if (!isTransValid && pos.y > mask.GetBottom()) {
            return;
        }
        int16_t nextLine = pos.y + lineHeight;
        if (isTransValid || nextLine >= mask.GetTop()) {
            ...
            if ((i == (lineCount - 1)) && (tailIndex != TEXT_ELLIPSIS_END_INV) && !isTruncateMode_) {
...
uint16_t Text::GetLine(int16_t width, uint8_t letterSpace, uint16_t tailIndex, uint32_t& maxLineBytes)
{
    if (text_ == nullptr) {
        return 0;
    }
    ...
    if ((tailIndex != TEXT_ELLIPSIS_END_INV) && (tailIndex < textLen)) {
        textLen = tailIndex;
    }
    uint32_t begin = 0;
    ...
    if ((lineNum != 0) && (tailIndex != TEXT_ELLIPSIS_END_INV)  && !isTruncateMode_) {
...
uint16_t Text::GetTruncateIndex(const Rect& textRect, const Style& style)
{
    if ((textSize_.y <= textRect.GetHeight()) || (TypedText::GetUTF8CharacterSize(text_) <= TEXT_ELLIPSIS_DOT_NUM)) {
        return TEXT_TRUNCATE_END_INV;
    }
    UIFont* fontEngine = UIFont::GetInstance();
    fontEngine->SetCurrentFontId(fontId_, fontSize_);
    Point p;
    p.x = textRect.GetWidth();
    p.y = textRect.GetHeight();
    int16_t height = style.lineHeight_;
    if (height == 0) {
        height = fontEngine->GetHeight() + style.lineSpace_;
