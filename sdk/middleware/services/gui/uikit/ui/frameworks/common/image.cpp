...
#if ENABLE_JPEG_AND_PNG || CAPABILITY_HW_JPEG_DECODE
...
#endif
    if (fd < 0) {
        GRAPHIC_LOGE("can't open %s, errno: %d\n", src, errno);
        return IMG_UNKNOWN;
    }
...
#if (ENABLE_JPEG_AND_PNG && !CACHE_JPEG_AND_PNG)
                      ...
                      uint8_t opaScale) const
{
    if (srcType_ == IMG_SRC_VARIABLE) {
        ...
        ImageCacheManager::GetInstance().UpdateImageInfoIfNecessary(*const_cast<ImageInfo*>(imageInfo_));
        if (imageInfo_->data == nullptr) {
            GRAPHIC_LOGE("DrawImage failed. UpdateImageInfoIfNecessary failed.");
            return;
        }
#endif
