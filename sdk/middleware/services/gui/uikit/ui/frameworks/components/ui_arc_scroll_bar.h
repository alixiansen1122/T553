...
public:
    ...
    virtual ~UIArcScrollBar() override {
...
protected:
    ...
    uint8_t side_;
#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
    uint8_t* cmds_[2] = {nullptr, nullptr}; // 2: two cmd buffer for front and back arc.
    float* pathData_[2] = {nullptr, nullptr}; // 2: two data buffer for front and back arc.
    void DrawArcByHWPath(BufferInfo& dst, ArcInfo arc, uint8_t barLayer, const Rect& mask, uint16_t opa);
    void ClearPath();
#endif
