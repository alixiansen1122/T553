...
public:
    ...
    void RefreshBar()
    {
        ...
        } else if ((scrollView_.yScrollBar_ != nullptr) && (scrollView_.yScrollBar_->GetOpacity() == OPA_TRANSPARENT)) {
            animator_.Start();
        } else {
...
int16_t UIAbstractScroll::CalculateElasticDragLen(int16_t maxLen, int16_t currLen, int16_t dragLen)
{
    float factor = pow((float)(maxLen - abs(currLen)) / maxLen, 2);
    return static_cast<int16_t>(factor * dragLen);
}

bool UIAbstractScroll::ElasticAnimator(uint16_t times)
{
    int16_t distanceX = 0;
    int16_t distanceY = 0;

    if (direction_ == HORIZONTAL) {
        if (childrenHead_->GetX() > 0) {
            distanceX = -childrenHead_->GetX();
        } else {
            distanceX = GetWidth() - (childrenTail_->GetX() + childrenTail_->GetWidth());
        }
    } else {
        if (childrenHead_->GetY() > 0) {
            distanceY = -childrenHead_->GetY();
        } else {
            distanceY = GetHeight() - (childrenTail_->GetY() + childrenTail_->GetHeight());
        }
    }
    ...
    easingFunc_ = EasingEquation::ElasticEaseIn;
    animatorCallback_.SetDragStartValue(0, 0);
    animatorCallback_.SetDragEndValue(distanceX, distanceY);
    animatorCallback_.SetDragTimes(times);
...
bool UIAbstractScroll::DragThrowAnimator(Point currentPos,
                                         ...
                                         bool dragBack)
{
    if (!throwDrag_ && (reboundSize_ == 0 && reboundSizeTop_ == 0 && reboundSizeBottom_ == 0 &&
        reboundSizeLeft_ == 0 && reboundSizeRight_ == 0)) {
        return false;
    }
    ...
    if (reboundSize_ != 0 || reboundSizeTop_ != 0 || reboundSizeBottom_ != 0 ||
        reboundSizeLeft_ != 0 || reboundSizeRight_ != 0) {
