...
UIChartPillar::~UIChartPillar()
{
#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
    ClearPath();
    if (isDrawGradient_) {
        UIFree(gradient_.colorStops);
        gradient_.colorStops = nullptr;
        isDrawGradient_ = false;
    }
#endif
}

#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
Path* UIChartPillar::AllocPath(uint32_t cmdNum, uint32_t dataNum)
{
    Path* path = new Path();
    if (path == nullptr) {
        GRAPHIC_LOGE("new path failed.");
        return nullptr;
    }
    path->cmdNum = cmdNum;
    ...
    cmdInfo.expectedSize = sizeof(uint8_t) * cmdNum;
    cmdInfo.usage = HBM_USE_ASSIGN_SIZE | HBM_USE_MEM_PATH;
    if (!GrallocEngines::GetInstance()->AllocBuffer(cmdInfo, cmdBuffer)) {
        GRAPHIC_LOGE("Alloc buffer for cmd buffer failed.");
        delete path;
        return nullptr;
    }

    AllocInfo dataInfo;
    GrallocBuffer dataBuffer;
    dataInfo.expectedSize = sizeof(float) * dataNum;
    dataInfo.usage = HBM_USE_ASSIGN_SIZE | HBM_USE_MEM_PATH;
    if (!GrallocEngines::GetInstance()->AllocBuffer(dataInfo, dataBuffer)) {
        GRAPHIC_LOGE("Alloc buffer for data buffer failed.");
        delete path;
        GrallocEngines::GetInstance()->FreeBuffer(reinterpret_cast<uint8_t*>(cmdBuffer.virAddr));
        return nullptr;
    }

    path->cmds = (uint8_t *)cmdBuffer.virAddr;
    path->data = reinterpret_cast<float*>(dataBuffer.virAddr);

    return path;
}

void UIChartPillar::ClearPath()
{
    if (paths_.IsEmpty()) {
        return;
    }
    ...
    ListNode<Path*>* node = paths_.Begin();
    while (node != paths_.End()) {
        if (node->data_ != nullptr) {
            Path* path = node->data_;
...
bool UIChartPillar::SetGradient(const LinearGradient& linear, const ColorStop* stops, uint16_t stopCount)
{
    if (stops == nullptr || stopCount == 0) {
        return false;
    }

    if (gradient_.colorStops != nullptr) {
        UIFree(gradient_.colorStops);
    }
    gradient_.colorStops = static_cast<VGUColorStop*>(UIMalloc(sizeof(VGUColorStop) * stopCount));
    if (gradient_.colorStops == nullptr) {
        GRAPHIC_LOGE("UIMalloc for stops failed.");
        return false;
    }
    if (memcpy_s(gradient_.colorStops, sizeof(ColorStop) * stopCount, stops, sizeof(ColorStop) * stopCount) != EOK) {
        UIFree(gradient_.colorStops);
        gradient_.colorStops = nullptr;
        return false;
    }
    gradient_.stopCount = stopCount;
    gradient_.type = static_cast<VGUGradientType>(GradientType::GRADIENT_LINEAR);
    gradient_.spread = static_cast<VGUFillSpread>(GradientSpread::SPREAD_PAD);
    gradient_.linear.x1 = linear.startX;
    gradient_.linear.y1 = linear.startY;
    gradient_.linear.x2 = linear.endX;
    gradient_.linear.y2 = linear.endY;
    isDrawGradient_ = true;

    return true;
}

void UIChartPillar::ClearGradient()
{
    if (isDrawGradient_) {
        if (gradient_.colorStops != nullptr) {
            UIFree(gradient_.colorStops);
        }
        (void)memset_s(&gradient_, sizeof(gradient_), 0, sizeof(gradient_));
        isDrawGradient_ = false;
    }
}

void UIChartPillar::HWDrawLine(BufferInfo& gfxDstBuffer, const Rect& mask, Point start, Point end, int16_t width,
    uint32_t color)
{
    Path* path = AllocPath(2, 3);
    if (path == nullptr) {
        GRAPHIC_LOGE("AllocPath failed.");
        return;
    }
    paths_.PushBack(path);
    path->cmds[0] = CMD_MOVE_TO;
    path->cmds[1] = CMD_VLINE_TO;
    path->data[0] = start.x;
    path->data[1] = start.y;
    path->data[2] = end.y;

    VGUStrokeAttr strokeAttr = {static_cast<VGULineCap>(CapType::CAP_NONE),
        static_cast<VGUJointType>(JoinType::JOIN_BEVEL), 0, width};
    VGUPaintStyle strokeStyle = {{0}};
    strokeAttr.style = &strokeStyle;
    if (style_->lineCap_ == CapType::CAP_ROUND) {
        strokeAttr.cap = VGU_LINECAP_ROUND;
        path->data[2] = enableReverse_ ? end.y - capRadius_ : end.y + capRadius_;
    }

    if (isDrawGradient_) {
        strokeStyle.gradient = &gradient_;
    } else {
    ...
    LiteMGfxEngine::GetInstance()->RenderPath(gfxDstBuffer, vguPath, mask, &strokeAttr, nullptr, &mat);
}
#endif
...
void UIChartPillar::RefreshChart()
{
    ...
    int16_t barWidth = static_cast<int16_t>(width - DEFAULT_MARK_PERCENTAGE * (width << 1));
    uint8_t dataSerialIndex = 0;

    Rect rect = GetContentRect();
    for (; iter != list_.End(); iter = iter->next_) {
        UIChartDataSerial* data = iter->data_;
        int16_t xOffset = width * dataSerialIndex + (width >> 1);
        if (data == nullptr) {
            break;
        }
...
void UIChartPillar::DrawDataSerials(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea)
{
    ...
    for (ListNode<UIChartDataSerial*>* iter = list_.Begin(); iter != list_.End(); iter = iter->next_) {
...
#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
        uint32_t mixedColor = data->GetFillColor().full;
        if (!isDrawGradient_) {
            uint8_t alpha = ((mixedColor >> 24) * opa) / OPA_OPAQUE; // 24: alpha shift
            mixedColor = (alpha << 24) | (mixedColor & 0x00FFFFFF); // 24: alpha shift
        }
#endif
        for (uint16_t index = 0; index < data->GetDataCount(); index++) {
            ...
            xStart.x = current.x;
#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
            HWDrawLine(gfxDstBuffer, invalidatedArea, xStart, current, barWidth, mixedColor);
#else
...
UIChartPolyline::~UIChartPolyline()
{
#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
    ClearPath();
#endif
...
void UIChartPolyline::DrawDataSerials(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea)
{
    ...
    for (; iter != list_.End(); iter = iter->next_) {
        ...
        if (data->IsGradient()) {
            ...
            GradientWithVGU(gfxDstBuffer, invalidatedArea, data);
#else
                                   ...
                                   UIChartDataSerial* data)
{
...
#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
    HWDrawPolyLine(gfxDstBuffer, invalidatedArea, style_->lineWidth_,
        data->GetLineColor().full, startIndex, endIndex, data);
#else
    ...
    ArcInfo arcinfo = {{0}};
    arcinfo.imgPos = Point{0, 0};
    arcinfo.radius = ((style_->lineWidth_ + 1) >> 1) + 1;
    arcinfo.startAngle = 0;
    arcinfo.endAngle = CIRCLE_IN_DEGREE;
...
#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
void UIChartPolyline::ClearPath()
{
    if ((cmds_ == nullptr) && (pathData_ == nullptr)) {
        return;
    }
    ...
    if (cmds_ != nullptr) {
        GrallocEngines::GetInstance()->FreeBuffer(reinterpret_cast<uint8_t*>(cmds_));
        cmds_ = nullptr;
    }
    if (pathData_ != nullptr) {
        GrallocEngines::GetInstance()->FreeBuffer(reinterpret_cast<uint8_t*>(pathData_));
        pathData_ = nullptr;
    }
}

bool UIChartPolyline::AllocMemForPath(uint32_t cmdCount, uint32_t dataCount)
{
    ClearPath();

    AllocInfo cmdInfo;
    cmdInfo.expectedSize = sizeof(uint8_t) * cmdCount;
    cmdInfo.usage = HBM_USE_ASSIGN_SIZE | HBM_USE_MEM_PATH;
    GrallocBuffer cmdBuffer;
    if (!GrallocEngines::GetInstance()->AllocBuffer(cmdInfo, cmdBuffer)) {
    ...
    cmds_ = (uint8_t *)cmdBuffer.virAddr;

    AllocInfo dataInfo;
    dataInfo.expectedSize = sizeof(float) * dataCount;
    dataInfo.usage = HBM_USE_ASSIGN_SIZE | HBM_USE_MEM_PATH;
    GrallocBuffer dataBuffer;
    if (!GrallocEngines::GetInstance()->AllocBuffer(dataInfo, dataBuffer)) {
        GRAPHIC_LOGE("Alloc buffer for pathData_ failed.");
        GrallocEngines::GetInstance()->FreeBuffer(reinterpret_cast<uint8_t*>(cmds_));
        cmds_ = nullptr;
        return false;
    }
    pathData_ = reinterpret_cast<float*>(dataBuffer.virAddr);

    return true;
}

void UIChartPolyline::GradientWithVGU(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea, UIChartDataSerial* data)
{
    Point yStart = yAxis_.GetStartPoint();
    yStart.y = enableReverse_ ? (yStart.y + gradientBottom_) : (yStart.y - gradientBottom_);
    int16_t topY = data->GetPeakData();
    yAxis_.TranslateToPixel(topY);

    int16_t startY = enableReverse_ ? yStart.y : topY;
    int16_t endY = enableReverse_ ? topY : yStart.y;
    if ((invalidatedArea.GetBottom() < startY) || (invalidatedArea.GetTop() > endY)) {
        return;
    }

    uint16_t dataCount = data->GetDataCount();
    uint32_t pathDataCount = dataCount * 2 + 2; // 2: multiply by 2 and add 2.
    uint32_t pathCmdCount = dataCount + 3; // 3: add 3.
    if (!AllocMemForPath(pathCmdCount, pathDataCount)) {
        return;
    }

    cmds_[0] = CMD_MOVE_TO;
    cmds_[1] = CMD_VLINE_TO;
    cmds_[dataCount + 1] = CMD_VLINE_TO;
    cmds_[dataCount + 2] = CMD_CLOSE;

    Point temp;
    data->GetPoint(0, temp);
    pathData_[0] = temp.x;
    pathData_[1] = yStart.y;
    pathData_[2] = temp.y;
    pathData_[dataCount * 2 + 1] = yStart.y;
    uint16_t dataIndex = 3;
    for (int i = 1; i < dataCount; i++) {
        cmds_[i + 1] = CMD_LINE_TO;
        data->GetPoint(i, temp);
        pathData_[dataIndex++] = temp.x;
        pathData_[dataIndex++] = temp.y;
    }

    ColorType color = data->GetFillColor();
    int16_t opaScaleTop = enableReverse_ ? (topY - yStart.y - 1) : (yStart.y - topY - 1);
    if (opaScaleTop < 0) {
        return;
    }
    VGUColorStop stops[2] = {{0, Color::ColorTo32(Color::GetColorFromRGBA(
        color.red, color.green, color.blue, color.alpha * mixData_[0] / 255))},
        {1, Color::ColorTo32(Color::GetColorFromRGBA(
        color.red, color.green, color.blue, color.alpha * mixData_[opaScaleTop] / 255))}};

    VGUGradient gradient = {stops, 2};
    gradient.type = VGU_GRADIENT_LINEAR;
    gradient.linear = {yAxis_.GetStartPoint().x, yStart.y, yAxis_.GetStartPoint().x, topY};
    VGUPaintStyle style = {{&gradient}};
    VGUFillAttr fillAttr = {VGU_RULE_WINDING, &style};

    uintptr_t vguPath = LiteMGfxEngine::GetInstance()->InitPath(true);
    LiteMGfxEngine::GetInstance()->SetPathData(vguPath, pathCmdCount, cmds_, pathDataCount, pathData_);
    LiteMGfxEngine::GetInstance()->RenderPath(gfxDstBuffer, vguPath, invalidatedArea, nullptr, &fillAttr, nullptr);
}

void UIChartPolyline::HWDrawPolyLine(BufferInfo& gfxDstBuffer, const Rect& mask, int16_t width,
    uint32_t color, uint16_t startIndex, uint16_t endIndex, UIChartDataSerial* data)
{
    if (endIndex <= startIndex || data == nullptr) {
        return;
    }
    const uint32_t cmdNum = endIndex - startIndex + 1;
    const uint32_t dataNum = cmdNum * 2; // 2: double
    if (!AllocMemForPath(cmdNum, dataNum)) {
        GRAPHIC_LOGE("AllocPath failed.");
        return;
    }
    Point point;
    for (uint32_t i = 0, cnt = 0; i < cmdNum; ++i) {
        cmds_[i] = CMD_LINE_TO;
        data->GetPoint(startIndex + i, point);
        pathData_[cnt++] = point.x;
        pathData_[cnt++] = point.y;
    }
    cmds_[0] = CMD_MOVE_TO;
    VGUStrokeAttr strokeAttr = {static_cast<VGULineCap>(CapType::CAP_NONE),
        static_cast<VGUJointType>(JoinType::JOIN_ROUND), 0, width, nullptr};
    VGUPaintStyle strokeStyle = {{0}};
    strokeAttr.style = &strokeStyle;
    if (style_->lineCap_ == CapType::CAP_ROUND) {
        strokeAttr.cap = VGU_LINECAP_ROUND;
    ...
    VGUMatrix3 mat = {{{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}};
    uintptr_t vguPath = LiteMGfxEngine::GetInstance()->InitPath(true);
    LiteMGfxEngine::GetInstance()->SetPathData(vguPath, cmdNum, cmds_, dataNum, pathData_);
    LiteMGfxEngine::GetInstance()->RenderPath(gfxDstBuffer, vguPath, mask, &strokeAttr, nullptr, &mat);
}
#endif
