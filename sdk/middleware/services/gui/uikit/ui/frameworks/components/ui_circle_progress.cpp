...
void UICircleProgress::DrawForegroundGradient(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea,
        const Point& point, const uint16_t& radius, const Vector2<float>& angle)
{
    ArcInfo arcInfo = {point, {0, 0}, radius, angle.x_, angle.y_, nullptr};
    while (FloatLess(arcInfo.endAngle, arcInfo.startAngle)) {
        arcInfo.endAngle += CIRCLE_IN_DEGREE;
    }

    arcInfo.radius -= foregroundStyle_->lineWidth_ / 2; // 2: half
    ...
    cmdNum_ = 0;
    dataNum_ = 0;
    if (!HWDrawUtils::GetInstance()->ObtainArcPath(arcInfo, cmdNum_, cmd_, dataNum_, data_)) {
        GRAPHIC_LOGE("ObtainArcPath failed.\n");
        return;
    }

    int16_t strokeWidth = foregroundStyle_->lineWidth_;
...
void UICircleProgress::DrawCommonCircle(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea)
{
    ...
    if ((startAngle != endAngle) || (foregroundStyle_->lineCap_ == CapType::CAP_ROUND)) {
        ...
        if (foregroundStopCount_ != 0) {
            DrawForegroundGradient(gfxDstBuffer, invalidatedArea, arcinfo.center,
                arcinfo.radius, Vector2<float>{arcinfo.startAngle, arcinfo.endAngle});
            return;
        }
#endif
