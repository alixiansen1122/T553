...
UIView* UIList::Recycle::GetView(int16_t index)
{
    ...
    auto currentNode = scrapView_.Begin();
    if (scrapView_.Size() != 0) {
        CommonAdapter* adapter = dynamic_cast<CommonAdapter*>(adapter_);
        if (adapter != nullptr) {
            uint32_t type = adapter->GetViewType(index);
            if (type != 0) {
                UIView* tempView = nullptr;
                while (currentNode != scrapView_.End()) {
                    tempView = currentNode->data_;
                    if ((reinterpret_cast<uint32_t>(tempView->GetViewId()) & 0xff) == type) {
                        inView = currentNode->data_;
                        break;
                    }
                    currentNode = scrapView_.Next(currentNode);
                }
            }
        } else {
            inView = scrapView_.Back();
            currentNode = scrapView_.Tail();
        }
    }
    ...
    if (retView != nullptr) {
        ...
        if (inView != nullptr && inView == retView) {
            scrapView_.Remove(currentNode);
        }
    }
    return retView;
}
...
bool UIList::DragXInner(int16_t distance)
{
    ...
    if (distance > 0) {
        if (childrenHead_ != nullptr) {
            if (isElastic_ && (reboundSizeLeft > 0) &&
                (childrenHead_->GetX() > (scrollBlankSizeLeft_ + childrenHead_->GetStyle(STYLE_MARGIN_LEFT)))) {
                int16_t offset = childrenHead_->GetX() -
                    (scrollBlankSizeLeft_ + childrenHead_->GetStyle(STYLE_MARGIN_LEFT));
                if (offset > reboundSizeLeft) {
                    offset = reboundSizeLeft;
                }
                distance = CalculateElasticDragLen(reboundSizeLeft, offset, distance);
            }

            if ((childrenHead_->GetX() + distance) >
                (scrollBlankSizeLeft_ + reboundSizeLeft + childrenHead_->GetStyle(STYLE_MARGIN_LEFT))) {
                distance = scrollBlankSizeLeft_ + reboundSizeLeft + childrenHead_->GetStyle(STYLE_MARGIN_LEFT) -
                    childrenHead_->GetX();
            }
        }
    } else {
        if (childrenTail_ != nullptr) {
            if (isElastic_ && (reboundSizeRight > 0) &&
                (childrenTail_->GetRelativeRect().GetRight() <
                (listWidth - scrollBlankSizeRight_ - childrenTail_->GetStyle(STYLE_MARGIN_RIGHT)))) {
                int16_t offset = (listWidth - scrollBlankSizeRight_ - childrenTail_->GetStyle(STYLE_MARGIN_RIGHT))
                    - (childrenTail_->GetRelativeRect().GetRight());
                if (offset > reboundSizeRight) {
                    offset = reboundSizeRight;
                }
                distance = CalculateElasticDragLen(reboundSizeRight, offset, distance);
            }
            if (childrenTail_->GetRelativeRect().GetRight() <
                (listWidth - scrollBlankSizeRight_ - reboundSizeRight - childrenTail_->GetStyle(STYLE_MARGIN_RIGHT))) {
                distance = 0;
            } else if ((childrenTail_->GetRelativeRect().GetRight() + distance) <
                (listWidth - scrollBlankSizeRight_ - reboundSizeRight - childrenTail_->GetStyle(STYLE_MARGIN_RIGHT))) {
                    distance = listWidth - scrollBlankSizeRight_ - reboundSizeRight -
                        childrenTail_->GetStyle(STYLE_MARGIN_RIGHT) - childrenTail_->GetRelativeRect().GetRight();
            }
        }
    }
...
bool UIList::DragYInner(int16_t distance)
{
    ...
    if (distance > 0) {
        if (childrenHead_ != nullptr) {
            if (isElastic_ && (reboundSizeTop > 0) &&
                (childrenHead_->GetY() > (scrollBlankSizeTop_ + childrenHead_->GetStyle(STYLE_MARGIN_TOP)))) {
                int16_t offset = childrenHead_->GetY() -
                    (scrollBlankSizeTop_ + childrenHead_->GetStyle(STYLE_MARGIN_TOP));
                if (offset > reboundSizeTop) {
                    offset = reboundSizeTop;
                }
                distance = CalculateElasticDragLen(reboundSizeTop, offset, distance);
            }
            if ((childrenHead_->GetY() + distance) >
                (scrollBlankSizeTop_ + reboundSizeTop + childrenHead_->GetStyle(STYLE_MARGIN_TOP))) {
                distance = scrollBlankSizeTop_ + reboundSizeTop + childrenHead_->GetStyle(STYLE_MARGIN_TOP) -
                    childrenHead_->GetY();
            }
        }
    } else {
        if (childrenTail_ != nullptr) {
            if (isElastic_ && (reboundSizeBottom > 0) &&
                (childrenTail_->GetRelativeRect().GetBottom() <
                (listHeight - scrollBlankSizeBottom_ - childrenTail_->GetStyle(STYLE_MARGIN_BOTTOM)))) {
                int16_t offset = listHeight - scrollBlankSizeBottom_ - childrenTail_->GetStyle(STYLE_MARGIN_BOTTOM) -
                    childrenTail_->GetRelativeRect().GetBottom();
                if (offset > reboundSizeBottom) {
                    offset = reboundSizeBottom;
                }
                distance = CalculateElasticDragLen(reboundSizeBottom, offset, distance);
            }
            if (childrenTail_->GetRelativeRect().GetBottom() <
                (listHeight - scrollBlankSizeBottom_ - reboundSizeBottom -
                childrenTail_->GetStyle(STYLE_MARGIN_BOTTOM))) {
                distance = 0;
            } else if ((childrenTail_->GetRelativeRect().GetBottom() + distance) <
                (listHeight - scrollBlankSizeBottom_ - reboundSizeBottom -
                childrenTail_->GetStyle(STYLE_MARGIN_BOTTOM))) {
                distance = listHeight - scrollBlankSizeBottom_ - reboundSizeBottom -
                    childrenTail_->GetStyle(STYLE_MARGIN_BOTTOM) - childrenTail_->GetRelativeRect().GetBottom();
            }
        }
    }
...
void UIList::UpdateScrollBar()
{
    ...
    if (yScrollBarVisible_) {
        int16_t totalHeight = allItemsRect.GetHeight() + scrollBlankSizeTop_ + scrollBlankSizeBottom_;
        int16_t height = GetHeight();
        yScrollBar_->SetForegroundProportion(static_cast<float>(height) / totalHeight);
        yScrollBar_->SetScrollProgress(static_cast<float>(scrollBlankSizeTop_ - allItemsRect.GetTop()) /
                                    (totalHeight - height));
    }
    if (xScrollBarVisible_) {
        int16_t totalWidth = allItemsRect.GetWidth() + scrollBlankSizeLeft_ + scrollBlankSizeRight_;
        int16_t width = GetWidth();
        xScrollBar_->SetForegroundProportion(static_cast<float>(width) / totalWidth);
        xScrollBar_->SetScrollProgress(static_cast<float>(scrollBlankSizeLeft_ - allItemsRect.GetLeft()) /
                                    (totalWidth - width));
    }
    RefreshAnimator();
}
...
void UIList::CalculateReboundDistance(int16_t& dragDistanceX, int16_t& dragDistanceY)
{
    ...
    if ((direction_ == VERTICAL) || (direction_ == HORIZONTAL_AND_VERTICAL)) {
        if (top > scrollBlankSizeTop_) {
            if ((dragDistanceY + top) > (scrollBlankSizeTop_ + reboundSizeTop_)) {
                dragDistanceY = 0;
            }
            dragDistanceY += scrollBlankSizeTop_ - (top + dragDistanceY);
        }
        if (bottom < (scrollHeight - scrollBlankSizeBottom_ - 1)) {
            if ((dragDistanceY + bottom) < (scrollHeight - scrollBlankSizeBottom_ - reboundSizeBottom_ - 1)) {
                dragDistanceY = 0;
            }
            dragDistanceY += scrollHeight - scrollBlankSizeBottom_ - 1 - (bottom + dragDistanceY);
        }
    } else {
        if (left > scrollBlankSizeLeft_) {
            if ((dragDistanceX + left) > (scrollBlankSizeLeft_ + reboundSizeLeft_)) {
                dragDistanceX = 0;
            }
            dragDistanceX += scrollBlankSizeLeft_ - (left + dragDistanceX);
        }
        if (right < (scrollWidth - scrollBlankSizeRight_ - 1)) {
            if ((dragDistanceX + right) < (scrollWidth - scrollBlankSizeRight_ - reboundSizeRight_ - 1)) {
                dragDistanceX = 0;
            }
            dragDistanceX += scrollWidth - scrollBlankSizeRight_ - 1 - (right + dragDistanceX);
        }
    }
}
...
void UIList::FixHorDistance(int16_t& distanceX)
{
    ...
    if (childrenRect.GetX() > 0) {
        int16_t headMid = childrenHead_->GetX() + childrenHead_->GetRelativeRect().GetWidth() / 2;
        if ((headMid + distanceX >= selectPosition_) || (headMid >= selectPosition_)) {
            ofs = selectPosition_ - (headMid + distanceX);
        }
    } else if (childrenRect.GetRight() < GetWidth() - 1) {
        int16_t tailMid = childrenTail_->GetX() + childrenTail_->GetRelativeRect().GetWidth() / 2;
        if ((tailMid + distanceX <= selectPosition_) || (tailMid <= selectPosition_)) {
            ofs = selectPosition_ - (tailMid + distanceX);
        }
    }
...
void UIList::FixVerDistance(int16_t& distanceY)
{
    int16_t ofs = 0;
    Rect childrenRect = GetAllChildRelativeRect();
    if (childrenRect.GetY() > 0) {
        int16_t headMid = childrenHead_->GetY() + childrenHead_->GetRelativeRect().GetHeight() / 2;
        if ((headMid + distanceY >= selectPosition_) || (headMid >= selectPosition_)) {
            ofs = selectPosition_ - (headMid + distanceY);
        }
    } else if (childrenRect.GetBottom() < GetHeight() - 1) {
        int16_t tailMid = childrenTail_->GetY() + childrenTail_->GetRelativeRect().GetHeight() / 2;
        if ((tailMid + distanceY <= selectPosition_) || (tailMid <= selectPosition_)) {
            ofs = selectPosition_ - (tailMid + distanceY);
        }
    }
