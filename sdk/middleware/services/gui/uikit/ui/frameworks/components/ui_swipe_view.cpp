...
{
    if (childrenTail_ == childrenHead_) {
        return;
    }
...
{
    if (needAnimator && (childrenNum_ > 1)) {
        MoveHeadOrTailChild();
    }
...
bool UISwipeView::DragXInner(int16_t distance)
{
    ...
    if (!loop_) {
        if (isElastic_ && (reboundSize_ > 0) && (((childrenHead_->GetX() >= 0) && (distance > 0)) ||
            ((childrenTail_->GetX() + childrenTail_->GetRelativeRect().GetWidth() <= GetWidth()) && (distance < 0)))) {
            int16_t elasticLen = childrenHead_->GetX();
            if (distance < 0) {
                elasticLen = GetWidth() - (childrenTail_->GetX() + childrenTail_->GetRelativeRect().GetWidth());
            }
            accumulatedSize_ += distance;
            distance = CalculateElasticDragLen(reboundSize_ + blankSize_, elasticLen, accumulatedSize_);
            if (distance != 0) {
                accumulatedSize_ = 0;
            }
        } else {
...
bool UISwipeView::DragYInner(int16_t distance)
{
    ...
    if (!loop_) {
        if (isElastic_ && (reboundSize_ > 0) &&
            (((childrenHead_->GetY() >= 0) && (distance > 0)) ||
            ((childrenTail_->GetY() + childrenTail_->GetRelativeRect().GetHeight() <= GetHeight()) && (distance < 0)))) {
            int16_t elasticLen = childrenHead_->GetY();
            if (distance < 0) {
                elasticLen = GetHeight() - (childrenTail_->GetY() + childrenTail_->GetRelativeRect().GetHeight());
            }
            accumulatedSize_ += distance;
            distance = CalculateElasticDragLen(reboundSize_ + blankSize_, elasticLen, accumulatedSize_);
            if (distance != 0) {
...
bool UISwipeView::OnDragEndEvent(const DragEvent& event)
{
    if (isElastic_ && (reboundSize_ > 0)) {
        ...
        if (direction_ == HORIZONTAL) {
            elasticFlag = (childrenHead_->GetX() > 0) ||
                (childrenTail_->GetX() + childrenTail_->GetRelativeRect().GetWidth() < GetWidth());
        } else {
            elasticFlag = (childrenHead_->GetY() > 0) ||
                (childrenTail_->GetY() + childrenTail_->GetRelativeRect().GetHeight() < GetHeight());
        }
        if (elasticFlag) {
            ElasticAnimator(elasticTime_ / DEFAULT_TASK_PERIOD);
            return true;
        }
        accumulatedSize_ = 0;
    }
...
void UISwipeView::Vibrate()
{
    ...
    int16_t (UIView::*pfnGetWidthOrHeight)();
    if (direction_ == HORIZONTAL) {
        pfnGetXOrY = &UIView::GetX;
        pfnGetWidthOrHeight = &UIView::GetWidthWithMargin;
    } else {
        pfnGetXOrY = &UIView::GetY;
        pfnGetWidthOrHeight = &UIView::GetHeightWithMargin;
    }
    if (alignMode_ == ALIGN_LEFT) {
        swipeMid = 0;
    } else if (alignMode_ == ALIGN_RIGHT) {
        ...
        swipeMid = (this->*pfnGetWidthOrHeight)() / 2;
    }
    bool isFind = false;
    while (view != nullptr) {
        if ((swipeMid >= (view->*pfnGetXOrY)()) &&
            (swipeMid <= (view->*pfnGetXOrY)() + (view->*pfnGetWidthOrHeight)()) && !isFind) {
            vibrateIndex_ = view->GetViewIndex();
            isFind = true;
            break;
        }
        view = view->GetNextSibling();
    ...
    if (!isFind) {
        if ((childrenHead_->*pfnGetXOrY)() > swipeMid) {
            vibrateIndex_ = childrenHead_->GetViewIndex();
        } else {
            vibrateIndex_ = childrenTail_->GetViewIndex();
        }
    }

    VibratorFunc vibratorFunc = VibratorManager::GetInstance()->GetVibratorFunc();
    if (vibratorFunc != nullptr && needVibration_ && vibrateIndex_ != lastIndex_) {
        if (!loop_ && (vibrateIndex_ == 0 || vibrateIndex_ == childrenNum_ - 1)) {
            vibratorFunc(VibratorType::VIBRATOR_TYPE_THREE);
            GRAPHIC_LOGI("UISwipeView::Vibrate calls TYPE_THREE vibrator");
        } else {
            vibratorFunc(VibratorType::VIBRATOR_TYPE_ONE);
            GRAPHIC_LOGI("UISwipeView::Vibrate calls TYPE_ONE vibrator");
        }
        lastIndex_ = vibrateIndex_;
    }
}
