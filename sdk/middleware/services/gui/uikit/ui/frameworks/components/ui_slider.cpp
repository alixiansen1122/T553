...
void UISlider::DrawKnob(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea, const Rect& foregroundRect)
{
    ...
    if (transMap_ != nullptr) {
        transMap_->SetTransMapRect(knobBar);
        Rect updateKnobBar = transMap_->GetBoxRect();
        if (!knobBar.IsValid()) {
            updateKnobBar = knobBar;
        }
        transMap_->SetTransMapRect(invalidatedArea);
        Rect updateInvalidatedArea = transMap_->GetBoxRect();
        DrawValidRect(gfxDstBuffer, knobImage_, updateKnobBar, updateInvalidatedArea, *knobStyle_, 0);
...
void UISlider::DrawForeground(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea, Rect& coords)
{
    ...
    if (transMap_ != nullptr) {
        ...
        Rect updateCoords = transMap_->GetBoxRect();
        transMap_->SetTransMapRect(foregroundRect);
        foregroundRect = transMap_->GetBoxRect();
        if (tmp.Intersect(updateCoords, invalidatedArea)) {
            DrawValidRect(gfxDstBuffer, foregroundImage_, foregroundRect, tmp, *foregroundStyle_, radius);
        }
        transMap_->SetTransMapRect(GetOrigRect());
    } else {
        if (tmp.Intersect(coords, invalidatedArea)) {
            DrawValidRect(gfxDstBuffer, foregroundImage_, foregroundRect, tmp, *foregroundStyle_, radius);
        }
    }
}
