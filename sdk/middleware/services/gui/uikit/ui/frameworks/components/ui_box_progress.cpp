...
void UIBoxProgress::DrawBackground(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea)
{
    ...
    if (transMap_ != nullptr) {
        transMap_->SetTransMapRect(coords);
        DrawValidRect(gfxDstBuffer, backgroundImage_, transMap_->GetBoxRect(), invalidatedArea, *backgroundStyle_, radius);
...
void UIBoxProgress::DrawForeground(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea, Rect& coords)
{
    ...
    if (foregroundStopCount_ != 0) {
        DrawForegroundGradient(gfxDstBuffer, coords, invalidatedArea, radius);
        return;
    }
#endif
...
#if ENABLE_VGU_ENGINE && CAPABILITY_HWDRAW_PATH
void UIBoxProgress::DrawForegroundGradient(BufferInfo& gfxDstBuffer,
    const Rect& rect, const Rect& invalidatedArea, uint16_t radius)
{
    if (cmd_ == nullptr) {
        cmdNum_ = CMD_NUM;
        cmd_ = static_cast<uint8_t*>(AllocBufferForPath(cmdNum_ * sizeof(uint8_t)));
        if (cmd_ == nullptr) {
            GRAPHIC_LOGE("AllocBufferForPath failed for cmd.\n");
            return;
        }
    }
    if (data_ == nullptr) {
        dataNum_ = DATA_NUM;
        data_ = static_cast<float*>(AllocBufferForPath(dataNum_ * sizeof(float)));
        if (data_ == nullptr) {
            GRAPHIC_LOGE("AllocBufferForPath failed for data.\n");
    ...
    int16_t strokeWidth = 0;
    if (direction_ == Direction::DIR_LEFT_TO_RIGHT ||
        direction_ == Direction::DIR_RIGHT_TO_LEFT) {
        strokeWidth = rect.GetHeight();
        data_[0] = rect.GetX();
        data_[1] = rect.GetY() + rect.GetHeight() / 2; // 2: half
        data_[2] = rect.GetRight(); // 2: index;
        data_[3] = data_[1]; // 3: index;
    } else {
        strokeWidth = rect.GetWidth();
        data_[0] = rect.GetX() + rect.GetWidth() / 2; // 2: half
        data_[1] = rect.GetY();
        data_[2] = data_[0]; // 2: index;
        data_[3] = rect.GetBottom(); // 3: index;
    }
    RenderGradientStroke(gfxDstBuffer, invalidatedArea, strokeWidth);
}
#endif
