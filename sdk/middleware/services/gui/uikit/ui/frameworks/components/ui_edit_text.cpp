...
bool UIEditText::OnPressEvent(const PressEvent& event)
{
    if (touchable_) {
        RequestFocus();
        InputMethodManager::GetInstance().ShowInputMethod(this);
    }
...
void UIEditText::SetMaxLength(uint16_t maxLength)
{
    ...
    uint16_t curLen = static_cast<uint16_t>(TypedText::GetUTF8CharacterSize(textStr_.c_str()));
    if (curLen > maxLength) {
        uint32_t byteIndex = TypedText::GetByteIndexFromUTF8Id(textStr_.c_str(), maxLength);
        std::string str = textStr_.substr(0, byteIndex);
        SetText(str);
    }
}
