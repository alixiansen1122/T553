...
bool UIPicker::SetValues(const char* value[], uint16_t count)
{
    ...
    if (setSelectedIndex_ >= rangeValueCount_) {
        GRAPHIC_LOGW("selectIndex(%d) is greater than rangeValueCount(%d), set selectIndex to 0.",
            setSelectedIndex_, rangeValueCount_);
        setSelectedIndex_ = 0;
    }
