...
void UIImageView::UpdateDrawTransMap(bool updateContentMatrix)
{
    ...
    drawTransMap_->SetMatrix(transMap_->GetOrigTransformMatrix() * (*contentMatrix_));
}
        ...
        if ((srcType == IMG_SRC_FILE) || (srcType == IMG_SRC_VARIABLE)) {
            ...
            if ((drawTransMap_ == nullptr) || drawTransMap_->IsInvalid()) {
                ...
                if (srcType == IMG_SRC_FILE) {
                    ...
                    ImageCacheManager::GetInstance().UpdateImageInfoIfNecessary(*const_cast<ImageInfo*>(GetImageInfo()));
                    if (GetImageInfo()->data == nullptr) {
                        GRAPHIC_LOGE("Draw UIImageView failed. UpdateImageInfoIfNecessary failed.");
                        return;
                    }
#endif
...
#endif
    if (!updated) {
        if ((image_.GetPath() != nullptr) && (GetSrcType() == IMG_SRC_FILE)) {
            CacheManager::GetInstance().Close(image_.GetPath());
        }
...
void UIImageView::ReMeasure()
{
    ...
    if ((imageWidth_ != header.width) || (imageHeight_ != header.height)) {
...
void UIImageView::DumpImgInfo()
{
    if (image_.GetSrcType() == IMG_SRC_VARIABLE) {
...
std::string UIImageView::GetGuiInfo() const
{
    ...
    if (image_.GetSrcType() == IMG_SRC_VARIABLE && buf != nullptr) {
        memset_s(buf, size, 0, size);
        const ImageInfo* info = image_.GetImageInfo();
        sprintf_s(buf, size, "ImgInfo[%p]: size = [%d, %d], fmt = %d, data = %p, dataSize = %d, file = %s, resId = %u",
