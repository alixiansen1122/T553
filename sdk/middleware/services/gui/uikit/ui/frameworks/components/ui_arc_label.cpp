...
std::string UIArcLabel::GetGuiInfo() const
{
    if (arcLabelText_ == nullptr) {
        return "";
    }
    ...
    sprintf_s(buf, size, "text: %s, start angle: %f, end angle %f, radius: %u, center[%d, %d]",
        arcLabelText_->GetText(), startAngle_, endAngle_, radius_, arcCenter_.x, arcCenter_.y);
    std::string res(buf);
    delete []buf;
    return res;
}
...
void UIArcLabel::MeasureArcTextInfo()
{
    ...
    if (arcTextInfo_.startAngle >= 360.0f) {
        arcTextInfo_.startAngle -= floor(startAngle_ / 360.0f) * 360.0f;
    } else if (arcTextInfo_.startAngle < 0.0f) {
        arcTextInfo_.startAngle += ceil(-startAngle_ / 360.0f) * 360.0f;
    }
