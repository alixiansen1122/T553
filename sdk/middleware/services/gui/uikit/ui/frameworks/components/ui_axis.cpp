...
void UIYAxis::UpdateAxisPoints()
{
    ...
    start_.x = (axisAlign_ == AxisAlign::ALIGN_LEFT) ? current.GetLeft() : current.GetRight();
    end_.x = start_.x;
    if (enableReverse_) {
        start_.y = top;
        end_.y = bottom;
...
void UIYAxis::TranslateToPixel(int16_t& value)
{
    ...
    int16_t barLen = static_cast<int16_t>((value - minRange_) * minYStep);
    if (enableReverse_) {
        value = start_.y + barLen;
    } else {
        value = start_.y - barLen;
    }
}
...
void UIYAxis::DrawAxisMark(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea)
{
    ...
    while (index <= markDataCount_) {
        ...
        end.x = (axisAlign_ == AxisAlign::ALIGN_LEFT) ? start.x + AXIS_DEFAULT_MARK_LENGTH :
            start.x - AXIS_DEFAULT_MARK_LENGTH;
        end.y = start.y;
