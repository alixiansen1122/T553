...
void UIDialog::SetTitle(const char* title)
{
    ...
    titleText_ = new char[len + 1]();
    if (titleText_ == nullptr) {
        GRAPHIC_LOGE("new titleText_ fail");
        return;
    }
    if (memcpy_s(titleText_, len + 1, title, len) != EOK) {
...
void UIDialog::SetText(const char* text)
{
    ...
    textText_ = new char[len + 1]();
    if (textText_ == nullptr) {
        GRAPHIC_LOGE("new textText_ fail");
        return;
    }
    if (memcpy_s(textText_, len + 1, text, len) != EOK) {
        GRAPHIC_LOGE("memcpy_s error!!!");
    }
}
    ...
    switch (buttonType) {
        ...
        case DialogButtonType::BUTTON_RIGHT:
            if (button3_ != nullptr) {
                ...
                button3_->SetStyleForState(STYLE_BACKGROUND_OPA, color.alpha, UIButton::ButtonState::RELEASED);
                colorType3_ = color;
            }
            break;
        default:
