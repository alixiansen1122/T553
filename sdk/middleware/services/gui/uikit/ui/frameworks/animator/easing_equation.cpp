...
{
    if (curTime < durationTime) {
        float t = ((float)curTime * QUARTER_IN_DEGREE) / durationTime - QUARTER_IN_DEGREE;
        float x = Sin(t) + 1;
...
int16_t EasingEquation::ElasticEaseIn(int16_t startPos, int16_t endPos, uint16_t curTime, uint16_t durationTime)
{
    if (curTime < durationTime) {
        double factor = pow((float)(durationTime - curTime) / durationTime, 2);
        return static_cast<int16_t>((1 - factor) * (endPos - startPos)) + startPos;
    }
