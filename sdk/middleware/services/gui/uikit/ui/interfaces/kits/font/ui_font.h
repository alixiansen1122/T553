 ...
 * @{
 *
 * @brief Defines UI font operations.
 *
 * @since 1.0
 * @version 1.0
 */
...
/**
 *
 * @brief Provide UI font operations.
 *
 */
...
public:
    ...
    /**
     * @brief Get path data by unicode
     *
     */
    bool GetGlyphPathData(uint32_t unicode, uint8_t shapingFont, GlyphPathData& data);

    /**
     * @brief Try to free mem used for font cache by LRU rule. This is only applicable for UIFontHWVector.
     *
     */
    ...
    bool GetEmoji(uint32_t unicode, ImageInfo &unicodeInfo);
