...
public:
    ...
    bool SetGradient(const LinearGradient& linear, const ColorStop* stops, uint16_t stopCount);
    void ClearGradient();
...
private:
    ...
    void ClearPath();
    Path* AllocPath(uint32_t cmdNum, uint32_t dataNum);
    void HWDrawLine(BufferInfo& gfxDstBuffer, const Rect& mask, Point start, Point end, int16_t width,
        uint32_t color);
};
...
private:
    ...
    uint8_t* cmds_ = nullptr;
    float* pathData_ = nullptr;
    void GradientWithVGU(BufferInfo& gfxDstBuffer, const Rect& invalidatedArea, UIChartDataSerial* data);
    void ClearPath();
    bool AllocMemForPath(uint32_t cmdCount, uint32_t dataCount);
    void HWDrawPolyLine(BufferInfo& gfxDstBuffer, const Rect& mask, int16_t width,
