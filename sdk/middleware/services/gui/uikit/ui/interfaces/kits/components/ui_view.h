...
const char* const VIEW_TYPE_STRING[UI_NUMBER_MAX] = {
    ...
    "UIQrcode",         "UILabelExt",      "UILabelButtonExt",
    "UICanvasExt",      "UIChartPillar",   "UIChartPolyline",
    "UIXAxis",          "UIYAxis",         "UILiteSurfaceView",
    "Layout",           "ListLayout",      "FlexLayout",
    "GridLayout",
#if ENABLE_SOFT_KEYBOARD
    "UIEdit",           "UIKeyboardButton",
#endif
};
#endif // ENABLE_DEBUG
...
public:
     ...
     * @version 3.0
     */
    virtual void SetOnRotateListener(OnRotateListener* onRotateListener)
    {
     ...
     * @version 3.0
     */
    virtual OnRotateListener*& GetOnRotateListener()
    {
    ...
    /**
     * @brief Sets whether blur with full screen.
     * @param isFull Specifies whether blur with full screen. Value <b>true</b> means to blur with full screen,
     *                  and <b>false</b> means the opposite.
     * @since 1.0
     * @version 1.0
     */
    void SetFullScreenBlur(bool isFull)
    {
        fullScreenBlur_ = isFull;
    }

    /**
     * @brief Checks whether blur with full screen.
     * @return Returns <b>true</b> if the view blur with full screen; returns <b>false</b> otherwise.
     * @since 1.0
     * @version 1.0
     */
    bool IsFullScreenBlur() const
    {
        return fullScreenBlur_;
    }
    ...
    void AlignVerCenterToSibling(const char* id, int16_t offset = 0);
     ...
     /**
     * @brief  Align the view with the reference view in alignType.
     * @param alignType Indicates the alignment relationship.
     * @param referView Indicates the view to be aligned.
     * @param ofsX Indicates the offset added to the x-axis after the view is alignend.
     * @param ofsY Indicates the offset added to the y-axis after the view is alignend.
     * @since 1.0
     * @version 1.0
     */
    void AlignToReference(AlignmentType alignType, UIView* referView, int16_t ofsX = 0, int16_t ofsY = 0);

    /**
     * @brief  Update child view position when this size of parent view changed.
     * @param view Parent view pointer, the view align with it.
     * @since 1.0
     * @version 1.0
     */
    void UpdatePos();
     ...
     */
#if ENABLE_SWEEP_CLOCK
    void Rotate(float angle, const Vector2<float>& pivot);
#else
     ...
     */
#if ENABLE_SWEEP_CLOCK
    void Rotate(float angle, const Vector3<float>& pivotStart, const Vector3<float>& pivotEnd);
#else
     ...
     * @version 3.0
     */
    virtual void Scale(const Vector3<float>& scale, const Vector3<float>& pivot);
    ...
    /**
     * @brief Sets blur radius
     * @param radius Indicates the blur effect of different degrees, the valid range is [0.0f, 32.0f]
     * @since 1.0
     * @version 1.0
     */
    void SetBackgroundBlur(float radius, uint32_t quality = 0);

    /**
     * @brief Clear blur param
     * @since 1.0
     * @version 1.0
     */
     ...
     * @version 3.0
     */
    virtual void SetOnFocusListener(OnFocusListener* onFocusListener)
    {
     ...
     * @version 3.0
     */
    virtual OnFocusListener* GetOnFocusListener() const
    {
     ...
     * @version 3.0
     */
    virtual bool GetBitmap(ImageInfo& bitmap);

    /**
     * @brief If this view is the blurView with blurRadius, need to call this method
     *        to apply blur effect to the background.
     */
    void HandleGaussianBlur(BufferInfo& gfxDstBuffer, bool isNeedRefresh = true);

#if ENABLE_VGU_ENGINE
    void EnableGaussianBlurCache(bool enable);

    bool IsGaussianBlurCacheEnable() const;
#endif

    /**
     * @brief Check whether this view is lite surface view or it contains lite surface view
     */
    bool IsVideoContained();

    void DumpImgInfoRecursive();

    virtual void DumpImgInfo() {}

    virtual std::string GetGuiInfo() const;
...
protected:
    ...
    bool focusable_ : 1;
#endif
    bool fullScreenBlur_ : 1;
    uint8_t opaScale_;
    int16_t index_;
    int16_t pPrevWidth_;
    int16_t pPrevHeight_;
    int16_t ofsX_;
    int16_t ofsY_;
    const char* id_;
    UIView* parent_;
    ...
    void PrintImgInfo(const ImageInfo* info);
    bool IsParentSizeStabled(int16_t rw, int16_t rh)
    {
        bool stable = (rw == pPrevWidth_ && rh == pPrevHeight_);
        pPrevWidth_ = rw;
        pPrevHeight_ = rh;
        return stable;
    }
    void SaveBlurCache(const BufferInfo& gfxDstBuffer, const Rect &mask);
    void DrawBlurCache(BufferInfo& gfxDstBuffer, const Rect &mask);
    Rect GetInnerRect(const Rect &rect, int16_t topRadius, int16_t bottomRadius) const;
...
private:
    ...
    bool CheckIsNeedMapBufferToTrans(const char* funcName);
#endif
#if ENABLE_VGU_ENGINE
    BufferInfo *blurCache_ = nullptr;
    bool isGaussCacheEnabled_ = false;
    Rect lastBlurMask_;
    float lastBlurRadius_ = -1.0;
#endif
};
} // namespace OHOS
