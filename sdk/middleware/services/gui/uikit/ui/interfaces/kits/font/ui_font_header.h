...
constexpr uint8_t FONT_MAGIC_NUM_LEN = 6;
...
constexpr uint32_t MIN_FONT_PSRAM_LENGTH = 0x380000;

// text shaping psram
constexpr uint32_t SHAPING_TTF_TABLE_LENGTH         = 0x4B000; // 307K
constexpr uint32_t SHAPING_WORD_DICT_LENGTH         = 0xD000; // 53K
constexpr uint32_t SHAPING_CACHE_LENGTH             = 0x50000; // 328K
constexpr uint32_t MIN_SHAPING_PSRAM_LENGTH =
    SHAPING_TTF_TABLE_LENGTH + SHAPING_CACHE_LENGTH;
...
struct UITextLanguageFontParam {
    uint8_t size; // Invalid value for vector font
    uint8_t fontWeight;
    uint8_t shaping;
