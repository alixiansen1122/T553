...
public:
    ...
    virtual void Blur(BufferInfo& dst, float radius);
    ...
    virtual BufferInfo* GetNextFBBufferInfo()
    {
        return nullptr;
    }
    ...
    virtual uint16_t GetScreenRadius()
    {
        return radius_;
    }
    ...
    void SetScreenRadius(uint16_t radius)
    {
        radius_ = radius;
    }
#endif
protected:
    ...
    uint16_t height_ = VERTICAL_RESOLUTION;
     /* The radius of the rounded corners for the rounded rectangle screen. The default value is 0. */
    uint16_t radius_ = 0;
