...
#if (ENABLE_JPEG_AND_PNG && CACHE_JPEG_AND_PNG && !CAPABILITY_HW_JPEG_DECODE)
...
RetCode JpegImgDecoder::ReadToCache(ImgResDsc& dsc)
{
    ...
    uint32_t alignWidth = (width / 16) * 16 < width ? ((width / 16) + 1) * 16 : (width / 16) * 16; // 16 : align
    uint16_t height = cinfo_.output_height;
    uint32_t dataSize = alignWidth * height * pixelByteSize;
    ...
    while (cinfo_.output_scanline < cinfo_.output_height) {
        ...
        n += (alignWidth -  width) * 4; // 4 : one pixel
    }
    DestroyDecoder();
    dsc.inCache_ = true;
