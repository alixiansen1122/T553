...
/** calculate aligned byte */
#define ALIGN_BYTE(byte, align) (((byte) + (align) - 1) & (~((align) -1)))
/** 16-byte aligned */
#ifdef VERSION_SOCMN2
static constexpr uint8_t BYTE_ALIGNMENT = 32;
#else
static constexpr uint8_t BYTE_ALIGNMENT = 16;
#endif
/** stride divider */
static constexpr uint8_t STRIDE_DIVIDER = 8;
...
/** Pi value */
static constexpr double UI_PI = 3.14159265358979f;
...
static constexpr float VERTEX_DIST_EPSILON = 1e-14;
static constexpr float BOXER = 180.0f;
...
public:
     ...
     *
     * @param cosA Indicates the cosine of the angle to rotate.
	 * @param sinA Indicates the sine of the angle to rotate.
     ...
     * @version 3.0
     */
    static Matrix4<T> Rotate(T cosA, T sinA, const Vector3<T>& pivot1, const Vector3<T>& pivot2);
...
template<typename T>
Matrix4<T> Matrix4<T>::Rotate(T cosA, T sinA, const Vector3<T>& pivot1, const Vector3<T>& pivot2)
{
    Matrix4<T> rotateMat4;
    T* rData = rotateMat4.data_;
    T length = Sqrt((pivot2.x_ - pivot1.x_) * (pivot2.x_ - pivot1.x_) + // 0
                    (pivot2.y_ - pivot1.y_) * (pivot2.y_ - pivot1.y_) + // 0
                    (pivot2.z_ - pivot1.z_) * (pivot2.z_ - pivot1.z_)); // 1
    if (length == 0) {
        return rotateMat4;
    }

    T diff = 1 - cosA;
    T ux = (pivot2.x_ - pivot1.x_) / length; // 0
    T uy = (pivot2.y_ - pivot1.y_) / length; // 0
    T uz = (pivot2.z_ - pivot1.z_) / length; // 1

    Matrix4<T> transToOrigin = Matrix4<T>::Translate(Vector3<T> { -pivot1.x_, -pivot1.y_, -pivot1.z_ });

    rData[0] = ux * ux * diff + cosA; // 0 : index          // cosA
    rData[1] = uy  * ux * diff + uz * sinA; // 1 : index    // sinA
    rData[2] = uz * ux * diff - uy * sinA; // 2 : index     // 0

    rData[4] = ux * uy * diff - uz * sinA; // 4 : index     // -sinA
    rData[5] = uy * uy * diff + cosA; // 5 : index          // cosA
    rData[6] = uz * uy * diff + ux * sinA; // 6 : index     // 0

    rData[8] = ux * uz * diff + uy * sinA; // 8 : index     // 0
    rData[9] = uy * uz * diff - ux * sinA; // 9 : index     // 0
    rData[10] = uz * uz * diff + cosA; // 10 : index        // 0
...
Matrix4<T> Matrix4<T>::Rotate(T angle, const Vector3<T>& pivot1, const Vector3<T>& pivot2)
{
    ...
    return Matrix4<T>::Rotate(cosA, sinA, pivot1, pivot2);
}
...
{
    return MATH_ABS(lhs - rhs) < precision;
}

inline bool FloatMore(float lhs, float rhs, float precision = 0.00001f)
{
    return lhs - rhs > precision;
}

inline bool FloatLess(float lhs, float rhs, float precision = 0.00001f)
{
    return lhs - rhs < -precision;
}

inline bool FloatMoreEqual(float lhs, float rhs, float precision = 0.00001f)
{
    return lhs - rhs > -precision;
}

inline bool FloatLessEqual(float lhs, float rhs, float precision = 0.00001f)
{
    return lhs - rhs < precision;
}
