...
#ifdef _WIN32
#include "windows.h"
#elif defined(_LITEOS) || defined(VERSION_IOT)
#else
#include <cerrno>
#include <csignal>
#include <cstring>
...
void GraphicTimer::Stop()
{
    ...
    osStatus_t ret = osStatus_t::osOK;
    if (osTimerIsRunning(timer_)) {
        ret = osTimerStop(timer_);
    }
    if (ret != osStatus_t::osOK) {
        GRAPHIC_LOGE("Timer stop failed, ret: %d", ret);
        return;
    }
}
