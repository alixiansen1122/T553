...
#if LV_USE_IMG_CACHE_EXT
    LvImgCacheSetSize(LV_IMG_CACHE_DEF_SIZE);
#else
...
#if LV_USE_HW_BLUR
bool LvObjSetBackgroundBlur(lv_obj_t* obj, uint16_t radius, bool isFullScreenBlur)
{
    return LvRefrSetBlurObj(obj, radius, isFullScreenBlur);
}

bool LvObjClearBackgroundBlur(void)
{
    return LvRefrClearBlurObj();
}
#endif