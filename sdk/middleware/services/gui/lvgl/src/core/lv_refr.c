...
static bool g_displayPerfFlag = false;
static uint16_t g_printPerfCount = 0;
#endif

#if LV_USE_HW_BLUR
static lv_obj_t* g_blurObj = NULL;
static uint16_t g_blurRadius = 0;
static bool g_isFullScreenBlur = false;
#endif

#if LV_USE_SURFACE_VIEW
static lv_obj_t* g_surfaceView = NULL;
static uint8_t g_postInvalidateCount = 0;
#endif
/**********************
 *      MACROS
 **********************/
...
__attribute__((weak)) void LvPreprocessSurfaceView(void)
{
}

__attribute__((weak)) bool LvIsAsyncMode(void)
{
    return false;
}
...
#if LV_USE_SURFACE_VIEW
void LvAddSurfaceView(lv_obj_t* obj)
{
    g_surfaceView = obj;
    if (obj == NULL) {
        g_postInvalidateCount = 0;
    }
}

static void FlushSurfaceView(void)
{
...
__attribute__((weak)) void LvSurfacePostFlush(lv_obj_t* obj)
{
    (void)obj;
}

static void PostFlushSurfaceView(void)
{
    if (g_surfaceView != NULL) {
...
void LvPostInvalidate(void)
{
    if (g_surfaceView != NULL) {
        g_postInvalidateCount++;
    }
}

static void HandlePostInvalidate(void)
{
    if (g_surfaceView == NULL) {
        return;
    }
    if (g_postInvalidateCount > 0) {
        lv_obj_invalidate(g_surfaceView);
        g_postInvalidateCount--;
    }
}

lv_obj_t* LvGetSurfaceView(void)
{
...
#if LV_USE_HW_BLUR
bool LvRefrSetBlurObj(lv_obj_t* obj, uint16_t radius, bool isFullScreenBlur)
{
    if (obj == NULL) {
        return false;
    }
    if (g_blurObj != NULL) {
        LV_LOGE("Blur obj should be cleared before setting a new one.\n");
        return false;
    }
    if (radius > 32) {
        LV_LOGE("radius is invalid. valid range is [0, 32].\n");
        return false;
    }
    g_blurObj = obj;
    g_blurRadius = radius;
    g_isFullScreenBlur = isFullScreenBlur;
    LvRefrScr();
    return true;
}

bool LvRefrClearBlurObj(void)
{
    if (g_blurObj == NULL) {
        return false;
    }
...
void LvRefrScr(void)
{
    ...
    bool ori_full_refresh = default_disp->driver->full_refresh;
    default_disp->driver->full_refresh = true;

    lv_area_t scr_area;
    scr_area.x1 = 0;
    scr_area.y1 = 0;
    scr_area.x2 = lv_disp_get_hor_res(default_disp) - 1;
    scr_area.y2 = lv_disp_get_ver_res(default_disp) - 1;
    _lv_inv_area(default_disp, &scr_area);
    default_disp->driver->full_refresh = ori_full_refresh;
}
#endif

#if LV_USE_PERF_MONITOR_EXT
void LvSetDisplayPerfFlag(bool flag)
{
    if (g_displayPerfFlag == flag) {
        return;
    }
    if (!flag) {
        ...
        lv_obj_invalidate(perf_monitor.perf_label);
    } else {
        lv_obj_invalidate(lv_layer_sys());
    }
    g_displayPerfFlag = flag;
}

bool LvGetDisplayPerfFlag(void)
{
    return g_displayPerfFlag;
}

void LvSetPrintPerfCount(uint16_t count)
{
    g_printPerfCount = count;
}

uint16_t LvGetPrintPerfCount(void)
{
    return g_printPerfCount;
}

static void HandlePerfMonitorExt(void)
{
    perf_monitor.elaps_sum = lv_tick_elaps(perf_monitor.perf_last_time);
    if (px_num > 0) {
    ...
    if (g_displayPerfFlag || (g_printPerfCount > 0)) {
        uint32_t fps = (1000 * perf_monitor.frame_cnt) / perf_monitor.elaps_sum; // 1000: ms to s
        uint32_t fpsLimit;
        if (disp_refr->refr_timer) {
            fpsLimit = 1000 / disp_refr->refr_timer->period; // 1000: ms to s
        } else {
            fpsLimit = 1000 / LV_DISP_DEF_REFR_PERIOD; // 1000: ms to s
        }
        if (fps > fpsLimit) {
            fps = fpsLimit;
        }
        ...
        uint32_t cpu = 100 - lv_timer_get_idle(); // 100: max

        perf_monitor.perf_last_time = lv_tick_get();
        perf_monitor.elaps_sum = 0;
        perf_monitor.frame_cnt = 0;

        if (g_displayPerfFlag) {
            lv_obj_t* perfLabel = perf_monitor.perf_label;
            if (perfLabel == NULL) {
                perfLabel = lv_label_create(lv_layer_sys());
                lv_obj_set_style_bg_opa(perfLabel, LV_OPA_50, 0);
                lv_obj_set_style_bg_color(perfLabel, lv_color_black(), 0);
                lv_obj_set_style_text_color(perfLabel, lv_color_white(), 0);
                lv_obj_set_style_text_align(perfLabel, LV_TEXT_ALIGN_RIGHT, 0);
                lv_obj_align(perfLabel, LV_USE_PERF_MONITOR_POS, 0, 0);
                perf_monitor.perf_label = perfLabel;
            }
            lv_label_set_text_fmt(perfLabel, "%"LV_PRIu32" FPS\n%"LV_PRIu32"%% CPU", fps, cpu);
        } else {
            if (perf_monitor.perf_label != NULL) {
                lv_obj_del(perf_monitor.perf_label);
                perf_monitor.perf_label = NULL;
            }
        }
        if (g_printPerfCount > 0) {
            --g_printPerfCount;
            LV_LOGI("FPS: %u, CPU: %u\n", fps, cpu);
        }
    } else {
...
static bool AdjustArea(lv_area_t* area)
{
    bool isAdjusted = false;
    bool widthInc = false;
    bool heightInc = false;
    if (lv_area_get_width(area) % 2 == 1) { // 2: alignment
        lv_area_set_width(area, lv_area_get_width(area) + 1);
        widthInc = true;
        isAdjusted = true;
    }
    if (lv_area_get_height(area) % 2 == 1) { // 2: alignment
        lv_area_set_height(area, lv_area_get_height(area) + 1);
        heightInc = true;
        isAdjusted = true;
    }
    if (area->x1 % 2 == 1) { // 2: alignment
        _lv_area_set_pos(area, area->x1 - 1, area->y1);
        if (!widthInc && (area->x1 + lv_area_get_width(area) + 2 <= lv_disp_get_hor_res(disp_refr))) { // 2: increment
            lv_area_set_width(area, lv_area_get_width(area) + 2); // 2: increment
        }
        isAdjusted = true;
    }
    if (area->y1 % 2 == 1) { // 2: alignment
        _lv_area_set_pos(area, area->x1, area->y1 - 1);
        if (!heightInc && (area->y1 + lv_area_get_height(area) + 2 <= lv_disp_get_ver_res(disp_refr))) { // 2: increment
            lv_area_set_height(area, lv_area_get_height(area) + 2); // 2: increment
        }
        isAdjusted = true;
    }
    return isAdjusted;
...
static bool IsValidArea(lv_area_t* area)
{
    return ((area->x2 >= area->x1) && (area->y2 >= area->y1) && (area->x2 - area->x1 + area->y2 - area->y1 != 0));
}

static bool CalculateSyncArea(lv_area_t* lastDirtyArea, lv_area_t* currentDirtyArea, lv_area_t* syncArea)
{
    lv_area_t mask;
    if (_lv_area_intersect(&mask, lastDirtyArea, currentDirtyArea)) {
        if (_lv_area_is_equal(&mask, lastDirtyArea)) {
            return false;
        }
        if (lv_area_get_width(&mask) == lv_area_get_width(lastDirtyArea)) {
            if (mask.y1 == lastDirtyArea->y1) {
                syncArea->y1 = mask.y2 + 1;
            } else if (mask.y2 == lastDirtyArea->y2) {
                syncArea->y2 = mask.y1 - 1;
            }
        } else if (lv_area_get_height(&mask) == lv_area_get_height(lastDirtyArea)) {
            if (mask.x1 == lastDirtyArea->x1) {
                syncArea->x1 = mask.x2 + 1;
            } else if (mask.x2 == lastDirtyArea->x2) {
                syncArea->x2 = mask.x1 - 1;
            }
        }
    }
    return true;
...
static void SyncBufferArea(lv_disp_t* disp)
{
    if (disp->inv_p == 0) {
        return;
    }

    if (AdjustArea(&disp->current_dirty_area)) {
        disp->inv_p = 1;
        disp->inv_areas[0] = disp->current_dirty_area;
        disp->inv_area_joined[0] = 0;
    }

    lv_area_t lastDirtyArea = disp->last_dirty_area;
    if (!IsValidArea(&lastDirtyArea)) {
        return;
    }

    lv_area_t syncArea = lastDirtyArea;
    if ((disp->inv_p == 1) && !CalculateSyncArea(&lastDirtyArea, &disp->current_dirty_area, &syncArea)) {
        return;
    }

    uint8_t* lastBuf = NULL;
    if (disp->driver->draw_buf->buf_act == disp->driver->draw_buf->buf1) {
        lastBuf = disp->driver->draw_buf->buf2;
    } else {
        lastBuf = disp->driver->draw_buf->buf1;
    }

    lv_draw_ctx_t* drawCtx = disp->driver->draw_ctx;
    drawCtx->buf = disp->driver->draw_buf->buf_act;
    drawCtx->buf_stride = disp->driver->draw_buf->buf_stride;

    lv_draw_img_dsc_t dsc = {0};
    lv_draw_img_dsc_init(&dsc);
    dsc.opa = LV_OPA_COVER;
#if LV_USE_VGU
    dsc.blend_mode = LV_BLEND_MODE_SRC;
    dsc.stride = disp->driver->draw_ctx->buf_stride;
#endif

    lv_area_t screenArea = {0, 0, lv_disp_get_hor_res(disp_refr) - 1, lv_disp_get_ver_res(disp_refr) - 1};
    lv_area_t* oriBufArea = drawCtx->buf_area;
    drawCtx->buf_area = &screenArea;
    lv_area_t* oriClip = drawCtx->clip_area;
    drawCtx->clip_area = &syncArea;
    drawCtx->draw_img_decoded(drawCtx, &dsc, &screenArea, lastBuf, LV_IMG_CF_DEFAULT);
...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
...
#if LV_USE_DFX_CMD && LV_USE_DFX_TRACE
    PeriodTraceStart(PERIOD_TRACE_REFR);
#endif
...
#if LV_USE_DFX_CMD && LV_USE_DFX_TRACE
        PeriodTraceEnd(PERIOD_TRACE_REFR);
#endif
        REFR_TRACE("finished");
        return;
    }
...
#if LV_USE_DFX_CMD && LV_USE_DFX_TRACE
    BeginFrameTrace(true);
    PeriodTraceStart(PERIOD_TRACE_TOTAL_DRAW);
#endif

#if defined(MIPI_ULPS_SUPPORT)
    if (disp_refr->inv_p != 0) {
        LvDrawVguCtx* vguCtx = (LvDrawVguCtx*)(disp_refr->driver->draw_ctx);
        vguCtx->StartDraw();
    }
#endif

#if LV_USE_LOCAL_REFRESH
    SyncBufferArea(disp_refr);
#endif
    refr_invalid_areas();
    ...
    if(disp_refr->inv_p != 0) {
#if LV_USE_LOCAL_REFRESH
        if (disp_refr->driver->full_refresh == 0) {
#else
...
#if LV_USE_DFX_CMD && LV_USE_DFX_TRACE
        PeriodTraceEnd(PERIOD_TRACE_TOTAL_DRAW);
        ShowFrameTrace();
#endif
        ...
        disp_refr->inv_p = 0;
#if LV_USE_LOCAL_REFRESH
        lv_area_copy(&disp_refr->last_dirty_area, &disp_refr->current_dirty_area);
        lv_memset_00(&disp_refr->current_dirty_area, sizeof(lv_area_t));
#endif
        ...
        disp_refr->driver->flush_cb(disp_refr->driver, NULL, NULL);
#if LV_USE_DFX_CMD && LV_USE_DFX_TRACE
        PeriodTraceEnd(PERIOD_TRACE_TOTAL_DRAW);
        ShowFrameTrace();
#endif
...
#if LV_USE_LOCAL_REFRESH
        if (join_in == 0) {
            lv_area_copy(&disp_refr->current_dirty_area, &disp_refr->inv_areas[join_in]);
        } else {
            _lv_area_join(&disp_refr->current_dirty_area, &disp_refr->current_dirty_area,
...
#if LV_USE_SNAPSHOT_EXT
void LvRefrAreaForSnapshot(void)
{
    refr_area_part(disp_refr->driver->draw_ctx);
    disp_refr->driver->draw_ctx->wait_for_finish(disp_refr->driver->draw_ctx);
}
#endif
...
#if LV_USE_DPU
    draw_ctx->buf_stride = disp_refr->driver->draw_buf->buf_stride;
#endif
...
#if LV_USE_HW_BLUR && LV_USE_VGU
    if ((g_blurObj != NULL) && (obj == g_blurObj)) {
        lv_area_t* original = draw_ctx->clip_area;
        lv_area_t temp;
        if (g_isFullScreenBlur) {
            temp.x1 = draw_ctx->buf_area->x1;
            temp.y1 = draw_ctx->buf_area->y1;
            temp.x2 = draw_ctx->buf_area->x2;
            temp.y2 = draw_ctx->buf_area->y2;
        } else {
            temp.x1 = obj->coords.x1;
            temp.y1 = obj->coords.y1;
            temp.x2 = obj->coords.x2;
            temp.y2 = obj->coords.y2;
        }
        draw_ctx->clip_area = &temp;
        ((LvDrawVguCtx*)draw_ctx)->Blur(draw_ctx, g_blurRadius);
        draw_ctx->clip_area = original;
    }
#endif
...
#if LV_USE_LOCAL_REFRESH
            call_flush_cb(disp->driver, &disp->current_dirty_area, draw_ctx->buf);
#else
