...
#if LV_USR_CROSS_VIEW
#include "../../proprietary/include/widgets/lv_cross_view.h"
#endif
/*********************
 *      DEFINES
 *********************/
...
static lv_coord_t elastic_diff(lv_obj_t * scroll_obj, lv_coord_t diff, lv_coord_t scroll_start, lv_coord_t scroll_end,
                               lv_dir_t dir);
#if LV_USR_CROSS_VIEW
static lv_coord_t CrossViewFindSnapPointX(const lv_obj_t* obj, lv_coord_t ofs, _lv_indev_proc_t* proc);
#endif
/**********************
 *  STATIC VARIABLES
 **********************/
...
void _lv_indev_scroll_throw_handler(_lv_indev_proc_t * proc)
{
...
#if LV_USR_CROSS_VIEW
        if (lv_obj_check_type(scroll_obj, &LvCrossViewClass)) {
            if (((proc->types.pointer.scroll_sum.x > 0) && (proc->types.pointer.scroll_throw_vect.x < 0)) ||
                ((proc->types.pointer.scroll_sum.x < 0) && (proc->types.pointer.scroll_throw_vect.x > 0))) {
                proc->types.pointer.scroll_throw_vect.x = 0;
            }
        }
#endif
    ...
    if(proc->types.pointer.scroll_dir == LV_DIR_VER) {
        ...
        /*With snapping find the nearest snap point and scroll there*/
            ...
            lv_coord_t x = 0;
            if (lv_obj_check_type(scroll_obj, &LvCrossViewClass)) {
                x = CrossViewFindSnapPointX(scroll_obj, diff_x, proc);
            } else {
                x = find_snap_point_x(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, diff_x);
            }
#else
    ...
    for (uint32_t i = 0; i < child_cnt; i++) {
        ...
        switch (align) {
            ...
            case LV_SCROLL_SNAP_END:
                ...
                break;
            case LV_SCROLL_SNAP_CENTER:
                x_child = child->coords.x1 + lv_area_get_width(&child->coords) / 2;   // 2: Half the width
                if (proc->types.pointer.scroll_sum.x < 0) {
                    x_childT = child->coords.x1 + lv_area_get_width(&child->coords) / 5; // 5: 1/5 of the width
                } else {
                    // 4 5: 4/5 of the width
                    x_childT = child->coords.x1 + lv_area_get_width(&child->coords) * 4 / 5;
                }
                // 2: Half the width
        ...
        x_child += ofs;
        x_childT += ofs;
        if (align == LV_SCROLL_SNAP_CENTER && x_child >= LV_COORD_MIN && x_child <= LV_COORD_MAX) {
            lv_coord_t x = x_childT - x_parent;
            if (LV_ABS(x) < LV_ABS(distT)) {
                distT = x;
                dist = x_child - x_parent;
            }
        } else {
