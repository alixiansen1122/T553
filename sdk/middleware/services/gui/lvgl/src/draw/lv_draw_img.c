...
uint8_t lv_img_cf_get_px_size(lv_img_cf_t cf)
{
    ...
    switch(cf) {
        ...
        case LV_IMG_CF_RGB565:
            px_size = 16; // 16: RGB565 BPP
            break;
        case LV_IMG_CF_RESERVED_18:
            px_size = 1;
            break;
#endif
        default:
                                                      ...
                                                      const lv_area_t * coords, const void * src)
{
...
#if LV_USE_IMG_LRU_CACHE
    _lv_img_cache_entry_t cacheEntry;
    if (draw_dsc->resId != 0) {
        if (GetImgCacheEntry(draw_dsc->resId, &cacheEntry)) {
            cacheEntry.dec_dsc.color = draw_dsc->recolor;
            cacheEntry.dec_dsc.frame_id = draw_dsc->frame_id;
            cdsc = &cacheEntry;
        }
    } else {
#endif
#if LV_USE_IMG_CACHE_EXT
        cdsc = LvImgCacheOpen(src, draw_dsc->recolor, draw_dsc->frame_id, draw_dsc->isLongTermCached);
#else
        cdsc = _lv_img_cache_open(src, draw_dsc->recolor, draw_dsc->frame_id);
#endif
#if LV_USE_IMG_LRU_CACHE
    }
#endif
    ...
    else if ((cdsc->dec_dsc.header.cf == LV_IMG_CF_RGB888) || (cdsc->dec_dsc.header.cf == LV_IMG_CF_DEFAULT) ||
        (cdsc->dec_dsc.header.cf == LV_IMG_CF_RGBA8888) ||
        ((cdsc->dec_dsc.header.cf >= LV_IMG_CF_RESERVED_15) && (cdsc->dec_dsc.header.cf <= LV_IMG_CF_RESERVED_18))) {
#else
    else if ((cdsc->dec_dsc.header.cf == LV_IMG_CF_DEFAULT) || (cdsc->dec_dsc.header.cf == LV_IMG_CF_RGBA8888) ||
        ((cdsc->dec_dsc.header.cf >= LV_IMG_CF_RESERVED_15) && (cdsc->dec_dsc.header.cf <= LV_IMG_CF_RESERVED_18))) {
#endif
