...
LV_ATTRIBUTE_FAST_MEM void lv_draw_sw_blend_basic(lv_draw_ctx_t * draw_ctx, const lv_draw_sw_blend_dsc_t * dsc)
{
...
#if LV_USE_DPU
    uint32_t dest_stride = draw_ctx->buf_stride;
#else
    ...
    if(disp->driver->set_px_cb) {
        if(dsc->src_buf == NULL) {
#if LV_USE_DPU
            fill_set_px(dest_buf, &blend_area, dest_stride, dsc->color, dsc->opa, mask, mask_stride, dsc->blend_mode);
#else
...
#if LV_USE_DPU
                disp->driver->set_px_cb(disp->driver, (void *)dest_buf, dest_stride, x, y, color, opa, blend_mode);
#else
            ...
            for(x = 0; x < w; x++) {
                if(mask[x]) {
                    ...
                    disp->driver->set_px_cb(disp->driver, (void *)dest_buf, dest_stride, blend_area->x1 + x, blend_area->y1 + y, color,
                                            (uint32_t)((uint32_t)opa * mask[x]) >> 8, blend_mode);
#else
                    disp->driver->set_px_cb(disp->driver, (void *)dest_buf, dest_stride, blend_area->x1 + x, blend_area->y1 + y, color,
