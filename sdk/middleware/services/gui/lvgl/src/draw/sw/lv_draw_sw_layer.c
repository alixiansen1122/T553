...
#if LV_USE_DPU
    draw_ctx->buf_stride = layer_sw_ctx->buf_size_bytes / lv_area_get_height(&layer_ctx->area_act);
    draw_ctx->isDrawLayer = true;
#endif
}
...
#if LV_USE_DPU
    draw_ctx->buf_stride = layer_ctx->original.buf_stride;
    draw_ctx->isDrawLayer = false;
#endif
    lv_disp_t * disp_refr = _lv_refr_get_disp_refreshing();
