...
#if LV_USE_IMG_EXT
bool LvImgSetSrcExt(lv_obj_t* obj, const void* src)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_obj_invalidate(obj);
    lv_img_src_t srcType = lv_img_src_get_type(src);
    if (srcType != LV_IMG_SRC_FILE) {
        LV_LOGE("Only accept src file.\n");
        return false;
    }

#if LV_USE_HW_JPEG_DECODER
    if (strcmp(lv_fs_get_ext(src), "bin") && strcmp(lv_fs_get_ext(src), "jpg")) {
#else
    if (strcmp(lv_fs_get_ext(src), "bin")) {
#endif
        LV_LOGE("Only accept bin src file.\n");
        return false;
    }
    ...
    if (lv_img_decoder_get_info(src, &header) != LV_RES_OK) {
        LV_LOGE("Decode header info failed.\n");
        return false;
    }
    if (img->src != src) {
        const void * oldSrc = img->src;
        char* newStr = lv_mem_alloc(strlen(src) + 1);
        if (newStr == NULL) {
            LV_LOGE("malloc for path str failed.\n");
            return false;
        }
        strcpy(newStr, src);
        img->src = newStr;
        if (oldSrc != NULL) {
            lv_mem_free((void*)oldSrc);
        }
    }

    img->src_type = LV_IMG_SRC_FILE;
    img->w        = header.w;
    img->h        = header.h;
    img->cf       = header.cf;
    img->pivot.x = header.w / 2;
...
bool LvImgSetSrcWithCacheFlag(lv_obj_t* obj, const void* src, bool isLongTermCached)
{
    lv_img_t* img = (lv_img_t*)obj;
    img->isLongTermCached = isLongTermCached;
    return LvImgSetSrcExt(obj, src);
}
#endif
#endif
...
#if LV_USE_IMG_EXT
#if LV_USE_HW_JPEG_DECODER
                if ((img->cf == LV_IMG_CF_RGB888) || (img->cf == LV_IMG_CF_DEFAULT) ||
                    (img->cf == LV_IMG_CF_RGBA8888) ||
                    ((img->cf >= LV_IMG_CF_RESERVED_15) && (img->cf <= LV_IMG_CF_RESERVED_18))) {
#else
                if ((img->cf == LV_IMG_CF_DEFAULT) || (img->cf == LV_IMG_CF_RGBA8888) ||
                    ((img->cf >= LV_IMG_CF_RESERVED_15) && (img->cf <= LV_IMG_CF_RESERVED_18))) {
#endif
#if LV_USE_IMG_CACHE_EXT
                    img_dsc.isLongTermCached = img->isLongTermCached;
#endif
#if LV_USE_IMG_LRU_CACHE
                    img_dsc.resId = img->resId;
#endif
                    img_dsc.blend_mode = lv_obj_get_style_blend_mode(obj, LV_PART_MAIN);
                    lv_area_t img_clip_area1;
                    img_clip_area1.x1 = bg_coords.x1 + pleft;
                    img_clip_area1.y1 = bg_coords.y1 + ptop;
                    img_clip_area1.x2 = bg_coords.x2 - pright;
                    img_clip_area1.y2 = bg_coords.y2 - pbottom;
                    if(!_lv_area_intersect(&img_clip_area1, draw_ctx->clip_area, &img_clip_area1)) return;
                    draw_ctx->clip_area = &img_clip_area1;

                    lv_area_t coords_tmp1 = {img_max_area.x1 + img->offset.x, img_max_area.y1 + img->offset.y,
                        img_max_area.x1 + img->offset.x + img->w - 1,
                        img_max_area.y1 + img->offset.y + img->h - 1};
                    ...
                    if ((img->cf == LV_IMG_CF_DEFAULT) || (img->cf == LV_IMG_CF_RGBA8888)) {
#endif
                        img_dsc.stride = LV_ALIGN_BYTE((uint32_t)lv_area_get_width(&coords_tmp1),
                        LV_BYTE_ALIGNMENT) * (lv_img_cf_get_px_size(img->cf) >> 3); // 3: shift by 3 bits
                    }
                    if (img->src_type == LV_IMG_SRC_VARIABLE && img->cf == LV_IMG_CF_RESERVED_18) {
                        img_dsc.stride = (((lv_img_dsc_t*)img->src)->header.w + 7) / 8; // 7：bit, 8: bit to byte
                        img_dsc.stride = LV_ALIGN_BYTE((uint32_t)img_dsc.stride, LV_BYTE_ALIGNMENT);
                    }

                    lv_draw_img(draw_ctx, &img_dsc, &coords_tmp1, img->src);
                    return;
                }
#endif
