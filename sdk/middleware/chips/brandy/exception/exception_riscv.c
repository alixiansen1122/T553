...
#define MMC_BUFFER_ALIGN        64
#define START_SECTOR_NUM        2000
#endif
#ifdef __FREERTOS__
...
// Save nmi exception related registers
static exc_nmi_stat_t g_exc_nmi_store;
// Check whether the exception is repeated
#if (CORE == MASTER_BY_ALL) && (CORE != WIFI)
bool g_is_repeat_exc = false;
#endif
#if defined SAVE_EXC_INFO
...
void exc_info_display(const exc_info_t *exc)
{
    ...
    char* task_name = osThreadGetName(id);
    if (task_name != NULL) {
        PRINT("task       = %s\n", task_name);
    }
    PRINT("thrdPid    = 0x%x\n", (uint32_t*)id);
...
static void show_system_panic_info(const exc_context_t *exc_buf_addr)
{
    ...
    if (exc_type == NMI_INTERRUPT) {
        PRINT("M_CTL_NMI_INT_RAW_STS = 0x%x\n", g_exc_nmi_store.nmi_int_raw_sts);
        PRINT("XIP_CACHE_CTL_INTR_STS = 0x%x\n", g_exc_nmi_store.xip_cache_int_sts);
        PRINT("XIP_CTL_INTR_STS = 0x%x\n", g_exc_nmi_store.xip_int_sts);
    }
...
#endif
    if (exc_type == NMI_INTERRUPT) {
        exc_info->reg0 = g_exc_nmi_store.nmi_int_raw_sts;
        exc_info->reg1 = g_exc_nmi_store.xip_cache_int_sts;
        exc_info->reg2 = g_exc_nmi_store.xip_int_sts;
    }
...
{
    char *exc_info_buffer = (char *)osal_kmalloc_align(ONE_SECTOR_SIZE, 0, MMC_BUFFER_ALIGN);
    if (exc_info_buffer == NULL) {
        return ERRCODE_MALLOC;
    }

    (void)memset_s(exc_info_buffer, ONE_SECTOR_SIZE, 0, ONE_SECTOR_SIZE);
    uint32_t sector_num = mmc_direct_read(0, exc_info_buffer, START_SECTOR_NUM, 1);
    if (sector_num == 0) {
        osal_kfree(exc_info_buffer);
        return ERRCODE_FAIL;
    }
    ...
    (void)memset_s(exc_info_buffer, ONE_SECTOR_SIZE, 0, ONE_SECTOR_SIZE);
    sector_num = mmc_direct_write(0, exc_info_buffer, START_SECTOR_NUM, 1);
    if (sector_num == 0) {
        osal_kfree(exc_info_buffer);
        return ERRCODE_FAIL;
    }
...
static errcode_t exc_info_write_local(exc_info_save_t *exc_info_save, uint32_t exc_size)
{
    int32_t ret;
#if CONFIG_DFX_SUPPORT_FILE_SYSTEM == DFX_YES
...
static errcode_t exc_info_save(uint32_t exc_type, const exc_context_t *exc_buf_addr)
{
    ...
    errcode_t ret = exc_info_write_local(exc_info_save, exc_size);
...
void do_hard_fault_handler(exc_context_t *exc_buf_addr)
{
    ...
    LOS_TaskLock();
#endif
    /* Some exception apis are in the XIP region. Ensure the flash in XIP mode */
    uapi_flash_switch_to_xip_mode(CHIP_FLASH_ID);
#if (CORE == MASTER_BY_ALL) && (CORE != WIFI)
    if (g_is_repeat_exc == false) {
        // if is first excepion write cpu trace to norflash
        write_trace(exc_buf_addr);
    }

    if (g_is_repeat_exc == true) {
        cpu_utils_reset_chip_with_log((cores_t)MASTER_BY_ALL, REBOOT_CAUSE_APPLICATION_SECOND_FAULT);
    }
    /* If the process is restarted at the end of an abnormal process, the value of this variable is false. */
    /* Do not need to manually set false at the end. */
    g_is_repeat_exc = true;
#endif
    ...
    if (exc_type == NMI_INTERRUPT) {
        ...
        g_exc_nmi_store.nmi_int_raw_sts = uapi_reg_read_val16(M_CTL_NMI_INT_RAW_STS);
        g_exc_nmi_store.xip_cache_int_sts = uapi_reg_read_val16(XIP_CACHE_CTL_INTR_STS);
        g_exc_nmi_store.xip_int_sts = uapi_reg_read_val16(XIP_CTL_INTR_STS);
...
{
    /* 重启前关闭所有文件句柄，防止文件异常 */
    for (uint32_t fd = 0; fd < FS_MAX_FILE_HANDLES; fd++) {
        close(fd);
    }

#ifdef CFG_DRIVERS_NANDFLASH
    /* 重启前进行yaffs sync操作，防止文件系统异常 */
    nandflash_yaffs_sync();
#endif
}
...
void nmi_fault_handler(exc_context_t *exc_buff_addr)
{
    uint16_t nmi_raw_status = non_os_get_nmi_raw_status();
    uint16_t xip_cache_int_sts = readw(XIP_CACHE_CTL_INTR_STS);
    uint16_t xip_int_sts = readw(XIP_CTL_INTR_STS);

    PRINT("nmi fault reg is 0x%x 0x%x 0x%x"NEWLINE, nmi_raw_status, xip_cache_int_sts, xip_int_sts);

    if (nmi_raw_status & NON_OS_NMI_CWDT_INT_FLAG) {
        do_hard_fault_handler(exc_buff_addr);
    } else {
        cpu_trace_disable();

        reboot_cause_t reset_cause = hal_set_reset_cause(NMI_INTERRUPT);
        if (reset_cause == REBOOT_CAUSE_APPLICATION_XIP_CACHE) {
            set_fault_reason(readw(XIP_CACHE_CTL_INTR_STS));
        } else if (reset_cause == REBOOT_CAUSE_APPLICATION_XIP_CTRL) {
            set_fault_reason(readw(XIP_CTL_INTR_STS));
        }
