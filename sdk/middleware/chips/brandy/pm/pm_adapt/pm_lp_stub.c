...
void display_suspend(void)
{
    ...
    if (uapi_is_lcd_connect() && (lcd_get_status() || g_frs_sleep_flag)) {
...
void display_resume(void)
{
    ...
    if ((uapi_is_lcd_connect()) && (lcd_get_status() != 0)) {
#ifndef QSPI_DISPLAY
...
#ifdef CFG_DRIVERS_NANDFLASH
    reg16_clrbit(EMMC_CCLK_DIV_REG, 0); // close sd mmc clock enable.
#endif

#ifndef CFG_DRIVERS_SDIO
    reg16_clrbit(SD_HOST_CCLK_DIV_REG, 0); // close sdio host clock enable.
#endif
