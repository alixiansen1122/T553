...
// Random OTP(16bytes)
static errcode_t get_material1(uint8_t *material1, uint16_t material1_len)
{
    errcode_t ret = ERRCODE_FAIL;

    ret = uapi_efuse_read_buffer(material1, OTP_RANDOM_POS, material1_len);
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] uapi_efuse_read_buffer failed!! ret = 0x%x\r\n", ret);
        return ret;
    }
                          ...
                          const kv_key_header_t *header)
{
    ...
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] make_section_key failed! ret = 0x%x\r\n", ret);
        goto err_exit;
    }
    if (memcpy_s(salt, salt_len, section_key, NV_KEY_LENGTH) != EOK) {
        nv_log_err("[NV] memcpy_s failed!\r\n");
        goto err_exit;
    }
    if (memcpy_s(salt + NV_KEY_LENGTH, salt_len - NV_KEY_LENGTH, ta_uuid, sizeof(ta_uuid)) != EOK) {
        nv_log_err("[NV] memcpy_s failed!\r\n");
        goto err_exit;
    }
                              ...
                              const kv_key_header_t *header, uint32_t *die_id)
{
    ...
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] get salt failed!\r\n");
        goto err_exit;
    }

    ret = get_material1(material1, MATERIAL_LEN);
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] get material1 failed!\r\n");
        goto err_exit;
    }
    ...
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] uapi_drv_cipher_hkdf failed!\r\n");
        goto err_exit;
    }
                              ...
                              const kv_key_header_t *header)
{
    ...
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] get_die_id failed!\r\n");
        goto err_exit;
    }
    ...
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] get_password failed!\r\n");
        goto err_exit;
    }
    ...
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] uapi_drv_cipher_symc_gcm_create failed!\r\n");
        goto err_exit;
    }
...
errcode_t nv_crypto_validate_tag(uint32_t crypto_handle)
{
    ...
    if (ret != ERRCODE_SUCC) {
        nv_log_err("[NV] verify tag failed!\r\n");
        return ret;
    }
