...
/* Flash 物理地址起始和结束 */
#define FLASH_PHYSICAL_ADDR_START     (FLASH_START)
#define FLASH_PHYSICAL_ADDR_END       (FLASH_START + FLASH_LENGTH)

#define KV_PAGE_SIZE                  FLASH_PAGE_SIZE

/* NV运行区默认起始地址(物理地址) */
#define KV_STORE_START_ADDR           (NV_FLASH_REGION_START)
/* NV运行区page数 */
#define KV_STORE_DATA_PAGE_NUM        (NV_FLASH_REGION_PAGES)
/* NV运行区默认长度（不包含备份区） */
...
#define CONFIG_NV_SUPPORT_ASYNCHRONOUS_STORE    NV_YES
#endif
