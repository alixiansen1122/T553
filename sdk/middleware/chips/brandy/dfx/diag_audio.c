...
/* debugkit menu点击read/write用于获取或设置AEF参数 */
errcode_t diag_aef_param_trans(uint16_t cmd_id, void *cmd_param, uint16_t cmd_param_size, diag_option_t *option)
{
    errcode_t ret;
    uint8_t *param = (uint8_t *)cmd_param;
    if (cmd_param_size < NV_AUDIO_AEF_PARAM_SIZE) {
        PRINT("[audio][diag][aef] cmd_param_size invalid\r\n");
        return ERRCODE_FAIL;
    }

    if (param[GET_SEE_PARAM_FIRST_POS] != DEBUGKITS_READ_PARAM_TAG) {
        /* set param , this func return 0 when success */
        ret = AudioManagerSetAefParam(SND_PORT_AEF_TYPE, (uint8_t *)param, NV_AUDIO_AEF_PARAM_SIZE);
        if (ret != ERRCODE_SUCC) {
            PRINT("[audio][diag][aef] set param fail, errcode:%d\r\n", ret);
            return ERRCODE_FAIL;
        } else {
            PRINT("[audio][diag][aef] set param success !\r\n");
        }
    } else {
        /* get param , this func return 0 when success */
        ret = AudioManagerGetAefParam(SND_PORT_AEF_TYPE, (uint8_t *)param, NV_AUDIO_AEF_PARAM_SIZE);
        if (ret != ERRCODE_SUCC) {
            PRINT("[audio][diag][aef] get param fail, errcode:%d\r\n", ret);
            return ERRCODE_FAIL;
        } else {
            PRINT("[audio][diag][aef] get param success !\r\n");
        }
    }
    return uapi_diag_report_packet(cmd_id, option, (uint8_t *)cmd_param, NV_AUDIO_AEF_PARAM_SIZE, true);
}

static errcode_t audio_manager_save_aef_param(uint8_t *aef_param, uint16_t len)
{
    if (aef_param == NULL || len != sizeof(audio_aef_param_t)) {
        PRINT("[audio_manager_save_aef_param]sea_param null or len invalid");
        return ERRCODE_INVALID_PARAM;
    }

#ifdef CONFIG_NV_FEATURE_SUPPORT
    errcode_t nv_op = uapi_nv_write((uint16_t)NV_ID_AUDIO_AEF_SWS_PARAM, aef_param, len);
    if (nv_op != ERRCODE_SUCC) {
        return ERRCODE_FAIL;
    }
#endif
...
/* debugkit menu点击Save保存所有AEF参数 */
errcode_t diag_aef_param_save(uint16_t cmd_id, void *cmd_param, uint16_t cmd_param_size, diag_option_t *option)
{
    PRINT("[audio][diag][aef] diag_aef_param_save comes\r\n");
    audio_aef_param_t *nv_data = (audio_aef_param_t *)malloc(sizeof(audio_aef_param_t));
    if (nv_data == NULL) {
        PRINT("[audio][diag][aef] nv malloc fail\r\n");
        return ERRCODE_MALLOC;
    }
    (void)memset_s(nv_data, sizeof(audio_aef_param_t), 0, sizeof(audio_aef_param_t));

    nv_data->size = NV_AUDIO_AEF_PARAM_SIZE;
    errcode_t ret = AudioManagerGetAefParam(SND_PORT_AEF_TYPE, (uint8_t *)nv_data->data,
                                            NV_AUDIO_AEF_PARAM_SIZE * sizeof(uint8_t));
    if (ret != ERRCODE_SUCC) {
        free(nv_data);
        PRINT("[audio][diag][save] get aef param fail\r\n");
        return ERRCODE_FAIL;
    }

    errcode_t nv_op = audio_manager_save_aef_param((uint8_t *)nv_data, (uint16_t)sizeof(audio_aef_param_t));
    if (nv_op != ERRCODE_SUCC) {
        free(nv_data);
        PRINT("[audio][diag] save aef param nv fail\r\n");
        return ERRCODE_FAIL;
    }
    free(nv_data);
    ...
    if (op != ERRCODE_SUCC) {
        ...
        PRINT("[audio][diag][aef] read rpt success\r\n");
    }
    audio_unused(cmd_param);
    audio_unused(cmd_param_size);
    return op;
