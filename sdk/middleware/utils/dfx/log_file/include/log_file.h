...
/*
 * 判断data指向的数据是否是有效的record, 如有效，返回record的长度（包含record头长度），如无效，返回0。
 */
typedef uint32_t (*check_record_valid_cb)(uint8_t *data, uint32_t data_len);
...
typedef struct {
    ...
    uint32_t version;           /* 版本号:
                                   [0 ~ 7]: 存储类型：1 -- 文件存储; 2 -- flash存储;
                                   [8 ~ 15]: 0 -- 长记录头; 1 -- 短记录头; 2 -- 无记录头; 3 -- 预留
                                   [31 ~ 24]: 日志格式：0/0xFE -- FULL; 0xFF -- MINI; 0xFD -- NORMAL; 0xF9 -- EXTEND */
...
typedef struct {
    ...
    uint16_t cache_size;        /* 初始创建文件时申请缓存大小，关闭时释放（enable_cache为1时有效） */
    uint8_t enable_cache : 1;   /* 是否支持缓存 */
    uint8_t no_record_head : 1; /* 是否直接保存数据，不添加记录头 */
    uint8_t reserved : 6;       /* 预留 */
    uint8_t mult_files;         /* 支持的存储文件个数， 为1时表示单文件存储 */
    uint8_t record_max_size;    /* 单条记录最大长度（no_record_head为1时有效） */
    check_record_valid_cb  callback; /* 检查记录是否有效的回调函数（no_record_head为1时有效） */
} store_file_cfg_t;
...
/*
 * 暂时停止日志文件的写入（资源不释放，暂时不再接受日志的写入，和停止对flash的操作）
 * service_type 文件类型
 */
