...
{
    if (error_no != ENOSPC) {
        dfx_log_err("[logfile] save data failed. errno is %d, num is %d\r\n", error_no, num);
        return ERRCODE_DFX_LOGFILE_WRITE_FAIL;
    }

    if (g_not_space_nums < NOT_SPACE_NUM_MAX) {
        g_not_space_nums++;
        dfx_log_err("[logfile] save data failed. errno is %d, num is %d\r\n", error_no, num);
    }

    return ERRCODE_DFX_LOGFILE_NOT_ENOUGH_SPACE;
}
...
STATIC errcode_t logfile_save_data(store_file_info_t *file_info, uint8_t *data, uint32_t data_len)
{
    ...
    if ((file_info->file_head.file_size - file_info->file_head.cur_pos) >= data_len) {
        ...
        if (write_byte != (int32_t)data_len) {
            return logfile_save_data_err(get_errno(), 0);
        }
        ...
        if (write_byte != (int32_t)record_len_written) {
            return logfile_save_data_err(get_errno(), 1); /* 1 : 1st log */
        }
        g_not_space_nums = 0;
        ...
        if (write_byte != (int32_t)record_len_remained) {
            return logfile_save_data_err(get_errno(), 2); /* 2 : 2nd log */
        }
...
STATIC void logfile_load_multi_files_info(store_file_info_t *file_info)
{
    ...
    if (file_info->idx_fd < 0) {
        dfx_log_err("[logfile] open idx file %s failed. errno is %d\r\n", g_full_path, get_errno());
        return;
    }
    ...
    if (read_byte != sizeof(store_muti_file_idx_t)) {
        ...
        lseek(file_info->idx_fd, 0, SEEK_SET);
        write_byte = write(file_info->idx_fd, &(file_info->muti_file_idx), sizeof(store_muti_file_idx_t));
        if (write_byte != sizeof(store_muti_file_idx_t)) {
            dfx_log_err("[logfile] write multi index failed, errno is %d, num is %d\r\n", get_errno(), write_byte);
        }
...
STATIC errcode_t logfile_open_next_file(store_file_info_t *file_info)
{
    ...
    if (file_info->fd < 0) {
        dfx_log_err("[logfile] open next file %s failed. errno is %d\r\n", g_full_path, get_errno());
...
STATIC errcode_t logfile_multi_file_write(store_file_info_t *file_info, uint8_t *data, uint32_t data_len)
{
    int32_t write_byte;
    errcode_t ret_val;
    uint16_t record_len;
    ...
    if (file_info->file_cfg.no_record_head == 0) {
        ...
        if (ret_val != ERRCODE_SUCC) {
            if (ret_val != ERRCODE_DFX_LOGFILE_NOT_ENOUGH_SPACE) {
    ...
    write_byte = write(file_info->fd, &(file_info->file_head), sizeof(store_file_head_t));
    if (write_byte != sizeof(store_file_head_t)) {
        dfx_log_info("[logfile] write file head failed. errno is %d, num is %d\r\n", get_errno(), write_byte);
    }
...
#else /* (CONFIG_DFX_SUPPORT_MULTI_LOG_FILE == DFX_NO) */
...
STATIC errcode_t logfile_get_next_record(store_file_info_t *file_info, uint32_t start_pos, uint32_t *next_pos)
{
    ...
    if (file_info->file_cfg.no_record_head == 0) {
        len_to_read = sizeof(store_record_info_t);
    } else {
        len_to_read = file_info->file_cfg.record_max_size;
    }

    if (len_to_read > RECORD_MAX_LEN) {
        len_to_read = RECORD_MAX_LEN;
    }
    ...
    do {
        ...
        if (tmp_len >= (int32_t)len_to_read) {
            read_byte = read(file_info->fd, record_info, len_to_read);
        } else {
            read_byte = read(file_info->fd, record_info, (uint32_t)tmp_len);
            lseek(file_info->fd, file_info->file_head.offset, SEEK_SET);
            read_byte += read(file_info->fd, record_info + tmp_len, len_to_read - (uint32_t)tmp_len);
        }

        if (read_byte != len_to_read) {
            dfx_log_err("[logfile] read record head failed. errno is %d, num is %d\r\n", get_errno(), read_byte);
        }

        uint32_t record_len = logfile_check_record_valid(file_info, record_info, len_to_read);
        if (record_len != 0) {
            if (tmp_len > record_len) {
                next_record_pos = record_pos + record_len;
            } else {
                next_record_pos = (uint32_t)(file_info->file_head.offset + record_len - tmp_len);
            }
...
STATIC errcode_t logfile_discard_older_records(store_file_info_t *file_info, uint16_t record_len)
{
    uint32_t next_pos = 0;
    errcode_t ret;
    uint32_t space_left;
    ...
    do {
        ...
        if (space_left > record_len) {
            /* 剩余长度与下一条记录长度相等的情况，会导致cur_pos追上first_record_pos，因此此处条件不使用>= */
            break;
        }
    ...
    } while (space_left <= record_len);

    return ERRCODE_SUCC;
}

STATIC errcode_t logfile_single_file_write(store_file_info_t *file_info, uint8_t *data, uint32_t data_len)
{
    int32_t write_byte;
    uint16_t record_len;
    store_record_info_t record_info;
    ...
    /* 废弃最旧的记录以腾出空间 */
    if (logfile_discard_older_records(file_info, record_len) != ERRCODE_SUCC) {
        dfx_log_info("[logfile] discard older records failed\r\n");
    }
    ...
    write_byte = write(file_info->fd, &(file_info->file_head), sizeof(store_file_head_t));
    if (write_byte != sizeof(store_file_head_t)) {
        dfx_log_err("[logfile] write file head failed. errno is %d, num is %d\r\n", get_errno(), write_byte);
    }
    return ERRCODE_SUCC;
}
...
STATIC errcode_t write_cache_to_single_file(store_file_info_t *file_info, uint8_t *data, uint32_t data_len)
{
    ...
    if (logfile_discard_older_records(file_info, (uint16_t)data_len) != ERRCODE_SUCC) {
        dfx_log_info("[logfile] discard older records failed\r\n");
    }
...
#endif /* #if (defined(CONFIG_DFX_SUPPORT_MULTI_LOG_FILE) && (CONFIG_DFX_SUPPORT_MULTI_LOG_FILE == DFX_YES)) */
...
#if (defined(CONFIG_DFX_SUPPORT_MULTI_LOG_FILE) && (CONFIG_DFX_SUPPORT_MULTI_LOG_FILE == DFX_NO))
...
errcode_t logfile_create_path(store_file_cfg_t *cfg)
{
    ...
    if (access(cfg->path, 0) != 0) {
        ret = mkdir(cfg->path, S_IRWXU | S_IRWXG | S_IRWXO);
        if (ret == 0) {
            return ERRCODE_SUCC;
        } else {
            dfx_log_err("[logfile] mkdir %s failed, errno is %d \r\n", cfg->path, get_errno());
...
errcode_t logfile_remove_files(store_service_t service_type, store_file_cfg_t *cfg)
{
    ...
    do {
        ...
        if (strncmp(pdirent->d_name, cfg->name, strlen(cfg->name)) != 0) {
            continue;
        }
        ...
        if (remove((char *)full_path_tmp) != 0) {
            dfx_log_err("[logfile] remove %s failed! errno is %d \r\n", full_path_tmp, get_errno());
            ret = ERRCODE_FAIL;
            break;
        }
    } while (pdirent != NULL);
...
errcode_t logfile_check_parameter(store_file_info_t *file_info)
{
...
errcode_t logfile_prepare_file_fd(store_file_info_t *file_info)
{
    ...
    (void)memset_s((char *)g_full_path, MAX_FILE_PATH_LEN, 0, MAX_FILE_PATH_LEN);
    ret = (errcode_t)strcpy_s((char *)g_full_path, MAX_FILE_PATH_LEN, file_info->file_cfg.path);
    ret += (errcode_t)strcat_s((char *)g_full_path, MAX_FILE_PATH_LEN, file_info->file_cfg.name);
...
#endif
    if (ret != ERRCODE_SUCC) {
        dfx_log_err("[logfile] strcpy_s or strcat_s failed, ret = 0x%x\r\n", ret);
        return ERRCODE_MEMCPY;
    }

    file_info->fd = open((const char *)g_full_path, O_RDWR | O_CREAT, 0664);   /* 0664打开方式 */
    if (file_info->fd < 0) {
        dfx_log_err("[logfile] logfile open file %s failed, errno is %d\r\n", g_full_path, get_errno());
        file_info->fd = INVALID_FILE_ID;
        return ERRCODE_DFX_LOGFILE_OPEN_FAIL;
    }

#if (defined(CONFIG_DFX_SUPPORT_MULTI_LOG_FILE) && (CONFIG_DFX_SUPPORT_MULTI_LOG_FILE == DFX_YES))
    if (file_info->file_cfg.mult_files > 1 && file_info->muti_file_idx.file_count == 0) {
        file_info->muti_file_idx.cur_file_idx = 1;
        file_info->muti_file_idx.oldest_file_idx = 1;
        file_info->muti_file_idx.file_count = 1;
        lseek(file_info->idx_fd, 0, SEEK_SET);
        int32_t write_byte = write(file_info->idx_fd, &(file_info->muti_file_idx), sizeof(store_muti_file_idx_t));
        if (write_byte != sizeof(store_muti_file_idx_t)) {
            dfx_log_info("[logfile] write multi index failed, errno is %d, num is %d\r\n", get_errno(), write_byte);
        }
        fsync(file_info->idx_fd);
    }
#endif
