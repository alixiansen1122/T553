...
{
    int i= 0;
    MyPrintf("================trng-test-start================\n\n");
    trng_test_init();
    for(i = 0; i < 5; i++){
        hal_rng_get_data((void *)(&gRandNum[i]),6);
        MyPrintf("cycle %d csi rand_num: %4d %4d %4d %4d %4d %4d\n",i,
    ...
    for(i = 0; i < 5; i++) {
        hal_rng_get_data((void *)(&gRandNum[i]),6);
        MyPrintf("cycle %d hal rand_num: %4d %4d %4d %4d %4d %4d\n",i,
																						gRandNum[i].rand_num[0],
																						gRandNum[i].rand_num[1],
																						gRandNum[i].rand_num[2],
																						gRandNum[i].rand_num[3],
...
const static uint8_t aes128_cbc_64_iv[] = {
    0x01,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
    0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,
};
const static uint8_t aes128_cbc_64_iv2[] = {
    0x01,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
    0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,
};
...
int aes_test_demo_hal(uint8_t index, uint8_t mode)
{
    ...
    sec_aes_context ctx;
    char *desc;
    uint8_t iv_en_temp[16] = {0};
    uint8_t iv_dec_temp[16] = {0};
    uint16_t keylen;
    ...
    if ( gAesData[index].aes_iv_en != NULL ) {
        memcpy(iv_en_temp, gAesData[index].aes_iv_en, sizeof(iv_en_temp));
    }

    if ( gAesData[index].aes_iv_de != NULL ) {
        memcpy(iv_dec_temp, gAesData[index].aes_iv_de, sizeof(iv_dec_temp));
    }
    ...
    if (ret == 0) {
        MyPrintf("case%d: aes_%s_%d data_len_%d encrypt success! Fuzzing testing pass\n",index,desc,keylen,gAesData[index].aes_original_size);
    } else {
        MyPrintf("case%d: aes_%s_%d data_len_%d encrypt failed! Fuzzing testing not pass\n",index,desc,keylen,gAesData[index].aes_original_size);
		return CSI_ERROR;
	}
    ...
    if (ret == 0) {
        MyPrintf("case%d: aes_%s_%d data_len_%d decrypt success, Fuzzing testing pass\n", index, desc, keylen, gAesData[index].aes_original_size);
    } else {
        MyPrintf("case%d: aes_%s_%d data_len_%d decrypt failed, Fuzzing testing not pass\n", index, desc, keylen, gAesData[index].aes_original_size);
		return CSI_ERROR;
	}
	...
	if (mode) {//cbc
		ret = hal_aes_cbc_encrypt(NULL,
									...
									iv_en_temp);
		if (ret != CSI_OK) {
			MyPrintf("exception test context is NULL failed get out. Fuzzing testing pass\n");
		} else {
			MyPrintf("exception test context is NULL success get out. Fuzzing testing not pass\n");
			return CSI_ERROR;
		}
		...
		ret = hal_aes_cbc_encrypt(&ctx,
									...
									iv_en_temp);
		if (ret != CSI_OK) {
			MyPrintf("exception test key is NULL failed get out. Fuzzing testing pass\n");
		} else {
			MyPrintf("exception test key is NULL success get out. Fuzzing testing not pass\n");
			return CSI_ERROR;
		}
		...
		ret = hal_aes_cbc_encrypt(&ctx,
									...
									iv_en_temp);
		if (ret != CSI_OK) {
			MyPrintf("exception test key_len is 0 failed get out. Fuzzing testing pass\n");
		} else {
			MyPrintf("exception test key_len is 0 success get out. Fuzzing testing not pass\n");
			return CSI_ERROR;
		}
		...
		ret = hal_aes_cbc_encrypt(&ctx,
									...
									iv_en_temp);
		if (ret != CSI_OK) {
			MyPrintf("exception test key_len is 255 failed get out. Fuzzing testing pass\n");
		} else {
			MyPrintf("exception test key_len is 255 success get out. Fuzzing testing not pass\n");
			return CSI_ERROR;
		}
		...
		ret = hal_aes_cbc_encrypt(&ctx,
									...
									iv_en_temp);
		if (ret != CSI_OK) {
			MyPrintf("exception test in is NULL failed get out. Fuzzing testing pass\n");
		} else {
			MyPrintf("exception test in is NULL success get out. Fuzzing testing not pass\n");
			return CSI_ERROR;
		}
		...
		ret = hal_aes_cbc_encrypt(&ctx,
									...
									iv_en_temp);
		if (ret != CSI_OK) {
			MyPrintf("exception test out is NULL failed get out. Fuzzing testing pass\n");
		} else {
			MyPrintf("exception test out is NULL success get out. Fuzzing testing not pass\n");
			return CSI_ERROR;
		}
		...
		ret = hal_aes_cbc_encrypt(&ctx,
									...
									iv_en_temp);
		if (ret != CSI_OK) {
			MyPrintf("exception test size is 0 failed get out. Fuzzing testing pass\n");
		} else {
			MyPrintf("exception test size is 0 success get out. Fuzzing testing not pass\n");
			return CSI_ERROR;
		}
		...
		ret = hal_aes_cbc_encrypt(&ctx,
									...
									NULL);
		if (ret != CSI_OK) {
			MyPrintf("exception test iv is NULL failed get out. Fuzzing testing pass\n");
		} else {
			MyPrintf("exception test iv is NULL success get out. Fuzzing testing not pass\n");
			return CSI_ERROR;
		}
...
int test_case_sha(void)
{
    ...
    if (ret != 0){
        MyPrintf("sha256 digest faled! Fuzzing testing not pass\n");
        return -1;
    }
    ret = memcmp(sum_result,sum,32);
    if (ret == 0) {
        MyPrintf("normal test data_len_%d success! Fuzzing testing pass\n",strlen(msg));
    } else {
        MyPrintf("normal test sha256 data_len_%d failed! Fuzzing testing not pass\n",strlen(msg));
		return -1;
    }

    ret = hal_sha_digest(NULL, 0, (void *)msg, strlen(msg), sum);
    if (ret != 0){
        MyPrintf("exception test context is NULL failed get sha256 digest!  Fuzzing testing pass\n");
    } else {
        MyPrintf("exception test context is NULL success get sha256 digest!  Fuzzing testing not pass\n");
		return -1;
    }

    ret = hal_sha_digest(&ctx_hal, 0, NULL, strlen(msg), sum);
    if (ret != 0){
        MyPrintf("exception test in is NULL failed get sha256 digest!  Fuzzing testing pass\n");
    } else {
        MyPrintf("exception test in is NULL success get sha256 digest!  Fuzzing testing not pass\n");
        return -1;
    }

    ret = hal_sha_digest(&ctx_hal, 0, (void *)msg, 0, sum);
    if (ret != 0){
        MyPrintf("exception test size is 0 failed get sha256 digest!  Fuzzing testing pass\n");
    } else {
        MyPrintf("exception test size is 0 success get sha256 digest!  Fuzzing testing not pass\n");
        return -1;
    }

    ret = hal_sha_digest(&ctx_hal, 0, (void *)msg, strlen(msg), NULL);
    if (ret != 0){
        MyPrintf("exception test digest is NULL failed get sha256 digest!  Fuzzing testing pass\n");
    } else {
        MyPrintf("exception test digest is NULL success get sha256 digest!  Fuzzing testing not pass\n");
        return -1;
    }
    MyPrintf("\n================sha-test-over================\n\n");
