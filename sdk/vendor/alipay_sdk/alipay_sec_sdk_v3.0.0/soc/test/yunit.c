...
static void *malloc_set_zero(int count)
{
    void *pt = hal_malloc(count);
...
int yunit_run_test_suite(void *test_suite)
{
    ...
    if (ret != 0) {
        MyPrintf( "run test suite %s error: ret = %d", node->name, ret);
        return ret;
    }
...
static int _run_test_case(void *test_suite, void *test_case)
{
    ...
    if ( cnt2 - cnt1 != 0 ) {
        MyPrintf("test case #%s fails\n", test_case_node->name);
    } else {
        MyPrintf("test case #%s success\n", test_case_node->name);
    }
...
int yunit_run_test_case(void *test_suite, void *test_case)
{
    ...
    if (ret != 0) {
        MyPrintf("run test suite %s error: ret = %d", tnode->name, ret);
        return ret;
    }
...
void yunit_test_print_result(void)
{
    ...
    MyPrintf("%d test cases, %d success, %d failed\n\n",
           g_test_context.all_test_cases_count,
           g_test_context.all_test_cases_count - (g_test_context.failed_test_cases_count + g_test_context.fatal_test_cases_count),
