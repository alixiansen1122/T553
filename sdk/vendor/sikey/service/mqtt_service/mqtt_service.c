...
static void connectionLost(void *context, char *cause)
{
    ...
    mqtt_backoff_next();  /* Increase backoff on unexpected disconnect; reset only on successful connect. */
...
void mqtt_service_init(void)
{
    ...
    /* Register command handler — must be after transport is up */
    mqtt_cmd_handler_init();

    /* Publish boot event and snapshot initial battery / GNSS state */
    mqtt_event_init();

    /* Initialize activity / health data accumulator */
    mqtt_activity_init();
}
