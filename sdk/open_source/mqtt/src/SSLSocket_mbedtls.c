	...
	const char* hostname, size_t hostname_len)
{
	...
	if (net->ctx != NULL || (ret_state = SSLSocket_createContext(net, opts)) == 0)
	{
		...
		mbedtls_ssl_conf_read_timeout(&net->ctx->conf, 10000); /* 10000: ssl handshake timeout */
		mbedtls_ssl_set_bio(net->ssl, &net->socket, mbedtls_net_send, mbedtls_net_recv, mbedtls_net_recv_timeout);
#else
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && defined(IOT_LITEOS_ADAPT)
