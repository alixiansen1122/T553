...
static void CompressBlock(uint32_t state[8], const uint8_t block[CRYPT_SHA2_256_BLOCKSIZE])
{
    ...
    // a, b, ..., g, h = H(i-1)[0..7]
    uint32_t a = state[0]; // a = H(i-1)0
    uint32_t b = state[1]; // b = H(i-1)1
    uint32_t c = state[2]; // c = H(i-1)2
    uint32_t d = state[3]; // d = H(i-1)3
    uint32_t e = state[4]; // e = H(i-1)4
    uint32_t f = state[5]; // f = H(i-1)5
    uint32_t g = state[6]; // g = H(i-1)6
    uint32_t h = state[7]; // h = H(i-1)7
