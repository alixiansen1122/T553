 ...
 * @file crypt_sha2_256.c
 * Description: SHA2自研算法实现，源码引用于HICRYPTO组件自研SHA2算法
 * Author: liushaoming
 * Create: 2022-02-14
******************************************************************************/

#ifndef HITLS_CRYPTO_NO_SHA256
...
void CRYPT_SHA2_256_Deinit(CRYPT_SHA2_256_Ctx *ctx)
{
    ...
    SAL_CleanseData((void *)(ctx), sizeof(CRYPT_SHA2_256_Ctx));
}
...
static uint32_t CheckIsCorrupted(CRYPT_SHA2_256_Ctx *ctx, uint32_t nbytes);
...
int32_t CRYPT_SHA2_224_Init(CRYPT_SHA2_224_Ctx *ctx)
{
    ...
    ctx->h[2] = 0x3070dd17UL;   // 第2位按照RFC 4634赋值
    ctx->h[3] = 0xf70e5939UL;   // 第3位按照RFC 4634赋值
    ctx->h[4] = 0xffc00b31UL;   // 第4位按照RFC 4634赋值
    ctx->h[5] = 0x68581511UL;   // 第5位按照RFC 4634赋值
    ctx->h[6] = 0x64f98fa7UL;   // 第6位按照RFC 4634赋值
    ctx->h[7] = 0xbefa4fa4UL;   // 第7位按照RFC 4634赋值
...
void CRYPT_SHA2_224_Deinit(CRYPT_SHA2_224_Ctx *ctx)
{
    ...
    SAL_CleanseData((void *)(ctx), sizeof(CRYPT_SHA2_224_Ctx));
}
