 ...
 * @file crypt_aes.c
 * Description: 提供AES算法实现
 * Author: mayihui
 * Create: 2022-5-5
 * Modification History
 * DATE        NAME             DESCRIPTION
 * --------------------------------------------------------------------------
 * 2022-5-5  mayihui        提供AES算法实现
 */
#include "crypt_aes.h"
#include "securec.h"
...
#define SSEARCH(t0, t1, t2, t3)                                                    \
    (((uint32_t)S[((t0) >> 24)] << 24) ^ ((uint32_t)S[((t1) >> 16) & 0xff] << 16) ^ \
...
/* S盒 来源于 FIPS 197 Figure 7. S-box */
...
/* 逆S盒 来源于 FIPS 197 Figure 14. Inverse S-box */
...
static inline uint32_t AES_G(uint32_t w, uint32_t rcon)
{
    uint32_t ret = 0;
    /* 查表并进行移位 */
    ret ^= (uint32_t)S[(w & 0x000000FF) >>  0] << 8;  // 0, 8, S盒内行列转换关系
    ret ^= (uint32_t)S[(w & 0x0000FF00) >>  8] << 16; // 8, 16, S盒内行列转换关系
    ret ^= (uint32_t)S[(w & 0x00FF0000) >> 16] << 24; // 16, 24, S盒内行列转换关系
    ret ^= (uint32_t)S[(w & 0xFF000000) >> 24] << 0; // 24, S盒内行列转换关系
    ret ^= rcon;
    return ret;
}
...
int32_t CRYPT_AES_SetEncryptKey128(CRYPT_AES_Key *ctx, const uint8_t *key, uint32_t len)
{
    ...
    ekey[0] = GET_UINT32_BE(key, 0);  // ekey 0:  key 0 下标开始的4个字节
    ekey[1] = GET_UINT32_BE(key, 4);  // ekey 1:  key 4 下标开始的4个字节
    ekey[2] = GET_UINT32_BE(key, 8);  // ekey 2:  key 8 下标开始的4个字节
    ekey[3] = GET_UINT32_BE(key, 12);  // ekey 3:  key 12 下标开始的4个字节

    // 128bit 需要秘钥长度 11 * 4 = 44，拓展轮数为10 -> 10 * 4 + 4 = 44
    uint32_t times;
...
int32_t CRYPT_AES_SetEncryptKey192(CRYPT_AES_Key *ctx, const uint8_t *key, uint32_t len)
{
    ...
    ekey[2] = GET_UINT32_BE(key, 8);  // ekey 2:  key 8 下标开始的4个字节
    ekey[3] = GET_UINT32_BE(key, 12); // ekey 3:  key 12 下标开始的4个字节
    ekey[4] = GET_UINT32_BE(key, 16); // ekey 4:  key 16 下标开始的4个字节
    ekey[5] = GET_UINT32_BE(key, 20); // ekey 5:  key 20 下标开始的4个字节
...
int32_t CRYPT_AES_SetEncryptKey256(CRYPT_AES_Key *ctx, const uint8_t *key, uint32_t len)
{
    ...
    ekey[4] = GET_UINT32_BE(key, 16); // ekey 4:  key 16 下标开始的4个字节
    ekey[5] = GET_UINT32_BE(key, 20); // ekey 5:  key 20 下标开始的4个字节
    ekey[6] = GET_UINT32_BE(key, 24); // ekey 6:  key 24 下标开始的4个字节
    ekey[7] = GET_UINT32_BE(key, 28); // ekey 7:  key 28 下标开始的4个字节

    /*  需要秘钥长度 15 * 4 = 60，拓展轮数为7 -> 7 * 8 + 8 - 4 = 60 */
    uint32_t times;
    ctx->rounds = 14;  // 14 轮
    uint32_t tmp;
...
void SetDecryptKey(CRYPT_AES_Key *ctx)
{
    ...
    for (i = 1; i < ctx->rounds; i++) {
        dkey += 4;  // 4， 每次运算16个字节
        for (j = 0; j < 4; j++) {   // for 0 to 4 , 运算 16 / sizeof(uint32_t) 个循环
            dkey[j] = TD0[S[(dkey[j] >> 24) & 0xff]] ^  // dkey j >> 24
                      TD1[S[(dkey[j] >> 16) & 0xff]] ^  // dkey j >> 16
                      TD2[S[(dkey[j] >>  8) & 0xff]] ^  // dkey j >> 8
                      TD3[S[(dkey[j] >>  0) & 0xff]];  // dkey j >> 0
        }
    }
}
...
{
    int32_t ret;
    ret = CRYPT_AES_SetEncryptKey128(ctx, key, len);
...
{
    int32_t ret;
    ret = CRYPT_AES_SetEncryptKey192(ctx, key, len);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
{
    int32_t ret;
    ret = CRYPT_AES_SetEncryptKey256(ctx, key, len);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
int32_t CRYPT_AES_Encrypt(const CRYPT_AES_Key *ctx, const uint8_t *in, uint8_t *out, uint32_t len)
{
    ...
    if (ctx->rounds > 10) {         // AES192/AES256执行10、11轮运算
        AES_ENC_ROUND(t, c, 10, ekey);
        AES_ENC_ROUND(c, t, 11, ekey);
    }

    if (ctx->rounds > 12) {         // AES256执行12、13轮运算
        AES_ENC_ROUND(t, c, 12, ekey);
        AES_ENC_ROUND(c, t, 13, ekey);
    }

    /* 最后一轮不进行列混淆，直接进行移位并查找s盒 */
    ekey += ctx->rounds * 4;    // 最后一轮计算ekey的位置，前面已经使用过4 * rounds的ekey

    c0 = SSEARCH(t0, t1, t2, t3) ^ ekey[0];  // operation ekey 0
    c1 = SSEARCH(t1, t2, t3, t0) ^ ekey[1];  // operation ekey 1
    c2 = SSEARCH(t2, t3, t0, t1) ^ ekey[2];  // operation ekey 2
    c3 = SSEARCH(t3, t0, t1, t2) ^ ekey[3];  // operation ekey 3

    PUT_UINT32_BE(c0, out, 0);  // c0转换为out下标为0的4个字节
    PUT_UINT32_BE(c1, out, 4);  // c1转换为out下标为4的4个字节
    PUT_UINT32_BE(c2, out, 8);  // c2转换为out下标为8的4个字节
    PUT_UINT32_BE(c3, out, 12);  // c3转换为out下标为12的4个字节
...
{
    (void)len;
    const uint32_t *dkey = ctx->key + ctx->rounds * 4;  // 4 byte
    ...
    if (ctx->rounds > 10) {         // AES192执行10、11轮运算
    ...
    PUT_UINT32_BE(p0, out, 0);  // in 从 0 下标开始的4个字节
    PUT_UINT32_BE(p1, out, 4);  // in 从 4 下标开始的4个字节
    PUT_UINT32_BE(p2, out, 8);  // in 从 8 下标开始的4个字节
    PUT_UINT32_BE(p3, out, 12);  // in 从 12 下标开始的4个字节
    SAL_CleanseData(&p0, sizeof(uint32_t));
    SAL_CleanseData(&p1, sizeof(uint32_t));
    SAL_CleanseData(&p2, sizeof(uint32_t));
    SAL_CleanseData(&p3, sizeof(uint32_t));
...
void CRYPT_AES_Clean(CRYPT_AES_Key *ctx)
{
    ...
    SAL_CleanseData((void *)(ctx), sizeof(CRYPT_AES_Key));
}