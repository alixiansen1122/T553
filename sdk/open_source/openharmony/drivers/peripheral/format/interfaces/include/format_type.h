 ...
 */
typedef enum {
    AUDIO_SAMPLE_FMT_S8 = 8, /**< 8-bit integer for a single sample */
    AUDIO_SAMPLE_FMT_S16 = 16,    /**< 16-bit integer for a single sample */
    AUDIO_SAMPLE_FMT_S24 = 24,    /**< 24-bit integer for a single sample */
    AUDIO_SAMPLE_FMT_S32 = 32,    /**< 32-bit integer for a single sample */
    AUDIO_SAMPLE_FMT_FLOAT = 33,  /**< Single-precision floating point number for a single sample */
} AudioSampleFmt;
 ...
 */
typedef enum {
    EXT_SVR_SUBTITLE_BITMAP = 0x0, /**< Bitmap (BMP) */
    EXT_SVR_SUBTITLE_TEXT,         /**< Text */
} SubtitleFrameType;
...
typedef enum {
    ...
    DATA_FLAG_AUDIO_FRAME   = 64,
    DATA_FLAG_VIDEO_FRAME   = 128,
    DATA_FLAG_VIDEO_KEY_FRAME = 256,
} DataFlags;
...
typedef struct {
     ...
     */
    uint64_t offset; /**< Local file offset */
    union {
...
typedef enum {
    ...
    OUTPUT_FORMAT_WAV = 5, /**< WAV */
    OUTPUT_FORMAT_MP3 = 6, /**< mp3 */
    OUTPUT_FORMAT_SILK = 7, /* Silk */
...
typedef struct {
    const char *key; /**< album info key. */
    const char *value; /**< album info value. */
} FormatAlbumArtInfoItem;

typedef struct {
    uint32_t infoCnt; /**< album info cnt. */
    FormatAlbumArtInfoItem *info; /**< album info data. For details, see {@link FormatAlbumArtInfoItem}. */
} FormatAlbumArtInfo;
