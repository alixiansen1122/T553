...
const char *trustPath = "/user/music";
const int trustPathSize = strlen(trustPath);
const int MAX_READ_FILE_SIZE = 10 * 1024 * 1024;
...
bool IsValidPath(const char* path)
{
    ...
    bool isTrustPath = (strncmp(path, trustPath, trustPathSize) == 0);
    if (!isTrustPath && (strncmp(path, FILE_PREFIX, PREFIX_LEN) != 0)) {
        return false;
    }

    if (isTrustPath && (pathLen < strlen(trustPath))) {
        return false;
    }

    if ((strncmp(path, FILE_PREFIX, PREFIX_LEN) == 0) && (pathLen < PREFIX_LEN)) {
        return false;
    }
    ...
    if (strpbrk(path + (isTrustPath ? trustPathSize : PREFIX_LEN), "\"*+,:;<=>\?[]|\x7F")) {
        return false;
    }
...
{
    JSIValue retValue = JSI::CreateUndefined();

    if (args != nullptr && argsNum > 0 && NativeapiCommon::IsValidJSIValue(args, argsNum)) {
        ...
        if (params != nullptr) {
            ...
            bool isSuccess = JsAsyncWork::DispatchAsyncWork(ExecuteFunc, reinterpret_cast<void *>(params));
            if (!isSuccess) {
                JSI::ReleaseValue(params->thisVal);
                JSI::ReleaseValue(params->args);
                delete params;
                retValue = JSI::CreateErrorWithCode(-1, "Async work dispatch failed");
            }
        } else {
            retValue = JSI::CreateErrorWithCode(-1, "Failed to allocate FuncParams");
        }
    } else {
        retValue = JSI::CreateErrorWithCode(-1, "Invalid arguments");
    }
    return retValue;
}
...
void ExecuteCopyFile(void* data)
{
    ...
    int ret = NativeapiFs::GetFullPath(src, g_uriFullPath, sizeof(g_uriFullPath));
    ...
    ret = NativeapiFs::GetFullPath(dest, destFullPath, FILE_NAME_MAX_LEN + 1);
    if (ret != NATIVE_SUCCESS) {
        NativeapiCommon::FailCallBack(thisVal, args, ret);
        goto EXIT;
    }
...
void ExecuteGetFileList(void* data)
{
    ...
    int ret = NativeapiFs::GetFullPath(uri, g_uriFullPath, sizeof(g_uriFullPath));
...
int ReadArrayFileInner(const char* path, size_t len, unsigned int position, JSIValue& result)
{
    ...
    size_t fileSize = (info.st_size < 0) ? 0 : static_cast<size_t>(info.st_size);
    if (fileSize > static_cast<size_t>(MAX_READ_FILE_SIZE)) {
        return ERROR_CODE_GENERAL;
    }

    size_t pos = static_cast<size_t>(position);
    if (pos > static_cast<size_t>(INT_MAX)) {
        return ERROR_CODE_GENERAL;
    }

    size_t maxCanRead = (pos < fileSize) ? (fileSize - pos) : 0;
    size_t toRead = (len == 0 || len > maxCanRead) ? maxCanRead : len;

    if (toRead == 0) {
        uint8_t *p = nullptr;
        JSIValue emptyBuf = JSI::CreateArrayBuffer(0, p);
        JSIValue emptyArr = JSI::CreateTypedArray(TypedArrayType::JSI_UINT8_ARRAY, 0, emptyBuf, 0);
        JSI::SetNamedProperty(result, FILE_BUFFER, emptyArr);
        JSI::ReleaseValueList(emptyArr, emptyBuf, ARGS_END);
        return NATIVE_SUCCESS;
    }
    ...
    size_t actualLen = 0;
    ret = ReadFileImpl(path, buf, toRead, static_cast<int>(position), &actualLen);
    if (ret != NATIVE_SUCCESS) {
        free(buf);
        return ret;
    }
    ...
    ret = memcpy_s(ptr, actualLen, buf, actualLen);
    free(buf);
    if (ret != EOK) {
        JSI::ReleaseValue(arrayBuffer);
        return ERROR_CODE_GENERAL;
...
void ExecuteReadArrayFile(void* data)
{
    ...
    ret = NativeapiFs::GetFullPath(src, g_uriFullPath, sizeof(g_uriFullPath));
    JSI::ReleaseString(src);
    if (ret != NATIVE_SUCCESS) {
        NativeapiCommon::FailCallBack(thisVal, args, ret);
        goto EXIT;
...
int NativeapiFs::GetFullPath(const char* uri, char* fullPath, int len)
{
    ...
    int size = PREFIX_LEN;
    bool isTrustPath = (strncmp(uri, trustPath, strlen(trustPath)) == 0);
    if (isTrustPath) {
        size = 0;
    }
    ...
    if (sprintf_s(fullPath, len, "%s%s", (isTrustPath ? "" : dataPath), uri + size) < 0) {
        return ERROR_CODE_GENERAL;
    }
