...
/* osMessageQueuePut API Timeout value. */
#define osNoWait            0x0U

#define sys_sem_valid(x)        (((*x) == NULL) ? 0 : 1)
#define sys_sem_set_invalid(x)  ((*x) = NULL)

#define OS_OK      OSAL_OK
#define OS_NOK     OSAL_NOK

/**
 * Type by which queues are referenced.  For example, a call to xQueueCreate() returns an QueueHandle_t variable
 * that can then be used as a parameter to xQueueSend(), xQueueReceive(), etc.
 */
typedef unsigned int sys_thread_t;
...
typedef osMutexId_t sys_prot_t;
