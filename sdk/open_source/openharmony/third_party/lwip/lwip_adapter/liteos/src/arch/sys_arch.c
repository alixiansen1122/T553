...
/* Return code for Error from sys_thread_new and sys_arch_sem_wait */
...
void sys_thread_delete(void *param)
{
    if (param == NULL) {
      return;
    }
    struct sys_arch_thread *threadCtl = (struct sys_arch_thread *)param;
    LOS_TaskJoin(threadCtl->taskId, NULL);

    if (threadCtl->stackMem != NULL) {
      free(threadCtl->stackMem);
      threadCtl->stackMem = NULL;
    }
    free(threadCtl);
}
...
sys_thread_t sys_thread_new(const char *name, lwip_thread_fn function, void *arg, int stacksize, int prio)
{
  ...
  struct sys_arch_thread *param = calloc(1, sizeof(struct sys_arch_thread));
  if (param == NULL) {
    return 0;
  }
  param->arg = arg;
  ...
  task.uwResved   = LOS_TASK_STATUS_JOINABLE;
...
#endif
#ifdef LOSCFG_TASK_STACK_STATIC_ALLOCATION
  stackMem = memalign(LOSCFG_STACK_POINT_ALIGN_SIZE, task.uwStackSize);
  if (stackMem == NULL) {
    LWIP_DEBUGF(SYS_DEBUG, ("sys_thread_new: memalign fail\r\n"));
    free(param);
    return 0;
  }
  param->stackMem = stackMem;
  ret = LOS_TaskCreateStatic(&taskId, &task, stackMem);
#else
