...
#include "stdint.h"
#include "app_memory_heap.h"

#include "generate-bytecode.h"

namespace OHOS {
class GtBundleInstaller : public AppMemoryHeap {
public:
    GtBundleInstaller() = default;
    ~GtBundleInstaller() = default;

    uint8_t Install(const char *path, InstallerCallback installerCallback);
    uint8_t Install(const char *path, const InstallParam *installParam, InstallerCallback installerCallback);
...
private:
    ...
    uint8_t ProcessBundleInstall(const char *path, const char *randStr, InstallRecord &installRecord,
        uint8_t bundleStyle, InstallerCallback installerCallback, bool &isUpDate, bool keepData);
    uint8_t HandleFileAndBackUpRecord(const InstallRecord &record, const char *tmpPath, const char *randStr,
        const char *dataPath, bool isUpDate, bool keepData);
    ...
    uint8_t TransformJsToBc(const char *codePath, InstallRecord &record, char *jsPath);
};
