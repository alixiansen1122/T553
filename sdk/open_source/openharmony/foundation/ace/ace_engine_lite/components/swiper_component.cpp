      ...
      changeListener_(nullptr)
{
    ...
    swiperView_.SetIntercept(true);
    RegisterNamedFunction(OHOS::ACELite::Component::FUNC_SCROLL, SwiperScroll);
...
JSValue SwiperComponent::SwiperScroll(
    const JSValue func, const JSValue dom, const JSValue args[], const jerry_length_t size)
{
    UIView *swipeView = GetScrollView(dom, size);
    if (swipeView == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "SiderScroll swipeView is nullptr");
        return UNDEFINED;
    }

    int16_t rotate = JerryGetIntegerProperty(args[0], OHOS::ACELite::Component::ROTATE_SCROLL);
    swipeView->OnRotateStartEvent(rotate);
    swipeView->OnRotateEvent(rotate);
    swipeView->OnRotateEndEvent(0);
