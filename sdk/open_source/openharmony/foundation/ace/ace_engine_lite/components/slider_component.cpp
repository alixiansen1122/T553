      ...
      direction_(UISlider::Direction::DIR_LEFT_TO_RIGHT)
{
    ...
    sliderView_.SetIntercept(true);
    RegisterNamedFunction(OHOS::ACELite::Component::FUNC_SCROLL, SiderScroll);
...
JSValue SliderComponent::SiderScroll(
    const JSValue func, const JSValue dom, const JSValue args[], const jerry_length_t size)
{
    UIView *slider = GetScrollView(dom, size);
    if (slider == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "SiderScroll slider is nullptr");
        return UNDEFINED;
    }

    int16_t rotate = JerryGetIntegerProperty(args[0], OHOS::ACELite::Component::ROTATE_SCROLL);
    slider->OnRotateStartEvent(rotate);
    slider->OnRotateEvent(rotate);
    slider->OnRotateEndEvent(0);
