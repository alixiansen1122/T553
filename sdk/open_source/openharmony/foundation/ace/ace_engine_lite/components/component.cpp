...
const char * const Component::ROTATE_SCROLL = "rotation";
const char * const Component::FUNC_SCROLL = "scroll";
const char * const Component::FUNC_GETX = "getX";
const char * const Component::FUNC_GETY = "getY";
      ...
      marginBottom_(-1, DimensionType::TYPE_UNKNOWN)
{
    ...
    RegisterNamedFunction(OHOS::ACELite::Component::FUNC_GETX, GetX);
    RegisterNamedFunction(OHOS::ACELite::Component::FUNC_GETY, GetY);
}
...
void Component::AdapteBoxRectArea(UIView &uiView) const
{
    ...
    const_cast<Component*>(this)->RefreshRectCallback();
}
...
void Component::SetViewOnTouchListener(UIView &view)
{
    if (view.GetTouchListener() != nullptr) {
        onTouchListener_ = dynamic_cast<ViewOnTouchListener*>(view.GetTouchListener());
    } else {
        onTouchListener_ = new ViewOnTouchListener();
    }
}
...
void Component::SetTouchStartEventListener(UIView &view, jerry_value_t eventFunc, uint16_t eventTypeId)
{
    ...
    onTouchListener_->SetOnTouchStartListener(onTouchStartListener_);
...
void Component::SetTouchMoveEventListener(UIView &view, jerry_value_t eventFunc, uint16_t eventTypeId)
{
    ...
    SetViewOnTouchListener(view);
    onTouchListener_->SetOnTouchMoveListener(onTouchMoveListener_);
...
void Component::SetTouchCancelEventListener(UIView &view, jerry_value_t eventFunc, uint16_t eventTypeId)
{
    ...
    SetViewOnTouchListener(view);
    onTouchListener_->SetOnTouchCancelListener(onTouchCancelListener_);
    view.SetOnTouchListener(onTouchListener_);
    view.SetTouchable(true);
    view.SetDraggable(true);
}
...
void Component::SetTouchEndEventListener(UIView &view, jerry_value_t eventFunc, uint16_t eventTypeId)
{
    ...
    SetViewOnTouchListener(view);
    onTouchListener_->SetOnTouchEndListener(onTouchEndListener_);
    view.SetOnTouchListener(onTouchListener_);
    view.SetTouchable(true);
}
...
void Component::SetKeyInputEventListener(UIView &view, jerry_value_t eventFunc, uint16_t eventTypeId)
{
    RootView *rootView = RootView::GetInstance();
    if (rootView == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "get rootView is nullptr");
    ...
    onKeyInputListener_ = new KeyInputEventListener(eventFunc, eventTypeId);
    if (onKeyInputListener_ == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "on key input event listener create failed");
        return;
    }
    rootView->SetOnKeyActListener(onKeyInputListener_);
}
...
void Component::SetRotateEventListener(UIView& view, jerry_value_t eventFunc)
{
    onRotateListener_ = new ViewOnRotateListener(eventFunc);
    if (onRotateListener_ == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "rotate listener create failed");
        return;
    }
    view.SetOnRotateListener(onRotateListener_);
}
                                            ...
                                            bool isStopPropagation)
{
    switch (eventTypeId) {
        ...
        case K_ROTATE_INPUT: {
            SetRotateEventListener(view, funcValue);
            break;
        }
        case K_SWIPE: {
    ...
    // Implemented in KeyInput Module
    //SetKeyInputEventListener(view, funcValue, eventTypeId);
    return true;
}
...
void Component::ReleaseCommonEventListeners()
{
    // Implemented in KeyInput Module
    //RootView *rootView = RootView::GetInstance();
    //if (rootView != nullptr && rootView->GetOnKeyActListener() == onKeyInputListener_) {
    //    rootView->ClearOnKeyActListener();
    //}
    //ACE_DELETE(onKeyInputListener_);
...
UIView *Component::GetScrollView(const JSValue dom, const jerry_length_t size)
{
    if (size != 1) {
        HILOG_ERROR(HILOG_MODULE_ACE, "GetScrollView function parameter error");
        return nullptr;
    }

    UIView *uiview = ComponentUtils::GetViewFromBindingObject(dom);
    if (uiview == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "GetScrollView uiview is nullptr");
        return nullptr;
    }

    UIView *focusview = FocusManager::GetInstance()->GetFocusedView();
    if (uiview == focusview) {
        HILOG_DEBUG(HILOG_MODULE_ACE, "GetScrollView uiview has focus");
        return nullptr;
    }
...
Component::ViewOnTouchListener::ViewOnTouchListener()
    : onTouchStartListener_(nullptr),
      onTouchCancelListener_(nullptr),
      onTouchEndListener_(nullptr),
      onTouchMoveListener_(nullptr)
...
void Component::ViewOnTouchListener::SetOnTouchStartListener(ViewOnTouchStartListener *startListener)
{
    onTouchStartListener_ = startListener;
}

void Component::ViewOnTouchListener::SetOnTouchEndListener(ViewOnTouchEndListener *endListener)
{
    onTouchEndListener_ = endListener;
}

void Component::ViewOnTouchListener::SetOnTouchCancelListener(ViewOnTouchCancelListener *cancelListener)
{
    onTouchCancelListener_ = cancelListener;
}

void Component::ViewOnTouchListener::SetOnTouchMoveListener(ViewOnTouchMoveListener *moveListener)
{
    onTouchMoveListener_ = moveListener;
}

bool Component::ViewOnTouchListener::OnPress(UIView &view, const PressEvent &event)
{
    bool ret = false;
    if (onTouchStartListener_ != nullptr) {
        ret = onTouchStartListener_->OnPress(view, event);
    }
    return ret;
}

bool Component::ViewOnTouchListener::OnRelease(UIView &view, const ReleaseEvent &event)
{
    bool ret = false;
    if (onTouchEndListener_ != nullptr) {
        ret = onTouchEndListener_->OnRelease(view, event);
    }
    return ret;
}

bool Component::ViewOnTouchListener::OnCancel(UIView &view, const CancelEvent &event)
{
    bool ret = false;
    if (onTouchCancelListener_ != nullptr) {
        ret = onTouchCancelListener_->OnCancel(view, event);
    }
    return ret;
};

bool Component::ViewOnTouchListener::OnDrag(UIView &view, const DragEvent &event)
{
    bool ret = false;
    if (onTouchMoveListener_ != nullptr) {
        ret = onTouchMoveListener_->OnDrag(view, event);
    }
    return ret;
}

bool Component::ViewOnTouchListener::OnDragEnd(UIView &view, const DragEvent &event)
{
    bool ret = false;
    if (onTouchEndListener_ != nullptr) {
        ReleaseEvent evtRelease(event.GetLastPoint());
        ret = onTouchEndListener_->OnRelease(view, evtRelease);
    }
    return ret;
}

JSValue Component::GetX(const JSValue func, const JSValue dom, const JSValue args[], const jerry_length_t size)
{
    UIView *view = reinterpret_cast<UIView *>(ComponentUtils::GetViewFromBindingObject(dom));
    if (view == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "failed to get globalX. view is nullptr");
        return -1;
    }
    double x = static_cast<double>(view->GetOrigRect().GetX());
    return jerry_create_number(x);
}

JSValue Component::GetY(const JSValue func, const JSValue dom, const JSValue args[], const jerry_length_t size)
{
    UIView *view = reinterpret_cast<UIView *>(ComponentUtils::GetViewFromBindingObject(dom));
    if (view == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "failed to get globalY. view is nullptr");
        return -1;
    }
    double y = static_cast<double>(view->GetOrigRect().GetY());
    return jerry_create_number(y);
}
