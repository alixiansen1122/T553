...
int16_t ListAdapter::CalculateItemIndex(int16_t itemIndex) const
{
    ...
    // ohos begin
    // list-item adaptive
    if (ListAdapterExt::GetItemAdaptive()) {
        return 0;
    }
    // ohos end
...
UIView *ListAdapter::GetView(UIView *inView, int16_t index)
{
    ...
    // ohos begin
    // list-item adaptive
    if (ListAdapterExt::GetItemAdaptive() && index >= itemsCount_) {
        return nullptr;
    }
    // ohos end
...
JSValue ListAdapter::GetElement(int16_t listItemsIndex, int16_t index, bool &isFor)
{
    ...
    if (isFor) {
        ...
        // list-item adaptive
        JSValue item =
            jerry_get_property_by_index(getterRetList, (ListAdapterExt::GetItemAdaptive() ? 0 : index) - startIndex);
        // ohos end
        ...
        // ohos begin
        // list-item adaptive
        JSValue itemIdx = jerry_create_number((ListAdapterExt::GetItemAdaptive() ? 0 : index) - startIndex);
        // ohos end
    ...
    // ohos begin
    // list-item adaptive
    ListAdapterExt::SetItemIndex(index);
    // ohos end
    BuildItemViewTree(element);
    return element;
}

void ListAdapter::BuildItemViewTree(const JSValue element)
{
    ...
    //text componet adapts to height and width in list-item
    ...
    if (direction_ == UIList::VERTICAL) {
        if (component->GetDimension(K_HEIGHT).type == DimensionType::TYPE_UNKNOWN) {
            JSValue children = component->GetChildren();
            uint16_t length = JSArray::Length(children);

            for (uint16_t idx = 0; idx < length; ++idx) {
                JSValue key = JSArray::Get(children, idx);

                Component *chidrenComponent = ComponentUtils::GetComponentFromBindingObject(key);

                if (chidrenComponent->GetComponentName() == K_TEXT) {
                    TextComponent* textComponent = reinterpret_cast<TextComponent *>(chidrenComponent);
                    UILabel *uiLabel = textComponent->GetUILabelView();
                    int16_t chidrenComponentHeight = uiLabel->GetHeight();
                    allh += chidrenComponentHeight;
                } else {
                    int16_t chidrenComponentHeight = chidrenComponent->GetHeight();
                    allh += chidrenComponentHeight;
                }
                jerry_release_value(key);
            }

            component->SetHeight(allh);
            component->AdaptBoxSizing();
        ...
        if (component->GetDimension(K_WIDTH).type == DimensionType::TYPE_UNKNOWN) {
            ...
            for (uint16_t idx = 0; idx < length; ++idx) {
                ...
                if (chidrenComponent->GetComponentName() == K_TEXT) {
                    TextComponent* textComponent = reinterpret_cast<TextComponent *>(chidrenComponent);
                    UILabel *uiLabel = textComponent->GetUILabelView();
                    int16_t chidrenComponentWidth = uiLabel->GetWidth();
                    allw += chidrenComponentWidth;
                } else {
                    int16_t chidrenComponentWidth = chidrenComponent->GetWidth();
                    allw += chidrenComponentWidth;
                }
                jerry_release_value(key);
            }

            component->SetWidth(allw);
            component->AdaptBoxSizing();
            component->LayoutChildren();
        }
    }
    ...
    // list-item adaptive
    if (ListAdapterExt::GetItemAdaptive()) {
        int16_t itemsCount = ListAdapterExt::BuildItemAdaptive(component, ListAdapterExt::GetItemIndex());
        itemsCount_ = itemsCount > 0 ? itemsCount : itemsCount_;
    }
    // ohos end
