...
UITextLanguageAlignment align_horizontal = UITextLanguageAlignment::TEXT_ALIGNMENT_LEFT;
UITextLanguageAlignment align_vertical = UITextLanguageAlignment::TEXT_ALIGNMENT_TOP;
...
bool TextComponent::SetPrivateAttribute(uint16_t attrKeyId, jerry_value_t attrValue)
{
    switch (attrKeyId) {
        ...
        case K_ORIENTATION:
        SetTextRotateAngle(uiLabel_, attrValue);
        break;
        default: {
            return false;
        }
...
bool TextComponent::ApplyPrivateStyle(const AppStyleItem *styleItem)
{
    switch (GetStylePropNameId(styleItem)) {
        ...
        case K_LETTER_SPACING:
            ...
            break;
        case K_TEXT_ALIGN_VERTICAL:
            SetTextAlignVertical(uiLabel_, styleItem);
            break;
        case K_TEXT_ALIGN:
...
void TextComponent::SetTextRotateAngle(UILabel &label, jerry_value_t attrValue)
{
    char *angle = MallocStringOf(attrValue);
    if (!strcmp(angle, "up")) {
        rotateAngle_ = 360;
    } else if (!strcmp(angle, "down")) {
        rotateAngle_ = 180;
    } else if (!strcmp(angle, "left")) {
        rotateAngle_ = 270;
    } else if (!strcmp(angle, "right")) {
        rotateAngle_ = 90;
    }
    ACE_FREE(angle);
    label.Rotate(rotateAngle_, Vector2<float>(label.GetWidth()/2, label.GetHeight()/2));
}
...
void TextComponent::SetTextAlignVertical(UILabel &label, const AppStyleItem *styleItem) const
{
    if (!IsStyleValueTypeString(styleItem)) {
        HILOG_ERROR(HILOG_MODULE_ACE, "text text vertical align style value is invalid!");
        return;
    }
    ...
    switch (alignId) {
        case K_CENTER:
            align_vertical = UITextLanguageAlignment::TEXT_ALIGNMENT_CENTER;
            break;
        case K_BOTTOM:
            align_vertical = UITextLanguageAlignment::TEXT_ALIGNMENT_BOTTOM;
            break;
        case K_TOP:
            align_vertical = UITextLanguageAlignment::TEXT_ALIGNMENT_TOP;
            break;
        default:
            HILOG_WARN(HILOG_MODULE_ACE, "text vertical textAlign style value =%s is invalid, using default instead",
                       stylePropValue);
            break;
    }
    label.SetAlign(align_horizontal, align_vertical);
}

void TextComponent::SetTextAlign(UILabel &label, const AppStyleItem *styleItem) const
{
    if (!IsStyleValueTypeString(styleItem)) {
        HILOG_ERROR(HILOG_MODULE_ACE, "text text horizontal align style value is invalid!");
        return;
    }
    ...
    switch (alignId) {
        case K_LEFT:
            align_horizontal = UITextLanguageAlignment::TEXT_ALIGNMENT_LEFT;
            break;
        case K_RIGHT:
            align_horizontal = UITextLanguageAlignment::TEXT_ALIGNMENT_RIGHT;
            break;
        case K_CENTER:
            align_horizontal = UITextLanguageAlignment::TEXT_ALIGNMENT_CENTER;
            break;
        default:
            HILOG_WARN(HILOG_MODULE_ACE, "text horizontal textAlign style value =%s is invalid, using default instead",
                       stylePropValue);
            break;
    }
    label.SetAlign(align_horizontal, align_vertical);
