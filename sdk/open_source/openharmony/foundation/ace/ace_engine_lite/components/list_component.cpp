...
const char * const ListComponent::FUNC_SCROLLTO_END = "scrollToEnd";
static uint16_t itemslenght = 0;
    ...
    : Component(options, children, styleManager)
{
    ...
    RegisterNamedFunction(FUNC_SCROLLTO_END, ListScrollToEnd);
    RegisterNamedFunction(OHOS::ACELite::Component::FUNC_SCROLL, ListScroll);
...
{
    if (attrKeyId == K_SCROLLBAR) {
        if (jerry_value_is_boolean(attrValue)) {
            list_.SetYScrollBarVisible(BoolOf(attrValue));
        } else {
            list_.SetYScrollBarVisible(false);
        }
        return true;
    }  else if (attrKeyId == K_LONG_TEXT) {
        if (jerry_value_is_boolean(attrValue)) {
            ListAdapterExt::SetItemAdaptive(BoolOf(attrValue));
        }
        return true;
    } else if (attrKeyId == K_VALUE) {
        if (jerry_value_is_string(attrValue)) {
            ...
            ListAdapterExt::SetListValue(value);
            ace_free(value);
        }
        return true;
    }
...
bool ListComponent::UpdateForView()
{
    ...
    list_.RefreshList();
    // refresh list , recompute list height
    ComputeListLength();
    return true;
}
...
void ListComponent::ComputeListLength()
{
    itemslenght = 0;
    for (uint16_t index = 0; index < adapter_.GetCount(); index++) {
        UIView *view = adapter_.GetView(nullptr, index);
        if (view != nullptr) {
            if (adapter_.GetDirection() == UIList::VERTICAL) {
                itemslenght += view->GetHeightWithMargin();
            } else {
                itemslenght += view->GetWidthWithMargin();
            }
            adapter_.DeleteItem(view);
        } else {
            HILOG_ERROR(HILOG_MODULE_ACE, "List compent: ComputeListLength  item view is null ");
        }
    }
}

JSValue ListComponent::ListScrollToEnd(
    const JSValue func, const JSValue dom, const JSValue args[], const jerry_length_t size)
{
    UIList *list = reinterpret_cast<UIList *>(ComponentUtils::GetViewFromBindingObject(dom));
    if (list != nullptr) {
        if (itemslenght > list->GetHeight()) {
            uint16_t listlenght = itemslenght - list->GetHeight();
            list->ScrollBy(-listlenght);
        }
    }
    return UNDEFINED;
}

JSValue ListComponent::ListScroll(
    const JSValue func, const JSValue dom, const JSValue args[], const jerry_length_t size)
{
    UIView *list = GetScrollView(dom, size);
    if (list == nullptr) {
        HILOG_ERROR(HILOG_MODULE_ACE, "ListScroll list is nullptr");
        return UNDEFINED;
    }
