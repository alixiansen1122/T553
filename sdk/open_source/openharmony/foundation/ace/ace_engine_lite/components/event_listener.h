...
class KeyInputEventListener final : public RootView::OnKeyActListener {
public:
    ACE_DISALLOW_COPY_AND_MOVE(KeyInputEventListener);
    KeyInputEventListener(jerry_value_t fn, const uint16_t id);
    ~KeyInputEventListener();
    bool OnKeyAct(UIView &view, const KeyEvent &event) override;
private:
...
class ViewOnRotateListener final : public UIView::OnRotateListener {
public:
    ACE_DISALLOW_COPY_AND_MOVE(ViewOnRotateListener);
    ViewOnRotateListener(jerry_value_t fn);

    ~ViewOnRotateListener() override;

    bool OnRotateStart(UIView& view, const RotateEvent& event) override;
    bool OnRotate(UIView& view, const RotateEvent& event) override;
    bool OnRotateEnd(UIView& view, const RotateEvent& event) override;
private:
    jerry_value_t fn_;
};
