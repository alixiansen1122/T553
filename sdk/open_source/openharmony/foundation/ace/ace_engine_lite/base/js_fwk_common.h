...
constexpr uint32_t FILE_CONTENT_LENGTH_MAX = 1024 * 128;
...
constexpr uint8_t FONT_FAMILY_FILE_NAME_SIZE = 128;
...
jerry_value_t ListForWatcherCallbackFunc(const jerry_value_t func,
                                         ...
                                         const jerry_length_t argsLength);
#ifdef JS_TOUCH_EVENT_SUPPORT
jerry_value_t *ConvertBaseEventInfo(const Event &event, const uint16_t id, UIView &view);
bool CallBaseEvent(const jerry_value_t func, const Event &event, const uint16_t id, UIView &view);
jerry_value_t *ConvertDragEventInfo(const DragEvent &event, const uint16_t id, UIView &view);
jerry_value_t *ConvertKeyEventInfo(const KeyEvent &event);
#endif
void ClearEventListener(const jerry_value_t args[], uint8_t argc);
bool DumpPrintMemoryInfo();
jerry_value_t *ConvertRotateEventInfo(const RotateEvent &event);
...
const char *ParseImageSrcExt(jerry_value_t source);
/**
