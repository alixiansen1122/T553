...
private:
    ...
    static const char * const KEY_APP_ID;
...
/**
 * @brief set terminate wrapper
 */
void TerminateWrapper(uint32_t token, bool forceStop)
{
    (void)forceStop;
    uint16_t preSliceId = NativeAbility::GetInstance().GetPreSliceId();
    if (preSliceId == VIEW_APPLIST) {
        if (SettingDesktopModel::GetInstance().GetDesktopStyle() == HEXAGONS_STYLE) {
            AbilityMsClient::GetInstance().TerminateAbility(token, TransitionType::TRANSITION_BACK_TO_HEXAGONS);
            return;
        } else if (SettingDesktopModel::GetInstance().GetDesktopStyle() == WATERFALL_STYLE) {
            AbilityMsClient::GetInstance().TerminateAbility(token, TransitionType::TRANSITION_BACK_TO_WATERFALL);
            return;
        }
    }
    AbilityMsClient::GetInstance().TerminateAbility(token);
    return;
}

void InitAppModule(JSIValue exports)
{
    // set terminate handler for app.terminate.
    ProductAdapter::RegTerminatingHandler(TerminateWrapper);
