...
const char * const AppModule::KEY_APP_ID = "appID";
...
JSIValue AppModule::GetInfo(const JSIValue thisVal, const JSIValue *args, uint8_t argsNum)
{
    ...
    cJSON *appID = cJSON_GetObjectItem(manifest, KEY_APP_ID);
    ...
    if (appID != nullptr && appID->type == cJSON_String) {
        BundleInfo bundleInfo;
        uint8_t ret = GetBundleInfo(appID->valuestring, true, &bundleInfo);
        if (ret == 0) {
            JSI::SetStringProperty(result, KEY_APP_NAME, bundleInfo.label);
        }
    }
