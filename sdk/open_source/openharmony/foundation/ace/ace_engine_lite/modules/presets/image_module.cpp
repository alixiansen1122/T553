                                       ...
                                       const jerry_length_t argsNum)
{
    ...
    RegisterAttributeFunc(context, attrOnload, OnLoadSetter, OnLoadGetter);
                                        ...
                                        const jerry_length_t argsNum)
{
    ...
    if (argsNum == 1 && jerry_value_is_function(args[0])) {
        ...
        imageModel->onLoadFunc_ = jerry_acquire_value(args[0]);
    } else {
        return jerry_create_error(JERRY_ERROR_EVAL,
                                         ...
                                         const jerry_length_t argsNum)
{
    ...
    if (argsNum >= 1 && jerry_value_is_function(args[0])) {
        ...
        imageModel->onErrorFunc_ = jerry_acquire_value(args[0]);
    } else {
        return jerry_create_error(JERRY_ERROR_EVAL,
                                  reinterpret_cast<const jerry_char_t *>("set imageModel onload arg fail"));
    }
                                       ...
                                       const jerry_length_t argsNum)
{
    ...
    if (argsNum >= 1) {
        ...
        imageModel->src_ = const_cast<char *>(ParseImageSrc(args[0]));
    }
    ...
    ImageInfo* image = ImageCacheManager::GetInstance().LoadSingleRes(imageModel->src_);
    if (!image) {
