...
public:
    ...
    {
        if (timerList_ != nullptr) {
            timerList_->ClearTimerListGlobal();
            delete (timerList_);
            timerList_ = nullptr;
        }
    }
...
private:
    ...
    {
        return CreateTimer(func, context, args, argsNum, false, false);
    }

    static jerry_value_t SetAppGlobalTimeout(const jerry_value_t func,
                                    const jerry_value_t context,
                                    const jerry_value_t *args,
                                    const jerry_length_t argsNum)
    {
        return CreateTimer(func, context, args, argsNum, false, true);
    }
    /**
    ...
    static jerry_value_t SetInterval(const jerry_value_t func,
                                     ...
                                     const jerry_length_t argsNum)
    {
        return CreateTimer(func, context, args, argsNum, true, false);
    }

    static jerry_value_t SetAppGlobalInterval(const jerry_value_t func,
                                     const jerry_value_t context,
                                     const jerry_value_t *args,
                                     const jerry_length_t argsNum)
    {
        return CreateTimer(func, context, args, argsNum, true, true);
    }
    /**
     * @brief set the repeated timer
     * @return the timer id
