...
bool CJSONParser::ChangeLanguageFileName()
{
    ...
    if ((languageFile_ != nullptr) && (!strcmp(languageFile, languageFile_))) {
        ...
        char *fullPath = RelocateJSSourceFilePath(filePath_, languageFile);
        if (!IsFileExisted(fullPath)) {
            ACE_FREE(fullPath);
            // If there is no specified language file, base.json will be used by default
            fullPath = RelocateJSSourceFilePath(filePath_, "base.json");
            ACE_FREE(languageFile);
            if (IsFileExisted(fullPath)) {
                // If there is no specified language file, base.json will be used by default
                languageFile = StringUtil::Copy("base.json");
            } else {
                // If there is no specified language file base.json , en-US.json will be used by default
                languageFile = StringUtil::Copy("en-US.json");
            }
        }
        ACE_FREE(fullPath);
