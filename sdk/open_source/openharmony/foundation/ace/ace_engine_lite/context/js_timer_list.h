...
public:
    ...
    /**
     * @brief Get the next available index 0~255 for timer.
     * @return int16_t. Returns the unused index. if there is no index, -1 is returned.
     */
    int16_t GetIndex()
    {
        uint8_t loopTimes = 0;
        index_++;
        while (GetTimer(index_) != nullptr) {
            if (loopTimes >= UINT8_MAX) {
                return -1;
            }
            // index_ = (index_ + 1) % 256;
            index_++;
