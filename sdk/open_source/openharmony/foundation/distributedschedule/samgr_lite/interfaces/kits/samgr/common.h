...
#define VECTOR_MAKE(key, compare) {0, 0, 0, NULL, (key), (compare)}
...
int16 VECTOR_Capacity(Vector *vector);
...
/**
 * @brief Traverse the elements of vector and execute lambda.
 *
 * This function is used to traverse the elements. \n
 *
 * @param vector Indicates the <b>this</b> pointer to the vector.
 * @param lambda the process action for each element. the data argument is the element.
 * @param args the first argument of the lambda.
 * @return The number of elements traversed; {@link INVALID_INDEX} if the arguments check fails.
 * @attention The return value of lambda FALSE will interrupt the traversal.
 * @since 1.0
 * @version 1.0
 */
 #if !defined(_WIN32)
int16 VECTOR_ForEach(Vector *vector, boolean(*lambda)(void *args, void *data), void *args);
#else
int16 VECTOR_ForEach(Vector *vector, int(*lambda)(void *args, void *data), void *args);
#endif
