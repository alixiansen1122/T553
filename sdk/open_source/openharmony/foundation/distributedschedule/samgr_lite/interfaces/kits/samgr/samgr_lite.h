...
typedef enum BootMessage {
    ...
    BOOT_SYS_COMPLETED,
    /** Message indicating that the system-layer services are initialized */
    BOOT_SYSEX_COMPLETED,
    /** Message indicating that the application-layer services are initialized */
    BOOT_APP_COMPLETED,
    /** Message indicating service registration during running */
    BOOT_REG_SERVICE,
    /** Message indicating start the boot init time */
    BOOT_INIT_TIME,
...
typedef struct SamgrLite {
    ...
    IUnknown *(*GetDefaultFeatureApiById)(uint16 serviceId);
} SamgrLite;
