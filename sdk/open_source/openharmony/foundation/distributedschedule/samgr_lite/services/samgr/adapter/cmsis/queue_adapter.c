...
boolean QUEUE_IsEmpty(MQueueId queueId)
{
    return osMessageQueueGetCount(queueId) == 0;
}
...
int QUEUE_Clean(MQueueId queueId, QUEUE_FreeItem free)
{
    if (queueId == NULL || free == NULL) {
        return EC_SUCCESS;
    }
    if (osMessageQueueGetMsgSize(queueId) > MAX_ITEM_SIZE) {
        return EC_INVALID;
    }

    uint8 item[MAX_ITEM_SIZE];
    while (osMessageQueueGetCount(queueId) > 0) {
        int32 ret = osMessageQueueGet(queueId, item, NULL, osNoWait);
        if (ret != osOK) {
            return EC_FAILURE;
        }
        free(item);
