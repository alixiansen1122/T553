...
void IUNKNOWN_SetIdentity(IUnknown* iUnknown, const Identity* identity)
{
    if (iUnknown == NULL || identity == NULL) {
        return;
    }
    IUnknownEntry* entry = GET_OBJECT((iUnknown), IUnknownEntry, iUnknown);
    if ((entry->ver & DEFAULT_VERSION) == DEFAULT_VERSION) {
        entry->id = *identity;
    }
}

const Identity* IUNKNOWN_GetIdentity(IUnknown* iUnknown)
{
    if (iUnknown == NULL) {
        return NULL;
    }
