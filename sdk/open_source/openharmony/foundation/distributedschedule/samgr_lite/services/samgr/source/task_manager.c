...
TaskPool *SAMGR_CreateFixedTaskPool(
    const TaskConfig *config, const char *name, uint8 size) {
...
//xx00        osThreadAttr_t taskAttr = {name, 0, NULL, 0, NULL, pool->stackSize, pool->priority, 0, 0};
        register ThreadId threadId = (ThreadId)THREAD_Create(TaskEntry, pool->queueId, &attr);
//xx00		osal_task *task = osal_kthread_create(TaskEntry, NULL, name, 0x1000);
...
static void EndWork(ServiceImpl *service, const Exchange *exchange) {
	...
	const char *name = service->service->GetName(service->service);
        SAMGR_LOG_ERROR("Message Timeout <service:%s, feature:%d, type:%d, reqId:%d, time:%ums>",
