...
public:
    ...
    int StartAbility(const Want *want, TransitionType type, bool enableSlideBack) const;

    int TerminateAbility(uint64_t token) const;

    int TerminateAbility(uint64_t token, TransitionType type) const;
    ...
    int ForceStop(char *bundleName, TransitionType type) const;
