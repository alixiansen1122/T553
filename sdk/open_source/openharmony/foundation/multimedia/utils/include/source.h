...
public:
     ...
     */
    explicit Source(int32_t fd, uint64_t offset);
    /**
     * @brief A constructor used to create a {@link Source} instance based on a specified URI.
     *
    ...
    /**
     * A constructor used to create a {@code Source} instance based on a file descriptor, start offset, and source
     * data length.
     *
     * @param fd Indicates the file descriptor.
     * @param offset Indicates the start offset.
     * @param len Indicates the source data length.
     * @note Not support to play with fd-source currently
     */
    Source(int32_t fd, uint64_t offset, uint64_t len);
    /**
    ...
    ~Source() = default;
    /**
     * get the file descriptor of the the source, , if source type is SOURCE_TYPE_FD, see {@link SourceType}
     *
     * @return returns the file descriptor.
     */
    int32_t GetSourceFd() const;
    /**
    ...
    /**
     * get the offset of the local file, , if source type is SOURCE_TYPE_FD, see {@link SourceType}
     *
     * @return returns the offset of the first byte would been read.
     */
    uint64_t GetSourceFdOffset() const;
    /**
     * get the length of the local file, if source type is SOURCE_TYPE_FD, see {@link SourceType}.
     *
     * @return returns the file length
     */
    uint64_t GetSourceFdLength() const;
    /**
