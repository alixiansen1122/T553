...
void *GetServiceProxy(void)
{
    IUnknown *iUnknown = SAMGR_GetInstance()->GetDefaultFeatureApi(SENSOR_SERVICE);
    if (iUnknown == NULL) {
        HILOG_ERROR(HILOG_MODULE_SEN, "%s get sensor service failed", __func__);
        return NULL;
    }
    ...
    if ((result != 0) || (sensor == NULL)) {
        HILOG_ERROR(HILOG_MODULE_SEN, "%s query interface failed", __func__);
        return NULL;
    }
    HILOG_DEBUG(HILOG_MODULE_SEN, "query interface :%p", sensor);
    return sensor;
}
...
int32_t GetAllSensorsByProxy(const void *proxy, SensorInfo **sensorInfo, int32_t *count)
{
    if ((proxy == NULL) || (count == NULL)) {
        HILOG_ERROR(HILOG_MODULE_SEN, "%s proxy or count is NULL", __func__);
...
{
    if (proxy == NULL) {
        HILOG_ERROR(HILOG_MODULE_SEN, "%s proxy is NULL", __func__);
        return SENSOR_ERROR_INVALID_PARAM;
    }
    SensorFeatureApi *sensor = (SensorFeatureApi *)proxy;
    return sensor->ActivateSensor(sensorId, sensorUser);
}
...
int32_t DeactivateSensorByProxy(const void *proxy, int32_t sensorId, const SensorUser *sensorUser)
{
    ...
    SensorFeatureApi *sensor = (SensorFeatureApi *)proxy;
    return sensor->DeactivateSensor(sensorId, sensorUser);
}
...
// ohos-hisi begin
int32_t GetSensorDataByProxy(const void *proxy, int32_t sensorId, const SensorUser *user, void *sensorData)
{
    ...
    SensorFeatureApi *sensor = (SensorFeatureApi *)proxy;
    return sensor->GetData(sensorId, user, sensorData);
}
// ohos-hisi end