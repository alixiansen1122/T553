...
{
    LBSLOGI(LOCATOR_GNSS, "OnReceiveEvent");
    ...
    if (action == CommonEventSupport::COMMON_EVENT_SMS_RECEIVE_COMPLETED) {
        LBSLOGI(LOCATOR_GNSS, "[AGNSS NI]:receive sms msg");
        agnssNiManager->CheckSmsSuplInit(want);
    } else if (action == CommonEventSupport::COMMON_EVENT_SMS_WAPPUSH_RECEIVE_COMPLETED) {
        LBSLOGI(LOCATOR_GNSS, "[AGNSS NI]:receive wap push msg");
        agnssNiManager->CheckWapSuplInit(want);
    } else if (action == CommonEventSupport::COMMON_EVENT_CALL_STATE_CHANGED) {
        LBSLOGI(LOCATOR_GNSS, "[AGNSS NI]:receive call state msg");
        agnssNiManager->OnCallStateChanged(want);
    } else if (action == AGNSS_NI_ACCEPT_EVENT) {
        LBSLOGI(LOCATOR_GNSS, "[AGNSS NI]:receive user accept event");
        agnssNiManager->SendUserResponse(LOCATOR_GNSS_NI_RESPONSE_CMD_ACCEPT);
    } else if (action == AGNSS_NI_REJECT_EVENT) {
        LBSLOGI(LOCATOR_GNSS, "[AGNSS NI]:receive user reject event");
        agnssNiManager->SendUserResponse(LOCATOR_GNSS_NI_RESPONSE_CMD_REJECT);
    }
}
} // namespace Location
} // namespace OHOS
