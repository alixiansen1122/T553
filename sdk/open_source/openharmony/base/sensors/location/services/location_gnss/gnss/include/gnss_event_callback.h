...
private:
    ...
    bool IsSvTypeGps(const std::shared_ptr<SatelliteStatus> &sv, int index);
    bool IsSvUsed(const std::shared_ptr<SatelliteStatus> &sv, int index);
    void AddDummySv(std::shared_ptr<SatelliteStatus> &sv, int svid, int cN0Dbhz);
    void ReportDummySv(const std::shared_ptr<SatelliteStatus> &sv);
};
}  // namespace Location
}  // namespace OHOS
