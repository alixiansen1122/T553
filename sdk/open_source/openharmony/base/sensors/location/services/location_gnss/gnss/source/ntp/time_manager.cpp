...
{
    LBSLOGI(LOCATOR_GNSS, "beginning timestamp_ is %s", std::to_string(timestamp_).c_str());
    if (timestamp_ < GPS_UTC_REFERENCE_TIME) {
        return INVALID_TIME;
    }
    if (!ElapsedRealTimeCheck::GetInstance()->CanTrustElapsedRealTime()) {
        LBSLOGI(LOCATOR_GNSS, "getCurrentTime ElapsedRealTime INVALID_TIME");
        return INVALID_TIME;
    }

    int64_t timeTillNow = MiscServices::TimeServiceClient::GetInstance()->GetBootTimeMs() - timeSynsBoot_;
    if (timeTillNow >= expireTime_) {
        LBSLOGI(LOCATOR_GNSS, "getCurrentTime INVALID_TIME");
        return INVALID_TIME;
    } else {
        LBSLOGI(LOCATOR_GNSS, "getCurrentTime:%s", std::to_string(timestamp_ + timeTillNow).c_str());
...
void TimeManager::SetCurrentTime(int64_t msTime, int64_t msTimeSynsBoot)
{
    ...
    LBSLOGI(LOCATOR_GNSS, "setCurrentTime timestamp_:%s,  mTimeReference:%s",
