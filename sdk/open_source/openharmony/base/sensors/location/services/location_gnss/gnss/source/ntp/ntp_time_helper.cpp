...
void NtpTimeHelper::RetrieveAndInjectNtpTime()
{
    if (ntpResult_.elapsedRealTime > 0) {
        ...
        LBSLOGD(LOCATOR_GNSS, "ntp age millis %s", std::to_string(ntpResult_.ageMillis).c_str());
    }
    ...
    if (ntpResult_.ntpTime == 0 || ntpResult_.ageMillis > NTP_INTERVAL) {
        ...
        if (ret != ERR_OK) {
            ...
            LBSLOGE(LOCATOR_GNSS, "failed to get ntp time %d", ret);
            return;
        }
        LBSLOGI(LOCATOR_GNSS, "get ntp time %s", std::to_string(ntpResult_.ntpTime).c_str());
