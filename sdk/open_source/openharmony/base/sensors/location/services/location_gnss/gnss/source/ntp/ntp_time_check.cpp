...
bool NtpTimeCheck::CheckNtpTime(int64_t ntpMsTime, int64_t msTimeSynsBoot)
{
    ...
    LBSLOGI(LOCATOR_GNSS, "checkNtpTime return false");
    return false;
}

bool NtpTimeCheck::CompareTime(int64_t currentNtpTime, int64_t compareTime)
{
    LBSLOGI(LOCATOR_GNSS, "compareTime currentNtpTime:%s, compareTime:%s",
    ...
    if (misstake > MISSTAKE_TIME) {
        LBSLOGI(LOCATOR_GNSS, "find error ntp time:%s", std::to_string(misstake).c_str());
        return false;
    } else {
...
void NtpTimeCheck::SetGpsTime(int64_t gpsMsTime, int64_t bootTimeMs)
{
    if (gpsMsTime <= 0 || bootTimeMs <= 0) {
        LBSLOGE(LOCATOR_GNSS, "set gps time failed :%s, %s", std::to_string(gpsMsTime).c_str(),
