...
{
    LBSLOGI(LOCATOR_GNSS, "AGNSS-NI SubscribeSaStatusChangeListerner");
    sptr<ISystemAbilityManager> samgrProxy = SystemAbilityManagerClient::GetInstance().GetSystemAbilityManager();
    if (samgrProxy == nullptr) {
        LBSLOGE(LOCATOR_GNSS, "failed to get samgrProxy");
        return;
    }
    statusChangeListener_ = new SystemAbilityStatusChangeListener();
    if (statusChangeListener_ == nullptr) {
        LBSLOGE(LOCATOR_GNSS, "statusChangeListener_ is nullptr!");
        return;
    }
    int32_t ret = samgrProxy->SubscribeSystemAbility(COMMON_EVENT_SERVICE_ID, statusChangeListener_);
    if (ret != ERR_OK) {
        LBSLOGE(LOCATOR_GNSS, "subscribe systemAbilityId: call manager service failed!");
        return;
    }
}
...
void AGnssNiManager::RegisterAgnssNiEvent()
{
    ...
    if (count > MAX_RETRY_TIMES || !result) {
        LBSLOGE(LOCATOR_GNSS, "Failed to subscriber gnss event");
    } else {
        LBSLOGI(LOCATOR_GNSS, "success to subscriber gnss event");
    }
}
...
void AGnssNiManager::RegisterNiResponseEvent()
{
    ...
    if (count > MAX_RETRY_TIMES || !result) {
        LBSLOGE(LOCATOR_GNSS, "Failed to subscriber ni response event");
    } else {
        LBSLOGI(LOCATOR_GNSS, "success to subscriber ni response event");
    }
}

void AGnssNiManager::Run()
{
    LBSLOGI(LOCATOR_GNSS, "AGNSS-NI: Run");
    ...
    if (gnssInterface_ == nullptr) {
        ...
        if (gnssAbility == nullptr) {
            LBSLOGE(LOCATOR_GNSS, "AGNSS-NI: gnss ability is nullptr");
            return;
        }
...
void AGnssNiManager::UnRegisterAgnssNiEvent()
{
    if (subscriber_ == nullptr) {
        LBSLOGE(LOCATOR_GNSS, "UnRegisterAgnssNiEvent subscriber_ is null");
        return;
    }
    bool result = CommonEventManager::UnSubscribeCommonEvent(subscriber_);
    subscriber_ = nullptr;
    LBSLOGI(LOCATOR_GNSS, "unSubscriber gnss event, result = %d", result);
}
...
void AGnssNiManager::UnRegisterNiResponseEvent()
{
    ...
    LBSLOGI(LOCATOR_GNSS, "unSubscriber ni response event, result = %d", result);
}
...
static bool IsFromDefaultSubId(const OHOS::EventFwk::Want &want)
{
    ...
    if (msgManager == nullptr) {
        LBSLOGE(LOCATOR_GNSS, "short message manager nullptr");
        return false;
    }
    int32_t defaultId = msgManager->GetDefaultSmsSlotId();
    LBSLOGD(LOCATOR_GNSS, "current subId %d, defaultSubId %d", subId, defaultId);
...
void AGnssNiManager::CheckWapSuplInit(const EventFwk::Want &want)
{
    ...
    if (!IsFromDefaultSubId(want)) {
        LBSLOGE(LOCATOR_GNSS, "supl init message does not come from default sub");
        return;
    }
    ...
    if (!(applicationId == "16" || applicationId == URN_APPLICATION_ID)) {
        LBSLOGE(LOCATOR_GNSS, "supl init message application Id %s", applicationId.c_str());
        return;
    }

    std::string rawData = want.GetStringParam("rawData");
    if (gnssInterface_ == nullptr) {
        LBSLOGE(LOCATOR_GNSS, "gnssInterfacev1_0 is nullptr");
        return;
    }
...
void AGnssNiManager::CheckSmsSuplInit(const EventFwk::Want &want)
{
    ...
    if (pdus.empty()) {
        LBSLOGI(LOCATOR_GNSS, "pdus is null");
        return;
    }
...
static void ConnectExtensionAbility(const AAFwk::Want &want, const std::string &commandStr)
{
    ...
    if (connection == nullptr) {
        LBSLOGE(LOCATOR_GNSS, "connect UIExtensionAbilityConnection fail");
        return;
    }
    ...
    LBSLOGI(LOCATOR_GNSS, "connect service extension ability result = %d", ret);
...
{
    LBSLOGI(LOCATOR_GNSS, "ConnectExtension");
    AAFwk::Want want;
...
std::string AGnssNiManager::DecodeNiString(std::string original, int coding)
{
    ...
    switch (coding) {
        ...
        default: {
            LBSLOGE(LOCATOR_GNSS, "unknow encoding %d for NI text %s", coding, original.c_str());
            return original;
        }
    }
}
...
void AGnssNiManager::SendNiNotification(const GnssNiNotificationRequest &notif)
{
    ...
    request.SetNotificationId(LOCATOR_GNSS_AGNSS_NI_NOTIFICATION_ID);
    ...
    if (ret != 0) {
        LBSLOGE(LOCATOR_GNSS, "Publish Notification errorCode = %d", ret);
        return;
    }
    LBSLOGI(LOCATOR_GNSS, "GNSS service publish notification success");
#else
    LBSLOGI(LOCATOR_GNSS, "GNSS service publish notification not support");
#endif
}
...
void AGnssNiManager::HandleNiNotification(const GnssNiNotificationRequest &notif)
{
    ...
    if (LocationDataRdbManager::QuerySwitchState() != ENABLED && !IsInEmergency()) {
        SendUserResponse(LOCATOR_GNSS_NI_RESPONSE_CMD_NO_RESPONSE);
    }
    ...
    if (needNotify) {
        if (needVerify) {
            ...
            SendUserResponse(LOCATOR_GNSS_NI_RESPONSE_CMD_ACCEPT);
        }
    }
...
{
    LBSLOGI(LOCATOR_GNSS, "AGNSS-NI systemAbilityId:%d", systemAbilityId);
    if (systemAbilityId != COMMON_EVENT_SERVICE_ID) {
        LBSLOGE(LOCATOR_GNSS, "systemAbilityId is not COMMON_EVENT_SERVICE_ID");
        return;
    }
    auto agnssNiManager = AGnssNiManager::GetInstance();
    if (agnssNiManager == nullptr) {
        LBSLOGE(LOCATOR_GNSS, "agnssNiManager nullptr");
        return;
    }
    agnssNiManager->Run();
...
#endif // FEATURE_AGNSS_NI_SUPPORT
#endif
