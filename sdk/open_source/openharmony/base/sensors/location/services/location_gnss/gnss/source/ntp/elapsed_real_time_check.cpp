...
void ElapsedRealTimeCheck::CheckRealTime(int64_t time)
{
    if (timeBegin_ == 0) {
        ...
        LBSLOGD(LOCATOR_GNSS, "checkRealTime");
    }

    if ((timeBegin_ != 0) && (timeCheck_ != 0)) {
        int64_t missTime = (timeCheck_ - timeBegin_) - (timeCheckElapsed_ - timeBeginElapsed_);
        LBSLOGD(LOCATOR_GNSS,
            "checkRealTime timeBegin_:%s, timeCheck_:%s, timeBeginElapsed_:%s, "
            "timeCheckElapsed_:%s",
        ...
        LBSLOGI(LOCATOR_GNSS, "checkRealTime missTime:%s", std::to_string(missTime).c_str());
        ...
        if (std::abs(missTime) >= MAX_MISS_MS) {
            LBSLOGI(LOCATOR_GNSS, "checkRealTime false");
