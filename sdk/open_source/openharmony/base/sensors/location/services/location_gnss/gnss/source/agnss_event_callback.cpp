...
{
    LBSLOGI(LOCATOR_GNSS, "AGnssEventCallback::RequestSetUpAgnssDataLink. agnsstype:%d, setUpType:%d",
...
{
    LBSLOGI(LOCATOR_GNSS, "AGnssEventCallback::RequestSubscriberSetId. type:%d", static_cast<int>(type));
    std::string imsi;
    ...
    setId.id = imsi;
    GnssAbility::GetInstance().SetSetId(setId);
    return ERR_OK;
}
...
void AGnssEventCallback::GetWiFiRefInfo(AGnssRefInfo& refInfo)
{
    ...
    if (ptrWifiScan == nullptr) {
        LBSLOGE(LOCATOR_GNSS, "WifiScan get instance failed");
        return;
    }
    int ret = ptrWifiScan->GetScanInfoList(wifiScanInfo);
    if (ret != Wifi::WIFI_OPT_SUCCESS) {
        LBSLOGE(LOCATOR_GNSS, "GetScanInfoList failed");
        return;
    }
    if (wifiScanInfo.size() == 0) {
        LBSLOGE(LOCATOR_GNSS, "empty mac.");
        return;
    }
    uint8_t macArray[MAC_LEN];
    if (CommonUtils::GetMacArray(wifiScanInfo[0].bssid, macArray) != EOK) {
        LBSLOGE(LOCATOR_GNSS, "GetMacArray failed.");
        return;
    }
...
void AGnssEventCallback::GetCellRefInfo(AGnssRefInfo& refInfo)
{
    ...
    if (cellInformations.size() == 0) {
        LBSLOGE(LOCATOR_GNSS, "empty cell info list.");
        return;
    }
    for (sptr<CellInformation> infoItem : cellInformations) {
        if (!infoItem->GetIsCamped()) {
            LBSLOGE(LOCATOR_GNSS, "GetIsCamped return false");
            continue;
        }
...
int32_t AGnssEventCallback::RequestAgnssRefInfo(AGnssRefInfoType type)
{
    ...
    GnssAbility::GetInstance().SetRefInfo(refInfo);
    return ERR_OK;
}
