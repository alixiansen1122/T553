...
static int32_t hks_aes_get_password(uint32_t keyslot, uint8_t *salt, uint32_t salt_length)
{
    errcode_t ret = ERRCODE_SUCC;
    uint32_t klad = 0;
    uapi_drv_klad_attr_t klad_attr = {
        .klad_cfg = {
            .rootkey_type = UAPI_DRV_KDF_HARD_KEY_TYPE_RDRK_REE,
        },
        .key_cfg = {
    ...
    uapi_drv_klad_effective_key_t klad_key = {
        .kdf_hard_alg = UAPI_DRV_KDF_HARD_ALG_SHA256,
        .key_parity = false,
        .key_size = AES_KEY_LENGTH,
        .salt = salt,
    ...
    ret = uapi_drv_klad_set_effective_key(klad, &klad_key);
    if (ret != ERRCODE_SUCC) {
        ret = HKS_ERROR_BAD_STATE;
        goto err_exit;
    }
...
static int32_t hks_crypto_cipher_create(uint32_t *cipher, uint32_t *keyslot)
{
    errcode_t ret = ERRCODE_SUCC;
...
static int32_t hks_aes_cipher_process(const struct HksRkcKsfData *ksfData, uint8_t *src, uint8_t *dst, uint32_t len)
{
    errcode_t ret = ERRCODE_SUCC;
    ...
    uapi_drv_cipher_symc_ctrl_t ctrl = {
        ...
        .iv_length = HKS_RKC_MK_IV_LEN,
    };

    (void)memcpy_s(ctrl.iv, sizeof(ctrl.iv), ksfData->mkIv, HKS_RKC_MK_IV_LEN);

    ret = hks_crypto_cipher_create(&cipher, &keyslot);
    if (ret != ERRCODE_SUCC) {
        return HKS_ERROR_BAD_STATE;
    }
    ...
    ret = hks_aes_get_password(keyslot, ksfData->rmkSalt, AES_SALT_LENGTH);
    if (ret != ERRCODE_SUCC) {
        goto err_exit;
    }
...
#ifdef HKS_HARDWARE_ROOT_KEY
    ret = hks_aes_cipher_process(ksfData, rawKey->data, rawKey->data, rawKey->size);
    HKS_IF_NOT_SUCC_LOGE_RETURN(ret, ret, "hks_aes_cipher_process failed! ret = 0x%" LOG_PUBLIC "X", ret)
#endif
...
static int32_t RkcLoadKsf(void)
{
    ...
    do {
        ...
        HKS_IF_NOT_SUCC_LOGE_BREAK(ret, "All ksf file are invalid! ret = 0x%" LOG_PUBLIC "X", ret)

        ret = RkcRecoverRkTime(validKsfData);
        HKS_IF_NOT_SUCC_LOGE_BREAK(ret, "Recover rook key memory failed! ret = 0x%" LOG_PUBLIC "X", ret)
        ...
        HKS_IF_NOT_SUCC_LOGE(ret, "Check all ksf failed! ret = 0x%" LOG_PUBLIC "X", ret)
    } while (0);
    ...
    struct HksRkcKsfData *ksfData)
{
    ...
    do {
        ...
        HKS_IF_NOT_SUCC_LOGE(ret, "Hash rkMaterial2 failed!")
    } while (0);
...
static int32_t RkcMakeRandomMaterial(struct HksRkcKsfData *ksfData)
{
    ...
    do {
        ...
        HKS_IF_NOT_SUCC_LOGE(ret, "Calc material failed! ret = 0x%" LOG_PUBLIC "X", ret)
    } while (0);
...
static int32_t RkcCreateKsf(void)
{
    ...
    do {
        ...
        HKS_IF_NOT_SUCC_LOGE_BREAK(ret, "Write ksf failed! ret = 0x%" LOG_PUBLIC "X", ret)
    } while (0);
...
static int32_t RkcInitKsf(void)
{
    ...
    HKS_IF_NOT_SUCC_LOGE(ret, "Load/Create ksf failed! ret = 0x%" LOG_PUBLIC "X", ret)
...
int32_t HksRkcInit(void)
{
    ...
    do {
        ...
        HKS_IF_NOT_SUCC_LOGE_BREAK(ret, "Init attribute of keystore file failed! ret = 0x%" LOG_PUBLIC "X", ret)

        /* Initialize the keystore file of root key component. */
        ret = RkcInitKsf();
        HKS_IF_NOT_SUCC_LOGE(ret, "Init root key component failed! ret = 0x%" LOG_PUBLIC "X", ret)
    } while (0);
