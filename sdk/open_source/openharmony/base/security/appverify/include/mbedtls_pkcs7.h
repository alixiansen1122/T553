...
/*
 * For performance test, define this here
...
typedef enum {
    ...
    PKCS7_IS_REVOKED,
    PKCS7_VERIFY_FAIL,
    PKCS7_SAVE_PUB_KEY_FAIL,
    PKCS7_GET_PUB_KEY_FAIL,
    PKCS7_DECRYPT_FAIL,
    PKCS7_FILE_OPERATION_FAIL,
} PKCS7_RetCode;
...
typedef struct {
    unsigned char *hashListData;
    size_t hashListSize;
    char certDirPath[MAX_CERTPATH_LEN];
    time_t verifyTime;
} CertVerifyCache;
...
typedef struct {
    ...
    size_t hashHexLen;
    char hashHex[65]; // 十六进制表示的哈希值
} CertHashEntry;

typedef struct {
    CertHashEntry *entries;
    size_t count;
    size_t capacity;
} CertHashList;
 ...
 *               cert depth, used it for whitelist match.
 * Input       : pkcs7   - pkcs7 signed data header
