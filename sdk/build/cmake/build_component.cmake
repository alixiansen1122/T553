...
macro(build_library)
    ...
    if(EXISTS ${SOURCES0})
        ...
        if(DEFINED LIB)
            ...
            set(TARGET_COMMAND_TMP ${TARGET_COMMAND})
            string(REPLACE "-user" "" TARGET_COMMAND_TMP ${TARGET_COMMAND_TMP})
            message(STATUS "${SOURCES0} is not found, finding lib${COMPONENT_NAME}.a in ${CMAKE_CURRENT_SOURCE_DIR}/${TARGET_COMMAND_TMP}")
            find_library(LINK_LIB_${COMPONENT_NAME} "lib${COMPONENT_NAME}.a" ${CMAKE_CURRENT_SOURCE_DIR}/${TARGET_COMMAND_TMP})
    ...
    if (DEFINED SDK_OUTPUT_PATH)
        if (${COMPONENT_NAME} IN_LIST CLOSED_COMPONENTS)
            ...
            elseif(DEFINED LIB_OUT_PATH)
                ...
                install_dest_sdk(${LIB_PATH}/ ${CMAKE_CURRENT_SOURCE_DIR}/${TARGET_COMMAND} "lib${COMPONENT_NAME}.a")
            endif()
...
if(${AUTO_DEF_FILE_ID})
    foreach(filename ${SOURCES})
        ...
        set_property(
            ...
            THIS_FILE_ID=${barename} THIS_MOD_ID=${mod_name})
    endforeach()
endif()
endmacro(define_this_file_id)
...
macro(build_component)
    ...
    component_on_rom()
    if(DEFINED COMPONENT_ON_ROM)
        need_whole_archiv()
        if(DEFINED AUTO_ADD_PATH AND AUTO_ADD_PATH STREQUAL false)
        else()
        ...
        if (RELEASE OR ${COMPONENT_NAME} IN_LIST CLOSED_COMPONENTS)
