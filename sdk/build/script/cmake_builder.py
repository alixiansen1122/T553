...
from usr_config import mconfig, save_kconfig_env
...
class CMakeBuilder(BuildEnvironment):
    ...
    def build(self):
        ...
        for group in self.group_names:
            ...
            for target in self.get_target_names_by_group_name(group):
                ...
                target_env = TargetEnvironment(target, self.extr_defines, self.extr_ccflags)
    ...
    def menuconfig_to_build(self, target):
        ...
        menuconfig_argv = ["", self.kconfig_param, env.get("chip"), env.get("core"), target, None]
        mconfig(menuconfig_argv)

    def build_target(self, target):
        env = TargetEnvironment(target, self.extr_defines, self.extr_ccflags)
        if env.get('build_type') == 'SDK':
            ...
            application_components = env.get('application_component', cmake_type=False)

            for sdk_target in sdk_pkg_target_name:
                sdk_target_env = TargetEnvironment(sdk_target, self.extr_defines, self.extr_ccflags)
        ...
        if env.get("fs_image"):
            ...
            if "CFG_DRIVERS_NANDFLASH" in defines:
                print("fs img nandflash mode")
                ...
                errcode = exec_shell([self.python_path, fs_script_path], None, True)
                if errcode != 0:
                    print("creat fs image error!")
                    self.print_build_result(target, errcode)
                    sys.exit(1)
                print("nandflash yaffs image generate success!")
            elif "CFG_DRIVERS_MMC" in defines:
                print("fs img emmc mode")
                fs_image_path = os.path.join(root_path, 'build', 'config', 'target_config', env.get('chip'), 'mk_emmc_image',)
            else:
                print("not cfg emmc or nandflash, cfg error!!!\n")
                sys.exit(1)

            # copy file.bin from make dir to application dir
            file_path = os.path.join(fs_image_path, 'file.bin')
            outfile_path = os.path.join(env.get_output_path(), 'file.bin')
            if os.path.exists(file_path):
                shutil.copyfile(file_path, outfile_path)
                print("copy file.bin to application dir over!\n")

        if env.get('packet'):
            if env.get('CONFIG_NORFLASH_SIZE') == '16':
                self.pack_fwpkg(env.get('chip'), target, "0x1000000")
            elif env.get('CONFIG_NORFLASH_SIZE') == '4':
                self.pack_fwpkg(env.get('chip'), target, "0x400000")
            else:
                defines = env.get("defines")
                if env.get('CONFIG_LOAD_FROM_FS'):
                    self.pack_fwpkg(env.get('chip'), target, "0x800000", "True")
                else:
                    self.pack_fwpkg(env.get('chip'), target, "0x800000")
    ...
    def start(self, env, target_name, output_path, nhso=None, clean=None):
        ...
        if os.path.exists(mconfig_file_path):
            menuconfig_argv = ["", "savemenuconfig", chip, core, target_name, output_path]
            mconfig(menuconfig_argv)
            save_kconfig_env(env, mconfig_file_path)
    ...
    def pack_fwpkg(self, chip, target_name, flash_size, no_dsp="False"):
        # bin packet all in one
        packet_script_path = os.path.join(pkg_tools_path, 'packet.py')
        print("packet ing...")
        errcode = exec_shell([self.python_path, packet_script_path, chip, target_name, flash_size, no_dsp], None, True)
        if errcode != 0:
            print("packet error!")
            self.print_build_result(target_name, errcode)
            sys.exit(1)
