...
from utils.build_utils import pkg_tools_path, jlink_tools_path, lzma_tools_path, sign_tools_path
from utils.build_utils import memcheck_tools_path, factory_tools_path, derived_tools_path
from utils.build_utils import ohos_tools_path
from utils.build_utils import xts_tools_path
...
sdk_copy_common_files = [
    ...
    os.path.join(memcheck_tools_path),
    os.path.join(factory_tools_path, 'factory_tools'),
    os.path.join(derived_tools_path),
    os.path.join(ohos_tools_path),
    os.path.join(xts_tools_path),
]
...
class SdkGenerator:
    def __init__(self, env: TargetEnvironment, sdk_root_path: str):
        ...
        self.copy_module = CopyModule(replace_root=sdk_root_path, replace_suffix='.srcrelease', copy_header = False)
    ...
    def copy_menuconfig(self):
        ...
        self.copy_srcs([os.path.join(root_path, 'product')])
    ...
    def sdk_delete_tmp_files(self):
        ...
        for dir_path, dir_names, file_names in os.walk(os.path.join(output_root, 'sdk', 'interim_binary', self.env.get('chip'), 'libs'), topdown=False):
            for name in file_names:
                if name.endswith('.a'):
                    if(name[3:-2] not in self.env.get('closed_components')):
    ...
    def sdk_build(self, build_time, nhso, build_level):
        ...
        for idx, target in enumerate(build_targets):
            ...
            if not compare_path_bin(sdk_output_path, org_output_path):
                # xts用例中存在__FILE__引用，无法比较二进制
                if target != 'brandy-xts':
