...
class TargetEnvironment:
    def __init__(self, target_name, extra_defines=None, extra_cclfgas=None):
        ...
        if extra_cclfgas:
            self.config["ccflags"].extend(extra_cclfgas)
        ...
        if org_target.get('dsp_version') != None:
            self.config['dsp_version'] = org_target.get('dsp_version')
        if org_target.get('ssb_version') != None:
            self.config['ssb_version'] = org_target.get('ssb_version')
        else:
            self.config['ssb_version'] = 'brandy-ssb'
...
class BuildEnvironment:
    ...
    def parse_cmd(self, param_list):
        ...
        for param in param_list:
            if param.startswith('-j'):
            ...
            elif param.startswith('-ccflags='):
                self.extr_ccflags.extend(param[9:].split(','))
                ...
                self.generator = 'Ninja'
            elif param == '-release' or param == '-debug':
