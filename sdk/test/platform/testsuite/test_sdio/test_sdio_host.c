...
static int test_sdio_host(int argc, char *argv[])
{
    ...
    device_t dev = mmcdev_get_child(sc->dev);
    if (dev == NULL) {
        return -1;
    }
    struct mmc_softc *mmc_sc = device_get_softc(dev);

    if (mmc_sc == NULL || mmc_sc->child_list[0] == NULL) {
        return -1;
    }
    device_t child = mmc_sc->child_list[0];
    g_manf_id = (uint16_t)mmc_get_cis_manid(child);
    g_device_id = (uint16_t)mmc_get_cis_proid(child);

    PRINT("get manf_id: 0x%x, device_id: 0x%x\r\n", g_manf_id, g_device_id);
    return 0;
}
...
static int test_sdio_write_byte(int argc, char *argv[])
{
    ...
    int func_num = emmc_parse_nums(argv[0]);
    unsigned long long int addr = emmc_parse_nums(argv[1]);
    uint8_t byte = emmc_parse_nums(argv[2]);
    int err;
    struct sdio_func* func;

    if (func_num == 0) {
        func = sdio_get_func(FUNC_NUM, g_manf_id, g_device_id);
        sdio_func0_write_byte(func, byte, addr, &err);
    } else {
        func = sdio_get_func(func_num, g_manf_id, g_device_id);
...
static int test_sdio_read_byte(int argc, char *argv[])
{
    UNUSED(argc);
    int func_num = emmc_parse_nums(argv[0]);
    unsigned long long addr = emmc_parse_nums(argv[1]);
    int err;
    uint8_t byte;
    struct sdio_func* func;
    ...
    if (func_num == 0) {
        ...
        PRINT("sdio_func0_read_byte result: 0x%x, err = %d\r\n", byte, err);
    } else {
        func = sdio_get_func(func_num, g_manf_id, g_device_id);
        byte = sdio_read_byte(func, addr, &err);
        PRINT("func%d sdio_read_byte result: 0x%x, err = %d\r\n", func_num, byte, err);
    }
    return err;
}
...
static int test_sdio_write_incr_block_addr(int argc, char *argv[])
{
    UNUSED(argc);
    unsigned int addr = emmc_parse_nums(argv[0]);
    unsigned long long size = emmc_parse_nums(argv[1]);
    unsigned long long block_count = emmc_parse_nums(argv[2]);
...
static int test_sdio_read_incr_block_addr(int argc, char *argv[])
{
    UNUSED(argc);
    uint32_t index;
    uint32_t addr = emmc_parse_nums(argv[0]);
    uint64_t size = emmc_parse_nums(argv[1]);
    uint64_t counts = emmc_parse_nums(argv[2]);
    uint64_t loop_times = emmc_parse_nums(argv[3]);
    uint64_t start_time;
    uint64_t end_time;
    int ret = -1;
    ...
    for (index = 0; index < loop_times; ++index) {
        ret = sdio_read_incr_block(func, read_buf, addr, counts);
        if (ret != 0) {
...
void add_sdio_host_case(void)
{
    ...
    uapi_test_suite_add_function("test_sdio_host_write_addr", "sdio_write_incr_block_addr",
                                 test_sdio_write_incr_block_addr);
    uapi_test_suite_add_function("test_sdio_host_read_addr", "sdio_read_incr_block_addr",
                                 test_sdio_read_incr_block_addr);
